...
async function resolveFavoriteBusiness(referenceId: string, type: string) {
  ...
  try {
    if (type === 'BUSINESS') {
      ...
      return { businessId: b?.id || undefined, businessName: b?.name || undefined };
    }
    if (type === 'PRODUCT') {
      ...
      return { businessId: p?.businessId || undefined, businessName: p?.business?.name || undefined };
    }
    if (type === 'SERVICE') {
      ...
      return { businessId: s?.businessId || undefined, businessName: s?.business?.name || undefined };
    }
    if (type === 'TRAINING') {
      ...
      return { businessId: t?.businessId || undefined, businessName: t?.business?.name || undefined };
    }
    if (type === 'EVENT') {
      ...
      return { businessId: e?.businessId || undefined, businessName: e?.business?.name || undefined };
    }
  } catch {
    // silencieux : ne jamais bloquer le favori si la résolution échoue
  }
  return { businessId: undefined, businessName: undefined };
}
...
export async function addFavorite(userId: string, type: string, referenceId: string) {
  ...
  const { businessId, businessName } = await resolveFavoriteBusiness(referenceId, type);
  publishFavoriteAdded({ userId, referenceId, type, businessId, businessName });
}
...
export async function removeFavorite(userId: string, favoriteId: string) {
  ...
  const { businessId, businessName } = await resolveFavoriteBusiness(fav.referenceId, fav.type);
  publishFavoriteRemoved({ userId, referenceId: fav.referenceId, type: fav.type, businessId, businessName });
}
