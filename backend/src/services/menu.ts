...
export async function updateMenuItem(
  ownerId: string,
  itemId: string,
  data: Partial<MenuItemCreateInput>
) {
  const business = await getBusinessByOwner(ownerId);
...
export async function getMenuItemStats(ownerId: string) {
  ...
  return {
    ...
    popular,
    star,
    totalSold: totalSold._sum?.orderCount || 0,
    byStatus,
  };
}
...
export async function createCategory(ownerId: string, data: any) {
  ...
  const existing = await prisma.menuCategory.findUnique({
    where: { businessId_name: { businessId: business.id, name: data.name } },
  });
...
export async function updateCategory(ownerId: string, categoryId: string, data: any) {
  ...
  if (data.parentId === null) upd.parentId = null;
    ...
    if (data.parentId === categoryId) throw new AppError('Auto-référence impossible', 400);
    upd.parentId = data.parentId;
  }
...
export async function deleteCategory(ownerId: string, categoryId: string) {
  ...
  if (existing._count.items > 0)
    throw new AppError("Supprimez d'abord les plats de cette catégorie", 400);
...
export async function listOrders(ownerId: string, filters: any) {
  ...
  if (dateFrom || dateTo) {
    ...
    if (dateTo) createdFilter.lte = new Date(dateTo + 'T23:59:59Z');
...
export async function getOrder(ownerId: string, orderId: string) {
  const business = await getBusinessByOwner(ownerId);
  const order = await prisma.menuOrder.findFirst({
...
export async function updateOrderStatus(
  ...
  if (status === 'CANCELLED')
    updateData.notes = (order as any).notes
      ? `${order.notes}\nCancelled: ${reason || ''}`
      : `Cancelled: ${reason || ''}`;
  const updated = await prisma.menuOrder.update({
    where: { id: orderId },
    data: updateData,
    include: orderInclude,
  });
...
export async function getOrderStats(ownerId: string) {
  ...
  const counts = await Promise.all(
    statuses.map((s) => prisma.menuOrder.count({ where: { ...where, status: s as any } }))
  );
  const [totalRevenue, todayRevenue] = await Promise.all([
    prisma.menuOrder.aggregate({
      where: { ...where, status: { in: ['COMPLETED'] as any } },
      _sum: { total: true },
    }),
    prisma.menuOrder.aggregate({
      where: {
...
export async function createTable(ownerId: string, data: any) {
  ...
  const existing = await prisma.restaurantTable.findUnique({
    where: { businessId_number: { businessId: business.id, number: data.number } },
  });
