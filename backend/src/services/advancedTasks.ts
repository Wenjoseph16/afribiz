  ...
  const task = await prisma.planningTask.findFirst({
    ...
    include: {
      ...
      comments: {
        ...
        include: { author: { select: { id: true, firstName: true, lastName: true, photo: true } } },
      },
...
export async function updateTask(ownerId: string, taskId: string, data: any) {
  ...
  if (data.startDate !== undefined)
    upd.startDate = data.startDate ? new Date(data.startDate) : null;
  ...
  if (data.status && data.status !== existing.status) {
    await logTaskEvent(
      ...
      taskId,
      'TASK_STATUS_CHANGED',
      `${existing.title}: ${existing.status} → ${data.status}`
    );
    if (data.status === 'DONE')
...
export async function reorderTask(
  ...
  await prisma.planningTask.update({
    where: { id: taskId, businessId: business.id },
    data: { status: newStatus as any, sortOrder: newSortOrder },
  });
}
...
export async function getKanbanBoard(ownerId: string, filters: any) {
  ...
  tasks.forEach((task: any) => {
    const colId = ['DONE', 'BLOCKED', 'IN_PROGRESS', 'ON_HOLD'].includes(task.status)
      ? task.status
      : 'TODO';
...
export async function startTimer(ownerId: string, taskId: string, userId: string) {
  await getBusinessByOwner(ownerId);
  await prisma.taskTimer.updateMany({
...
export async function stopTimer(ownerId: string, taskId: string, userId: string) {
  await getBusinessByOwner(ownerId);
  const timer = await prisma.taskTimer.findFirst({
    where: { taskId, userId, endedAt: null },
  ...
  return prisma.taskTimer.update({
    where: { id: timer.id },
    data: { endedAt, durationMs: Math.floor(durationMs / 1000) },
  });
}
...
export async function requestValidation(ownerId: string, taskId: string, data: any) {
  ...
  return prisma.taskValidation.create({
    data: {
      taskId,
      requestedBy: data.requestedBy || ownerId,
  ...
  const [
    ...
    prisma.planningTask.count({
      where: {
        ...where,
        dueDate: { lt: today },
        status: { notIn: ['DONE', 'BLOCKED', 'CANCELLED'] },
      },
    }),
    prisma.taskChecklist.count({ where: { task: { businessId: business.id, deletedAt: null } } }),
    prisma.taskChecklist.count({
      where: { completedAt: { not: null }, task: { businessId: business.id, deletedAt: null } },
    }),
    prisma.taskTimer.aggregate({
      where: { task: { businessId: business.id }, endedAt: { not: null } },
    ...
    try {
      ...
      await prisma.planningTask.create({
        data: {
          ...
          title: `Préparer commande #${order.orderNumber}`,
          description: `Commande ${order.type}`,
          priority: 'HIGH',
          status: 'TODO',
          orderId: order.id,
      ...
      if (order.type === 'DELIVERY') {
        await prisma.planningTask.create({
          data: {
            businessId: businessIdVal,
            title: `Livrer commande #${order.orderNumber}`,
            description: `Adresse: ${order.deliveryAddress || 'Non spécifiée'}`,
            priority: 'HIGH',
            status: 'TODO',
            orderId: order.id,
            clientName: order.contactName || null,
      ...
      logger.error('Auto task from order failed', { error: err });
    }
  });
    ...
    try {
      ...
      await prisma.planningTask.create({
        data: {
          ...
          title:
            booking.type === 'ROOM'
              ? `Préparer chambre pour ${booking.title}`
              : `Préparer service: ${booking.title}`,
          description: `Réservation #${booking.bookingNumber} - ${booking.customerName || 'Client'}`,
          priority: 'MEDIUM',
          status: 'TODO',
          bookingId: booking.id,
          dueDate: booking.startDate || undefined,
          clientName: booking.customerName || null,
        },
      });
    } catch (err) {
      logger.error('Auto task from booking failed', { error: err });
    }
  });
}
...
export async function listTaskHistory(ownerId: string, taskId: string) {
  const business = await getBusinessByOwner(ownerId);
  return prisma.planningLog.findMany({
    where: { businessId: business.id, entityType: 'ADVANCED_TASK', entityId: taskId },
    orderBy: { createdAt: 'desc' },
    take: 50,
  });
}
