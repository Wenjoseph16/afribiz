...
const txProxy = new Proxy({} as any, {
  get(_t, prop) {
    ...
    return {
      create: jest.fn().mockResolvedValue({ id: `${String(prop)}-1` }),
      createMany: jest.fn(),
      update: jest.fn(),
...
jest.mock('../../lib/db', () => ({
  prisma: {
    ...
    portfolioItem: { createMany: jest.fn() },
    subscriptionPlan: { findUnique: jest.fn().mockResolvedValue({ id: 'plan-afribiz' }) },
    user: {
      findUnique: jest.fn().mockResolvedValue({ id: 'user-1', roles: ['CLIENT'] }),
      update: jest.fn(),
    },
...
describe('Onboarding createBusiness', () => {
  ...
  test('Step 2: skills+certifications', async () => {
    const { createBusiness } = require('../../services/business');
    await createBusiness('u1', {
      ...B,
      skills: ['Web'],
      certifications: ['CFA'],
      certificationImages: ['c.webp'],
      experience: 3,
    });
    expect(prisma.$transaction).toHaveBeenCalled();
  });
  ...
  test('Step 4: openingHours', async () => {
    ...
    await createBusiness('u1', {
      ...B,
      openingHours: { lundi: { open: '08:00', close: '18:00', closed: false } },
    });
    expect(prisma.$transaction).toHaveBeenCalled();
  });
});
