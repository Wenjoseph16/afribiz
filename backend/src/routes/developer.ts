...
import {
  ...
  getPayoutHistory,
  requestPayout,
  uploadModuleVersionFile,
} from '../controllers/developerModules';
import {
  getOpenDemands,
  applyToDemand,
  getMyMatchedDemands,
  getDeveloperDemandById,
} from '../controllers/developerDemands';
...
router.post(
  '/modules/:moduleId/versions/:versionId/upload',
  authMiddleware,
  developerMiddleware,
...
router.post(
  ...
  catchAsyncErrors(async (req: AuthenticatedRequest, res: Response) => {
    ...
    res
      ...
      .json(
        successResponse(
          { ...story, isModuleMedia: true, moduleId: id, moduleName: moduleItem.name },
          'Média du module créé'
        )
      );
  })
);
...
router.post(
  '/marketplace/modules/:id/install',
  authMiddleware,
  requireRole(['BUSINESS', 'DEVELOPER']),
  installModule
);
router.post(
  '/marketplace/modules/:id/uninstall',
  authMiddleware,
  requireRole(['BUSINESS', 'DEVELOPER']),
  uninstallModule
);
router.post(
  '/marketplace/modules/:moduleId/reinstall',
  authMiddleware,
  requireRole(['BUSINESS', 'DEVELOPER']),
  reinstallModule
);
router.post(
  '/marketplace/modules/:id/purchase',
  authMiddleware,
  requireRole(['BUSINESS', 'DEVELOPER']),
  purchaseModule
);
router.post(
  '/marketplace/modules/:id/trial',
  authMiddleware,
  requireRole(['BUSINESS', 'DEVELOPER']),
  startTrial
);
...
router.get('/tickets/:id', authMiddleware, developerMiddleware, getTicketById);
router.post(
  '/tickets/:id/messages',
  authMiddleware,
  developerMiddleware,
...
router.get('/copilot/analytics', authMiddleware, developerMiddleware, getDeveloperCopilotCtrl);
...
router.post('/errors/:id/resolve', authMiddleware, developerMiddleware, resolveError);
...
router.post(
  '/validation-checks/:id/approve',
  authMiddleware,
  requireRole(['ADMIN']),
  approveValidationCheck
);
router.post(
  '/validation-checks/:id/reject',
  authMiddleware,
  requireRole(['ADMIN']),
  rejectValidationCheck
);
router.post(
  '/validations/:id/complete',
  authMiddleware,
  requireRole(['ADMIN']),
  completeValidation
);
...
router.put(
  '/modules/:id/configuration/toggle',
  authMiddleware,
  developerMiddleware,
  toggleModuleActive
);
router.get(
  '/modules/:id/configurations',
  authMiddleware,
  developerMiddleware,
  getModuleConfigurations
);
router.get(
  '/configurations/business/:businessId',
  authMiddleware,
  developerMiddleware,
  getBusinessModules
);
...
router.get('/modules/:id/activity/stats', authMiddleware, developerMiddleware, getActivityStats);
router.get('/activity/feed', authMiddleware, developerMiddleware, getDeveloperActivityFeed);
router.get(
  '/activity/business/:businessId',
  authMiddleware,
  developerMiddleware,
  getBusinessActivityFeed
);
...
router.get('/demands/open', authMiddleware, developerMiddleware, getOpenDemands);
router.get('/demands/my', authMiddleware, developerMiddleware, getMyMatchedDemands);
router.get('/demands/my/:id', authMiddleware, developerMiddleware, getDeveloperDemandById);
router.post('/demands/:id/apply', authMiddleware, developerMiddleware, applyToDemand);
