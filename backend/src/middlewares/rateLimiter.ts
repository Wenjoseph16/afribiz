 ...
 * Idem pour l'ensemble des limiteurs : le dashboard fait du polling
 * régulier (notifications, stats, business…) et les tests E2E
 * enchaînent plusieurs sessions complètes dans la même fenêtre.
 */
 ...
 */
export const resendLimiter = isDev
  ? passThrough
  : rateLimit({
      windowMs: 60 * 60 * 1000,
      max: 3,
 ...
 */
export const sensitiveLimiter = isDev
  ? passThrough
  : rateLimit({
      windowMs: 15 * 60 * 1000,
 ...
 */
export const adminLimiter = isDev
  ? passThrough
  : rateLimit({
      windowMs: 15 * 60 * 1000,
 ...
 */
export const strictLimiter = isDev
  ? passThrough
  : rateLimit({
      windowMs: 15 * 60 * 1000,
      max: 20,
 ...
 */
export const roleBasedLimiter = isDev
  ? passThrough
  : rateLimit({
      windowMs: 15 * 60 * 1000,
      max: (req) => {
 ...
 */
export const webhookLimiter = isDev
  ? passThrough
  : rateLimit({
      windowMs: 60 * 1000,
      max: 30,
