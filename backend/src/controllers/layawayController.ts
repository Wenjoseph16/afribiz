...
export const createOffer = catchAsyncErrors(async (req: AuthenticatedRequest, res: Response) => {
  ...
  const result = await layawayService.createLayawayOffer(req.user.id, { itemType, itemId, durationDays, minInstallment });
...
export const activeOffersBatch = catchAsyncErrors(async (req: AuthenticatedRequest, res: Response) => {
  const itemType = String(req.query.itemType || '');
...
export const createPlan = catchAsyncErrors(async (req: AuthenticatedRequest, res: Response) => {
  ...
  res.status(201).json(successResponse(plan, "Plan épargne créé — votre argent est sécurisé en escrow"));
});
...
export const contribute = catchAsyncErrors(async (req: AuthenticatedRequest, res: Response) => {
  ...
  if (!amount || amount <= 0) return res.status(400).json(successResponse(null, 'Montant invalide'));
  const result = await layawayService.contributeToLayaway(req.user.id, req.params.id, { amount, method, phone });
