...
export const getModulePermissions = catchAsyncErrors(
...
export const addModulePermission = catchAsyncErrors(
  async (req: AuthenticatedRequest, res: Response) => {
    ...
    res
      .status(201)
      .json({ success: true, data: permission, message: 'Permission ajoutée avec succès' });
  }
);

export const removeModulePermission = catchAsyncErrors(
  async (req: AuthenticatedRequest, res: Response) => {
    if (!req.user) throw new AppError('Non authentifié', 401);
    const { id } = req.params;
...
export const checkModulePermissions = catchAsyncErrors(
  async (req: AuthenticatedRequest, res: Response) => {
    const { id } = req.params;
    const businessId = req.query.businessId as string;
    if (!businessId) throw new AppError('businessId requis', 400);
    const result = await permissionsService.checkModulePermissions(id, businessId);
    res.json({ success: true, data: result });
  }
);

export const getPermissionSummary = catchAsyncErrors(
  async (req: AuthenticatedRequest, res: Response) => {
    const { id } = req.params;
...
export const activateLicense = catchAsyncErrors(
  async (req: AuthenticatedRequest, res: Response) => {
    const { licenseKey } = req.body;
    if (!licenseKey) throw new AppError('Clé de licence requise', 400);
...
export const getWebhookDeliveries = catchAsyncErrors(
  async (req: AuthenticatedRequest, res: Response) => {
    const { id } = req.params;
...
export const checkLicense = catchAsyncErrors(async (req: AuthenticatedRequest, res: Response) => {
  ...
  if (!moduleId || !businessId) {
    throw new AppError('moduleId et businessId requis', 400);
  }
  const result = await licensesService.checkLicense(moduleId, businessId);
  res.json({ success: true, data: result });
});

export const getModuleLicenses = catchAsyncErrors(
  async (req: AuthenticatedRequest, res: Response) => {
    const { id } = req.params;
...
export const getBusinessLicenses = catchAsyncErrors(
  async (req: AuthenticatedRequest, res: Response) => {
...
export const getLicenseStats = catchAsyncErrors(
  async (req: AuthenticatedRequest, res: Response) => {
    if (!req.user) throw new AppError('Non authentifié', 401);
...
export const getModuleAnalytics = catchAsyncErrors(
  async (req: AuthenticatedRequest, res: Response) => {
    const { id } = req.params;
...
export const getModuleErrors = catchAsyncErrors(
  async (req: AuthenticatedRequest, res: Response) => {
    const { id } = req.params;
...
export const submitForValidation = catchAsyncErrors(
  async (req: AuthenticatedRequest, res: Response) => {
    if (!req.user) throw new AppError('Non authentifié', 401);
    const { id } = req.params;
    ...
    res
      .status(201)
      .json({ success: true, data: validation, message: 'Module soumis à validation' });
  }
);

export const approveValidationCheck = catchAsyncErrors(
  async (req: AuthenticatedRequest, res: Response) => {
    const { id } = req.params;
...
export const rejectValidationCheck = catchAsyncErrors(
  async (req: AuthenticatedRequest, res: Response) => {
    const { id } = req.params;
...
export const completeValidation = catchAsyncErrors(
  async (req: AuthenticatedRequest, res: Response) => {
    if (!req.user) throw new AppError('Non authentifié', 401);
    const { id } = req.params;
...
export const getModuleValidation = catchAsyncErrors(
  async (req: AuthenticatedRequest, res: Response) => {
    const { id } = req.params;
...
export const getPendingValidations = catchAsyncErrors(
  async (req: AuthenticatedRequest, res: Response) => {
...
export const getValidationHistory = catchAsyncErrors(
  async (req: AuthenticatedRequest, res: Response) => {
    const { id } = req.params;
...
export const saveModuleConfiguration = catchAsyncErrors(
  async (req: AuthenticatedRequest, res: Response) => {
    const { id } = req.params;
    const { businessId, installationId, settings } = req.body;
    if (!businessId || !installationId) {
      throw new AppError('businessId et installationId requis', 400);
    }
    const config = await configurationService.saveModuleConfiguration(
      id,
      businessId,
      installationId,
      settings
...
export const getModuleConfiguration = catchAsyncErrors(
  async (req: AuthenticatedRequest, res: Response) => {
    const { id } = req.params;
    const businessId = req.query.businessId as string;
    if (!businessId) throw new AppError('businessId requis', 400);
...
export const toggleModuleActive = catchAsyncErrors(
  async (req: AuthenticatedRequest, res: Response) => {
    const { id } = req.params;
    ...
    res.json({
      success: true,
      data: config,
      message: isActive ? 'Module activé' : 'Module désactivé',
    });
  }
);

export const getModuleConfigurations = catchAsyncErrors(
  async (req: AuthenticatedRequest, res: Response) => {
    const { id } = req.params;
...
export const getBusinessModules = catchAsyncErrors(
  async (req: AuthenticatedRequest, res: Response) => {
    const { businessId } = req.params;
...
export const getModuleActivity = catchAsyncErrors(
  async (req: AuthenticatedRequest, res: Response) => {
    const { id } = req.params;
    const limit = req.query.limit ? parseInt(req.query.limit as string) : 50;
...
export const getDeveloperActivityFeed = catchAsyncErrors(
  async (req: AuthenticatedRequest, res: Response) => {
    if (!req.user) throw new AppError('Non authentifié', 401);
    const profile = await DeveloperRepository.findByUserId(req.user.id);
    if (!profile) throw new AppError('Profil développeur non trouvé', 404);
...
export const getBusinessActivityFeed = catchAsyncErrors(
  async (req: AuthenticatedRequest, res: Response) => {
    const { businessId } = req.params;
    const limit = req.query.limit ? parseInt(req.query.limit as string) : 50;
...
export const getActivityStats = catchAsyncErrors(
  async (req: AuthenticatedRequest, res: Response) => {
    const { id } = req.params;
...
export const getDeveloperAnalyticsOverview = catchAsyncErrors(
  async (req: AuthenticatedRequest, res: Response) => {
    if (!req.user) throw new AppError('Non authentifié', 401);
    const profile = await DeveloperRepository.findByUserId(req.user.id);
    if (!profile) throw new AppError('Profil développeur non trouvé', 404);
...
export const getDeveloperCopilotCtrl = catchAsyncErrors(
  async (req: AuthenticatedRequest, res: Response) => {
    if (!req.user) throw new AppError('Non authentifié', 401);
    const analytics = await getDeveloperAnalytics(req.user.id);
    res.json({ success: true, data: analytics });
  }
);
