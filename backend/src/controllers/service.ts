...
export const listServices = catchAsyncErrors(async (req: AuthenticatedRequest, res: Response) => {
  ...
  const result = await serviceService.listServices(req.user.id, {
...
export const toggleServiceActive = catchAsyncErrors(
...
export const getServiceStats = catchAsyncErrors(
  async (req: AuthenticatedRequest, res: Response) => {
    if (!req.user) throw new AppError('Non authentifi\u00e9', 401);
...
export const duplicateService = catchAsyncErrors(
  async (req: AuthenticatedRequest, res: Response) => {
    if (!req.user) throw new AppError('Non authentifié', 401);
...
export const exportServices = catchAsyncErrors(async (req: AuthenticatedRequest, res: Response) => {
  if (!req.user) throw new AppError('Non authentifié', 401);
  const result = await serviceService.exportServices(
    req.user.id,
...
export const bulkDeleteServices = catchAsyncErrors(
  async (req: AuthenticatedRequest, res: Response) => {
    if (!req.user) throw new AppError('Non authentifié', 401);
...
export const bulkToggleServices = catchAsyncErrors(
  async (req: AuthenticatedRequest, res: Response) => {
    if (!req.user) throw new AppError('Non authentifié', 401);
    const result = await serviceService.bulkToggleServices(
      req.user.id,
      req.body.ids,
      req.body.isActive
    );
