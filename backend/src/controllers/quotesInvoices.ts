...
export const updateQuoteStatus = catchAsyncErrors(
  async (req: AuthenticatedRequest, res: Response) => {
    if (!req.user) throw new AppError('Non authentifié', 401);
    const data = await qiService.updateQuoteStatus(
      req.user.id,
      req.params.id,
      req.body.status,
      req.body.reason
...
export const convertQuoteToInvoice = catchAsyncErrors(
  async (req: AuthenticatedRequest, res: Response) => {
    if (!req.user) throw new AppError('Non authentifié', 401);
...
export const updateInvoiceStatus = catchAsyncErrors(
  async (req: AuthenticatedRequest, res: Response) => {
    if (!req.user) throw new AppError('Non authentifié', 401);
    const data = await qiService.updateInvoiceStatus(
      req.user.id,
      req.params.id,
      req.body.status,
      req.body.reason
...
export const updateInvoicePayment = catchAsyncErrors(
  async (req: AuthenticatedRequest, res: Response) => {
    if (!req.user) throw new AppError('Non authentifié', 401);
...
export const downloadInvoicePdf = catchAsyncErrors(
  async (req: AuthenticatedRequest, res: Response) => {
    if (!req.user) throw new AppError('Non authentifié', 401);
...
export const downloadQuotePdf = catchAsyncErrors(
  async (req: AuthenticatedRequest, _res: Response) => {
    if (!req.user) throw new AppError('Non authentifié', 401);
    throw new AppError('G\u00e9n\u00e9ration PDF pour les devis bient\u00f4t disponible', 501);
  }
);
