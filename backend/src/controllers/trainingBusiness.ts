...
export const listBusinessTrainings = catchAsyncErrors(
...
export const getBusinessTraining = catchAsyncErrors(
  async (req: AuthenticatedRequest, res: Response) => {
    if (!req.user) throw new AppError('Non authentifie', 401);
...
export const createBusinessTraining = catchAsyncErrors(
  async (req: AuthenticatedRequest, res: Response) => {
    if (!req.user) throw new AppError('Non authentifie', 401);
...
export const updateBusinessTraining = catchAsyncErrors(
  async (req: AuthenticatedRequest, res: Response) => {
    if (!req.user) throw new AppError('Non authentifie', 401);
    const result = await trainingBizService.updateBusinessTraining(
      req.user.id,
      req.params.id,
      req.body
    );
...
export const deleteBusinessTraining = catchAsyncErrors(
  async (req: AuthenticatedRequest, res: Response) => {
    if (!req.user) throw new AppError('Non authentifie', 401);
...
export const getTrainingStudents = catchAsyncErrors(
  async (req: AuthenticatedRequest, res: Response) => {
    if (!req.user) throw new AppError('Non authentifie', 401);
    const result = await trainingBizService.getTrainingStudents(
      req.user.id,
      req.params.id,
      req.query
    );
...
export const getTrainingStats = catchAsyncErrors(
  async (req: AuthenticatedRequest, res: Response) => {
    if (!req.user) throw new AppError('Non authentifie', 401);
