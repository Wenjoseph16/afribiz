...
export const submitQuizAttempt = catchAsyncErrors(
  async (req: AuthenticatedRequest, res: Response) => {
    if (!req.user) throw new AppError('Non authentifié', 401);
    const result = await trainingSvc.submitQuizAttempt(
      req.user.id,
      req.params.quizId,
      req.body.answers
    );
    res.json({
      success: true,
      data: result,
      message: result.passed ? 'Quiz réussi !' : 'Quiz échoué, réessayez',
    });
  }
);

export const getQuizAttempts = catchAsyncErrors(
  async (req: AuthenticatedRequest, res: Response) => {
    if (!req.user) throw new AppError('Non authentifié', 401);
...
export const getTrainingProgress = catchAsyncErrors(
  async (req: AuthenticatedRequest, res: Response) => {
    if (!req.user) throw new AppError('Non authentifié', 401);
