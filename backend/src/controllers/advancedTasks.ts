...
export const createTask = catchAsyncErrors(async (req: AuthenticatedRequest, res: Response) => {
  ...
  res.status(201).json({
    success: true,
    data: await s.createTask(req.user.id, req.body),
    message: 'Tâche créée',
  });
});
...
export const updateTask = catchAsyncErrors(async (req: AuthenticatedRequest, res: Response) => {
  ...
  res.json({
    success: true,
    data: await s.updateTask(req.user.id, req.params.id, req.body),
...
export const addChecklistItem = catchAsyncErrors(
  async (req: AuthenticatedRequest, res: Response) => {
    if (!req.user) throw new AppError('Non authentifié', 401);
    res.status(201).json({
      success: true,
      data: await s.addChecklistItem(req.user.id, req.params.id, req.body),
    });
  }
);

export const toggleChecklistItem = catchAsyncErrors(
  async (req: AuthenticatedRequest, res: Response) => {
    if (!req.user) throw new AppError('Non authentifié', 401);
    res.json({
      success: true,
      data: await s.toggleChecklistItem(req.user.id, req.params.id, req.params.itemId),
    });
  }
);

export const deleteChecklistItem = catchAsyncErrors(
  async (req: AuthenticatedRequest, res: Response) => {
    if (!req.user) throw new AppError('Non authentifié', 401);
...
export const addComment = catchAsyncErrors(async (req: AuthenticatedRequest, res: Response) => {
  ...
  res.status(201).json({
    success: true,
    data: await s.addComment(req.user.id, req.params.id, req.body, req.user.id),
  });
});
...
export const addResource = catchAsyncErrors(async (req: AuthenticatedRequest, res: Response) => {
  ...
  res
    .status(201)
    .json({ success: true, data: await s.addResource(req.user.id, req.params.id, req.body) });
});
...
export const requestValidation = catchAsyncErrors(
  async (req: AuthenticatedRequest, res: Response) => {
    if (!req.user) throw new AppError('Non authentifié', 401);
    res.status(201).json({
      success: true,
      data: await s.requestValidation(req.user.id, req.params.id, req.body),
    });
  }
);

export const approveValidation = catchAsyncErrors(
  async (req: AuthenticatedRequest, res: Response) => {
    if (!req.user) throw new AppError('Non authentifié', 401);
    res.json({
      success: true,
      data: await s.approveValidation(
        req.user.id,
        req.params.id,
...
export const listTaskHistory = catchAsyncErrors(
  async (req: AuthenticatedRequest, res: Response) => {
    if (!req.user) throw new AppError('Non authentifié', 401);
