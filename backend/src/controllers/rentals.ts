...
export const downloadRentalContract = catchAsyncErrors(
  async (req: AuthenticatedRequest, res: Response) => {
    ...
    const booking = await prisma.booking.findFirst({
      where: {
        id: req.params.id,
        rentalId: { not: null },
        OR: [{ clientId: req.user.id }, ...(business ? [{ businessId: business.id }] : [])],
      },
      include: {
        rental: true,
...
export const toggleRentalActive = catchAsyncErrors(
  async (req: AuthenticatedRequest, res: Response) => {
    if (!req.user) throw new AppError('Non authentifié', 401);
...
export const createRentalBooking = catchAsyncErrors(
  async (req: AuthenticatedRequest, res: Response) => {
    if (!req.user) throw new AppError('Non authentifié', 401);
...
export const prolongRentalBooking = catchAsyncErrors(
  async (req: AuthenticatedRequest, res: Response) => {
    if (!req.user) throw new AppError('Non authentifié', 401);
