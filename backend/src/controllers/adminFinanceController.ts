  ...
  async (req: AuthenticatedRequest, res: Response) => {
    ...
    const [
      ...
      prisma.paymentTransaction.aggregate({
        where: { status: 'SUCCESS', createdAt: thirtyDays },
        _sum: { amount: true },
      }),
      prisma.paymentTransaction.aggregate({
        where: { status: 'SUCCESS', createdAt: thirtyDays },
    ...
    res.json({
      ...
      data: {
        ...
        revenue: {
          total30d: Number(totalRevenue._sum.amount || 0),
          fees30d: Number(platformFees._sum.fee || 0),
        },
        escrows: {
          active: activeEscrows,
          totalHeld: Number(escrowAmount._sum.amount || 0),
          disputes: disputedEscrows,
        },
        debts: {
          active: activeDebts,
          totalOwed: Number(debtAmount._sum.remainingAmount || 0),
...
export const getAdminAllTransactions = catchAsyncErrors(
  async (req: AuthenticatedRequest, res: Response) => {
    if (!req.user?.roles?.includes('ADMIN')) throw new AppError('Accès refusé', 403);
    ...
    res.json({
      ...
      data: {
        transactions: transactions.map((t) => ({
          ...t,
          businessName: (t as Record<string, any>).business?.name || null,
        })),
        total,
        page: pageNum,
        limit: limitNum,
...
export const getAdminAllEscrows = catchAsyncErrors(
  async (req: AuthenticatedRequest, res: Response) => {
    if (!req.user?.roles?.includes('ADMIN')) throw new AppError('Accès refusé', 403);
    ...
    res.json({
      ...
      data: {
        escrows: escrows.map((e) => ({
          ...e,
          businessName: (e as Record<string, any>).business?.name || null,
        })),
        total,
        page: pageNum,
        limit: limitNum,
...
export const getAdminFraudAlerts = catchAsyncErrors(
  async (req: AuthenticatedRequest, res: Response) => {
    if (!req.user?.roles?.includes('ADMIN')) throw new AppError('Accès refusé', 403);
    ...
    for (const e of disputedEscrows) {
      alerts.push({
        ...
        severity: 'HIGH',
        escrowId: e.id,
        businessName: (e as Record<string, any>).business?.name,
    ...
    const pendingVerification = await prisma.payment.findMany({
      where: { status: 'VERIFYING' as PaymentStatus, createdAt: { lte: twoDaysAgo } },
...
export const getAdminDebtRecovery = catchAsyncErrors(
  async (req: AuthenticatedRequest, res: Response) => {
    if (!req.user?.roles?.includes('ADMIN')) throw new AppError('Accès refusé', 403);
    ...
    const recoveryRate =
      Number(totalAmount._sum.totalAmount || 0) > 0
        ? Math.round(
            (Number(recoveredAmount._sum.amountPaid || 0) /
              Number(totalAmount._sum.totalAmount || 1)) *
              100
          )
        : 0;
