...
export const createSignatureReq = catchAsyncErrors(
...
export const signDocumentCtrl = catchAsyncErrors(
  async (req: AuthenticatedRequest, res: Response) => {
...
export const verifySignatureCtrl = catchAsyncErrors(
  async (req: AuthenticatedRequest, res: Response) => {
...
export const listSignatureReqs = catchAsyncErrors(
  async (req: AuthenticatedRequest, res: Response) => {
    if (!req.user) throw new AppError('Non authentifié', 401);
    const result = await signatureService.listSignatureRequests(
      req.user.id,
