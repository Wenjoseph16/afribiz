...
export const listBusinessBookings = catchAsyncErrors(
...
export const getBusinessBooking = catchAsyncErrors(
  async (req: AuthenticatedRequest, res: Response) => {
    if (!req.user) throw new AppError('Non authentifi\u00e9', 401);
...
export const getBookingStats = catchAsyncErrors(
  async (req: AuthenticatedRequest, res: Response) => {
    if (!req.user) throw new AppError('Non authentifi\u00e9', 401);
...
export const getCalendarBookings = catchAsyncErrors(
  async (req: AuthenticatedRequest, res: Response) => {
    if (!req.user) throw new AppError('Non authentifi\u00e9', 401);
    const data = await bookingService.getCalendarBookings(
      req.user.id,
...
export const sendReminder = catchAsyncErrors(async (req: AuthenticatedRequest, res: Response) => {
  if (!req.user) throw new AppError('Non authentifi\u00e9', 401);
  const data = await bookingService.sendReminder(
    req.user.id,
    req.params.id,
    req.body.type,
    req.body.channel
...
export const cancelMyBooking = catchAsyncErrors(
  async (req: AuthenticatedRequest, res: Response) => {
    if (!req.user) throw new AppError('Non authentifi\u00e9', 401);
    const booking = await prisma.booking.findFirst({
      where: { id: req.params.id, clientId: req.user.id },
    ...
    if (!booking) throw new AppError('R\u00e9servation introuvable', 404);
    if (booking.status === 'CANCELLED') throw new AppError('D\u00e9j\u00e0 annul\u00e9e', 400);
...
export const rescheduleMyBooking = catchAsyncErrors(
  async (req: AuthenticatedRequest, res: Response) => {
    if (!req.user) throw new AppError('Non authentifi\u00e9', 401);
    const booking = await prisma.booking.findFirst({
      where: { id: req.params.id, clientId: req.user.id },
    ...
    if (booking.status === 'CANCELLED')
      throw new AppError('Impossible de reporter une r\u00e9servation annul\u00e9e', 400);
