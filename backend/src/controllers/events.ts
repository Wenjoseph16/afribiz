...
export const updateTicket = catchAsyncErrors(async (req: AuthenticatedRequest, res: Response) => {
  if (!req.user) throw new AppError('Non authentifié', 401);
  const result = await eventService.updateTicket(
    req.user.id,
    req.params.id,
    req.params.ticketId,
    req.body
...
// ===== Participants =====
export const listParticipants = catchAsyncErrors(
...
export const clientRegisterForEvent = catchAsyncErrors(
  async (req: AuthenticatedRequest, res: Response) => {
    if (!req.user) throw new AppError('Non authentifié', 401);
...
export const updateParticipantStatus = catchAsyncErrors(
  async (req: AuthenticatedRequest, res: Response) => {
    if (!req.user) throw new AppError('Non authentifié', 401);
...
export const deletePromotion = catchAsyncErrors(
  async (req: AuthenticatedRequest, res: Response) => {
    if (!req.user) throw new AppError('Non authentifié', 401);
    const result = await eventService.deletePromotion(
      req.user.id,
      req.params.id,
      req.params.promoId
    );
...
export const deleteGalleryItem = catchAsyncErrors(
  async (req: AuthenticatedRequest, res: Response) => {
    if (!req.user) throw new AppError('Non authentifié', 401);
    const result = await eventService.deleteGalleryItem(
      req.user.id,
      req.params.id,
      req.params.itemId
    );
...
export const registerPublicParticipant = catchAsyncErrors(
  async (req: AuthenticatedRequest, res: Response) => {
    const result = await eventService.registerPublicParticipant(
      req.params.slug,
      req.params.eventId,
      req.body
    );
...
export const getMyTicket = catchAsyncErrors(async (req: AuthenticatedRequest, res: Response) => {
  if (!req.user) throw new AppError('Non authentifié', 401);
  const result = await eventService.getMyTicket(req.user.id, req.params.eventId);
  res.json({ success: true, data: result });
});
