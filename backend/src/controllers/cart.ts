...
export const guestCheckoutCtrl = catchAsyncErrors(
  async (req: AuthenticatedRequest, res: Response) => {
    const data = await cartService.guestCheckout(req.body);
    res.status(201).json(successResponse(data, 'Commande invitée créée avec succès'));
  }
);
