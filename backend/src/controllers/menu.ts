...
export const toggleMenuItemActive = catchAsyncErrors(
...
export const updateMenuItemStatus = catchAsyncErrors(
  async (req: AuthenticatedRequest, res: Response) => {
    if (!req.user) throw new AppError('Non authentifi\u00e9', 401);
    const data = await menuService.updateMenuItemStatus(
      req.user.id,
      req.params.id,
      req.body.status
    );
...
export const updateOrderStatus = catchAsyncErrors(
  async (req: AuthenticatedRequest, res: Response) => {
    if (!req.user) throw new AppError('Non authentifi\u00e9', 401);
    const data = await menuService.updateOrderStatus(
      req.user.id,
      req.params.id,
      req.body.status,
...
// Ingredients
export const listIngredients = catchAsyncErrors(
  async (req: AuthenticatedRequest, res: Response) => {
    if (!req.user) throw new AppError('Non authentifi\u00e9', 401);
...
export const createIngredient = catchAsyncErrors(
  async (req: AuthenticatedRequest, res: Response) => {
    if (!req.user) throw new AppError('Non authentifi\u00e9', 401);
...
export const updateIngredient = catchAsyncErrors(
  async (req: AuthenticatedRequest, res: Response) => {
    if (!req.user) throw new AppError('Non authentifi\u00e9', 401);
...
export const deleteIngredient = catchAsyncErrors(
  async (req: AuthenticatedRequest, res: Response) => {
    if (!req.user) throw new AppError('Non authentifi\u00e9', 401);
...
export const adjustStock = catchAsyncErrors(async (req: AuthenticatedRequest, res: Response) => {
  if (!req.user) throw new AppError('Non authentifi\u00e9', 401);
  const data = await menuService.adjustStock(
    req.user.id,
    req.params.id,
    req.body.type,
    req.body.quantity,
...
export const getIngredientStats = catchAsyncErrors(
  async (req: AuthenticatedRequest, res: Response) => {
    if (!req.user) throw new AppError('Non authentifi\u00e9', 401);
