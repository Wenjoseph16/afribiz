...
export const createRoom = catchAsyncErrors(async (req: AuthenticatedRequest, res: Response) => {
  ...
  res
    .status(201)
    .json({ success: true, data: room, message: 'Chambre cr\u00e9\u00e9e avec succ\u00e8s' });
});
...
export const toggleRoomActive = catchAsyncErrors(
  async (req: AuthenticatedRequest, res: Response) => {
    ...
    res.json({
      success: true,
      data: room,
      message: room.isActive ? 'Chambre activ\u00e9e' : 'Chambre d\u00e9sactiv\u00e9e',
    });
  }
);

export const updateRoomStatus = catchAsyncErrors(
  async (req: AuthenticatedRequest, res: Response) => {
    if (!req.user) throw new AppError('Non authentifi\u00e9', 401);
...
export const bulkDeleteRooms = catchAsyncErrors(
  async (req: AuthenticatedRequest, res: Response) => {
    if (!req.user) throw new AppError('Non authentifié', 401);
...
export const bulkToggleRooms = catchAsyncErrors(
  async (req: AuthenticatedRequest, res: Response) => {
    if (!req.user) throw new AppError('Non authentifié', 401);
