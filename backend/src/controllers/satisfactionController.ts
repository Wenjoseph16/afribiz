...
export const submitSurvey = catchAsyncErrors(async (req: AuthenticatedRequest, res: Response) => {
...
export const getContext = catchAsyncErrors(async (req: AuthenticatedRequest, res: Response) => {
  if (!req.user) throw new AppError('Authentification requise', 401);
  const { orderId, bookingId } = req.query as Record<string, string | undefined>;
  const data = await satisfactionService.getSatisfactionContext(req.user.id, orderId, bookingId);
