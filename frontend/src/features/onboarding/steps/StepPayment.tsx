...
export function StepPayment({ paymentMethods, onChange }: Props) {
  ...
  const addMethod = (method: string, name: string) => {
    ...
    onChange([...paymentMethods, { method, name: '', number: '', isActive: true }]);
    setEditing(method);
  };

  const updateMethod = (method: string, field: 'name' | 'number', value: string) => {
    onChange(paymentMethods.map((m) => (m.method === method ? { ...m, [field]: value } : m)));
  };
  ...
  const toggleActive = (method: string) => {
    onChange(
      paymentMethods.map((m) => (m.method === method ? { ...m, isActive: !m.isActive } : m))
    );
  };
    ...
    <div className="space-y-6">
      <div>
        ...
        <p className="text-sm text-gray-500 dark:text-gray-400 mt-1">
          Configurez les moyens de paiement que vos clients pourront utiliser. Vous pourrez les
          modifier plus tard dans vos paramètres.
        </p>
      </div>

      <div className="bg-amber-50 dark:bg-amber-900/10 border border-amber-200 dark:border-amber-800/30 rounded-xl p-4">
        <p className="text-sm text-amber-800 dark:text-amber-300 font-medium mb-1">Conseil</p>
        <p className="text-xs text-amber-700 dark:text-amber-400">
          Activez au moins Mobile Money (TMoney ou Flooz) — c'est le moyen de paiement le plus
          utilise en Afrique de l'Ouest. Wave est aussi tres populaire.
        </p>
      </div>
              ...
              {(isEditing || (existing && !existing.name)) && (
                <div className="mt-2 ml-4 pl-4 border-l-2 border-brand/30 space-y-3 p-4 bg-gray-50 dark:bg-gray-800/50 rounded-lg">
                  ...
                  <Input
                    ...
                    placeholder={
                      provider.method === 'BANK_TRANSFER' ? 'Numero de compte' : '+228 XX XX XX XX'
                    }
