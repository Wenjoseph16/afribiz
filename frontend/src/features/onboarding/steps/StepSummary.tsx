...
import { ShieldCheck, Pencil, MapPin, Globe, User, ShoppingBag, Smartphone } from 'lucide-react';
    ...
    <div className="border border-gray-200 dark:border-gray-700 rounded-xl p-5">
      <div className="flex items-center justify-between mb-4">
        ...
        {onEdit && (
          <button
            onClick={onEdit}
            className="p-1.5 rounded hover:bg-gray-100 dark:hover:bg-gray-700 text-gray-400 dark:text-gray-500 hover:text-brand dark:hover:text-brand-400 transition-colors"
          >
...
function Field({ label, value }: { label: string; value: string | number | undefined | null }) {
  ...
  return (
    <div className="flex justify-between py-1.5 border-b border-gray-100 dark:border-gray-700 last:border-0">
      ...
      <span className="text-sm font-medium text-gray-900 dark:text-gray-100 text-right max-w-[60%] truncate">
        {value}
      </span>
    </div>
  );
    ...
    <div className="space-y-6">
      ...
      {(data.managerName || data.experience) && (
        <Section title="Gérant" icon={User}>
          ...
          <Field
            label="Expérience"
            value={data.experience ? `${data.experience} ans` : undefined}
          />
          ...
          <div className="flex flex-wrap gap-2 mt-1">
            {data.modules.map((key) => {
              ...
              return mod ? (
                <span
                  key={key}
                  className="inline-flex items-center gap-1 px-3 py-1 bg-brand/5 text-brand text-xs font-medium rounded-full border border-brand/10"
                >
                  {mod.label}
                </span>
              ) : null;
      ...
      {/* Verification Level */}
      <Section title="Niveau de confiance" icon={ShieldCheck}>
        <div className="bg-amber-50 dark:bg-amber-900/20 border border-amber-200 dark:border-amber-700/50 rounded-lg p-3">
          <p className="text-sm text-amber-800 dark:text-amber-200 font-medium">Niveau Argent</p>
          <p className="text-xs text-amber-600 dark:text-amber-400 mt-1">
            Vous commencez au niveau Argent. Après votre inscription, vous pourrez passer au niveau
            Or puis Platine depuis votre tableau de bord pour débloquer des limites plus élevées et
