...
const inputBase =
  'w-full h-11 px-4 text-sm rounded-xl border bg-white dark:bg-gray-950/90 text-gray-900 dark:text-gray-100 ' +
  'placeholder:text-gray-400 dark:placeholder:text-gray-500 transition-all duration-200 ease-smooth outline-none ' +
  ...
  const renderLoginForm = () => (
    <form onSubmit={handleSubmit(onSubmit)} className="space-y-4" noValidate>
      <div className="space-y-1.5">
        ...
        {fieldError(errors.identifier?.message)}
      </div>
      ...
      {requires2FASetup ? (
        ...
        >
          <div className="text-center">
            ...
            <p className="text-sm text-gray-500 dark:text-gray-400 mt-1">
              Ce compte nécessite la double authentification (2FA).
            </p>
          </div>
          <button
            type="button"
            ...
            className="w-full h-11 text-sm font-semibold rounded-xl bg-gradient-to-r from-brand to-emerald-400 text-white shadow-lg shadow-brand/20 hover:shadow-xl transition-all"
          >
        ...
        >
          ...
          <div className="flex justify-center py-2">
            <OtpInput length={6} onComplete={setTotpCode} disabled={busy} />
          </div>
          <button
            type="button"
            disabled={busy || totpCode.length !== 6}
            onClick={handleVerify2FA}
            className="w-full h-11 text-sm font-semibold rounded-xl bg-gradient-to-r from-brand to-emerald-400 text-white shadow-lg shadow-brand/20 hover:shadow-xl hover:shadow-brand/30 transition-all disabled:opacity-50 disabled:cursor-not-allowed flex items-center justify-center gap-2"
          >
            {busy ? (
              <span className="flex items-center gap-2">
                <Loader2 className="animate-spin h-4 w-4" />
          ...
          <div className="p-8 sm:p-10">
            ...
            >
              ...
              <p className="text-sm text-gray-500 dark:text-gray-400 mt-1.5">
                Accédez à votre tableau de bord AfriBiz
              </p>
            </motion.div>

            {requires2FA || requires2FASetup ? render2FA() : renderLoginForm()}
          </div>
        </div>
      </div>
    </motion.div>
