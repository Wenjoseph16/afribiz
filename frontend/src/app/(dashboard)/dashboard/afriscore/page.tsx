...
const COMPONENT_META: Record<string, { label: string; icon: any; desc: string }> = {
  commercialActivity: {
    ...
    icon: ShoppingBag,
    desc: 'Volume de transactions et commandes',
  },
  financialBehavior: {
    label: 'Comportement financier',
    icon: Shield,
    desc: 'Régularité des paiements et solvabilité',
  },
  satisfaction: {
  ...
  operationalReliability: {
    ...
    icon: Activity,
    desc: 'Taux de complétion et respect des délais',
  },
  profileCompleteness: {
    label: 'Complétude du profil',
    icon: UserCheck,
    desc: 'Informations renseignées et documents',
  },
};
...
export default function AfriScorePage() {
    ...
    return (
      <div className="space-y-6 animate-fade-in">
        <PageHeader
          ...
          description={
            business
              ? 'Votre score de confiance et de performance'
              : 'Le score de confiance des businesses'
          }
          breadcrumbs={[{ label: 'AfriScore' }]}
        />
        {!business ? (
          <EmptyState
  ...
  return (
    <div className="space-y-6 animate-fade-in">
      <PageHeader
        title="AfriScore"
        description={business?.name || 'Votre score de confiance et de performance'}
        breadcrumbs={[{ label: 'AfriScore' }]}
        actions={
          <Button variant="secondary" size="sm" onClick={handleRecompute} isLoading={recomputing}>
      ...
      <Card className="relative overflow-hidden">
        ...
        <div className="relative flex flex-col lg:flex-row items-center gap-8 lg:gap-12">
          ...
          <div className="flex flex-col items-center shrink-0">
            <div
              className={`relative w-36 h-36 rounded-full ${level.bg} ${level.ring} ring-4 flex items-center justify-center`}
            >
            ...
            <div className={`mt-3 flex items-center gap-1.5 px-3 py-1 rounded-full ${level.bg}`}>
              {score >= 500 ? (
                <TrendingUp className={`h-3.5 w-3.5 ${level.color}`} />
              ) : (
                <TrendingDown className={`h-3.5 w-3.5 ${level.color}`} />
              )}
        ...
        {Object.entries(COMPONENT_META).map(([key, meta]) => {
          ...
          return (
            <StatsCard
              ...
              trend={
                val >= 100
                  ? { value: 'Bon', positive: true }
                  : val >= 50
                    ? { value: 'Moyen', positive: true }
                    ...
                    .map(([ck]) => {
                      ...
                      return (
                        <div
                          ...
                          style={{
                            backgroundColor:
                              cv >= 100 ? '#10b981' : cv >= 50 ? '#f59e0b' : '#ef4444',
                            opacity: 0.7,
                          }}
                        />
                      );
