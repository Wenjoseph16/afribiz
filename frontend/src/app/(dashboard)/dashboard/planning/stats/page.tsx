...
export default function PlanningStatsPage() {
  ...
  const tasks = useMemo(() => {
    const raw = Array.isArray(tasksData)
      ? tasksData
      : ((tasksData?.tasks || tasksData?.data || []) as any[]);
    return raw;
  }, [tasksData]);

  const schedules = useMemo(() => {
    const raw = Array.isArray(schedulesData)
      ? schedulesData
      : ((schedulesData?.schedules || schedulesData?.data || []) as any[]);
    return raw;
  }, [schedulesData]);
      ...
      <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
        <Card className="p-3 sm:p-4">
          <div className="flex items-center gap-3">
            ...
            <div>
              <p className="text-[10px] text-gray-500 uppercase font-semibold">Total tâches</p>
        ...
        <Card className="p-3 sm:p-4">
          <div className="flex items-center gap-3">
            ...
            <div>
              <p className="text-[10px] text-gray-500 uppercase font-semibold">Terminées</p>
              <p className="text-lg font-bold text-gray-900 dark:text-white">
        ...
        <Card className="p-3 sm:p-4">
          <div className="flex items-center gap-3">
            ...
            <div>
              <p className="text-[10px] text-gray-500 uppercase font-semibold">En retard</p>
              <p className="text-lg font-bold text-gray-900 dark:text-white">
                {overdueTasks.length}
              </p>
            </div>
        ...
        <Card className="p-3 sm:p-4">
          <div className="flex items-center gap-3">
            ...
            <div>
              <p className="text-[10px] text-gray-500 uppercase font-semibold">Employés</p>
              <p className="text-lg font-bold text-gray-900 dark:text-white">{uniqueEmployees}</p>
            </div>
          </div>
        </Card>
      </div>
              ...
              <div key={s.label}>
                <div className="flex justify-between text-xs mb-1">
                  ...
                  <span className="font-medium text-gray-900 dark:text-white">
                    {s.count} ({tasks.length > 0 ? Math.round((s.count / tasks.length) * 100) : 0}%)
                  </span>
                </div>
        ...
        <div className="grid grid-cols-2 sm:grid-cols-4 gap-3 text-center">
          ...
          <div className="p-3 rounded-xl bg-gray-50 dark:bg-gray-800/50">
            <p className="text-2xl font-bold text-gray-900 dark:text-white">
              {new Set(schedules.map((s: any) => s.dayOfWeek)).size}
            </p>
            <p className="text-[10px] text-gray-500">Jours couverts /7</p>
          </div>
          <div className="p-3 rounded-xl bg-gray-50 dark:bg-gray-800/50">
            <p className="text-2xl font-bold text-gray-900 dark:text-white">
              {schedules.filter((s: any) => s.isActive !== false).length}
            </p>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
          {overdueRate > 20 && (
            <div className="p-3 rounded-xl bg-white/60 dark:bg-gray-800/60">
              <p className="text-xs font-medium text-blue-800 dark:text-blue-300">
                ⚠️ Trop de tâches en retard
              </p>
              <p className="text-[10px] text-blue-600 dark:text-blue-400 mt-1">
                {overdueTasks.length} tâches sont en retard. Envisagez de redistribuer les
                priorités.
              </p>
            </div>
          )}
          {uniqueEmployees > 0 && tasks.length > 0 && (
            <div className="p-3 rounded-xl bg-white/60 dark:bg-gray-800/60">
              ...
              <p className="text-[10px] text-blue-600 dark:text-blue-400 mt-1">
                {Math.round(tasks.length / uniqueEmployees)} tâches par employé en moyenne.
              </p>
            </div>
          )}
          <div className="p-3 rounded-xl bg-white/60 dark:bg-gray-800/60">
            ...
            <p className="text-[10px] text-blue-600 dark:text-blue-400 mt-1">
              {schedules.length === 0
                ? 'Créez des horaires pour mieux organiser vos équipes.'
                : `${DAYS_LABEL[schedules[0]?.dayOfWeek - 1] || ''} : ${schedules.filter((s: any) => s.isActive !== false).length} horaires actifs.`}
            </p>
          </div>
        </div>
      </Card>
