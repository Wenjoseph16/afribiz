    ...
    <div className="animate-fade-in space-y-6">
        ...
        <Card className="p-4">
          <div className="grid grid-cols-7 gap-px bg-gray-200 dark:bg-gray-700 rounded-xl overflow-hidden">
            {DAYS.map((d) => (
              <div
                key={d}
                className="bg-white dark:bg-gray-800 p-2 text-center text-xs font-semibold text-gray-500 uppercase"
              >
                {d}
              </div>
            ))}
            {calendarDays.map((day, i) => {
              ...
              return (
                <div
                  key={i}
                  className={cn(
                    'bg-white dark:bg-gray-800 p-1.5 min-h-[80px] border-t border-gray-100 dark:border-gray-700/50',
                ...
                >
                  <p
                    className={cn(
                      ...
                      isToday
                        ? 'bg-brand text-white w-5 h-5 rounded-full flex items-center justify-center'
                        : 'text-gray-500'
                    )}
                  >
                    {day.date.getDate()}
                  ...
                  <div className="space-y-0.5">
                    {day.tasks.slice(0, 3).map((t: any) => (
                      <Link
                        key={t.id}
                        href={`/dashboard/planning/${t.id}`}
                        className={cn(
                          'block text-[9px] px-1 py-0.5 rounded text-white truncate',
                          priorityColors[t.priority] || 'bg-gray-400'
                        )}
                      >
                        {t.title}
                      </Link>
                      ...
                      return (
                        ...
                        >
                          ...
                          <div className="flex items-center gap-2">
                            ...
                            {isOverdue && (
                              <span className="text-[9px] text-red-500 font-medium">Retard</span>
                            )}
                            {t.status === 'COMPLETED' && (
      ...
      <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
        <Card className="p-3">
          <div className="flex items-center gap-2">
            ...
            <div>
              <p className="text-[10px] text-gray-500">Tâches ce mois</p>
              <p className="text-sm font-bold">
                {calendarDays.reduce((s, d) => s + d.tasks.length, 0)}
              </p>
            </div>
          </div>
        </Card>
      ...
      <div className="flex items-center gap-4 text-xs text-gray-500">
        <span className="flex items-center gap-1">
          <div className="w-2.5 h-2.5 rounded bg-red-500" /> Urgente
        </span>
        <span className="flex items-center gap-1">
          <div className="w-2.5 h-2.5 rounded bg-amber-500" /> Haute
        </span>
        <span className="flex items-center gap-1">
          <div className="w-2.5 h-2.5 rounded bg-blue-500" /> Moyenne
        </span>
        <span className="flex items-center gap-1">
          <div className="w-2.5 h-2.5 rounded bg-gray-400" /> Basse
        </span>
      </div>
    </div>
  );
