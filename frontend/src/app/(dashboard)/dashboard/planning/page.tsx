...
export default function PlanningPage() {
  ...
  const allTasks: PlanningTask[] = Array.isArray(tasksData)
    ? tasksData
          ...
          return (
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
              {suggestions.map((s: any, i) => (
                <Link
                  key={i}
                  href={s.link || '/dashboard/planning'}
...
function TaskCard({ task }: { task: PlanningTask }) {
  ...
  return (
    <Link
      href={`/dashboard/planning/${task.id}`}
    ...
    >
      <div className="p-4 space-y-3">
        <div className="flex items-start justify-between gap-2">
          ...
          <span
            className={cn('text-xs font-medium px-2 py-0.5 rounded-full shrink-0', pConf.color)}
          >
            {pConf.label}
          </span>
        </div>
    ...
    <tr className="hover:bg-gray-50 dark:hover:bg-gray-700/30 transition-colors group">
      ...
      <td className="p-4">
        <span
          className={cn(
            'text-sm',
            isOverdue ? 'text-red-500 font-medium' : 'text-gray-600 dark:text-gray-300'
          )}
        >
      ...
      <td className="p-4 text-center">
        <span className={cn('text-xs font-medium px-2 py-0.5 rounded-full', pConf.color)}>
          {pConf.label}
        </span>
      </td>
      <td className="p-4 text-center">
      ...
      <td className="p-4">
        <div className="flex items-center gap-1 opacity-0 group-hover:opacity-100 transition-opacity">
          ...
          <Link
            href={`/dashboard/planning/${task.id}/edit`}
