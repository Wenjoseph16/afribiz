...
export default function TablesPage() {
  ...
  const tables = Array.isArray(tablesData)
    ? tablesData
    : tablesData?.tables || tablesData?.data || [];
    ...
    <div className="animate-fade-in space-y-6">
      ...
      <div className="grid grid-cols-4 gap-3">
        {[
          ...
          {
            label: 'Libres',
            value: tables.filter((t: any) => t.status === 'FREE').length,
            color: 'text-emerald-600',
          },
          {
            label: 'Occupées',
            value: tables.filter((t: any) => t.status === 'OCCUPIED').length,
            color: 'text-red-600',
          },
          {
            label: 'Capacité',
            value: `${tables.reduce((a: number, t: any) => a + (t.capacity || 0), 0)} pers.`,
          },
        ].map((s) => (
          <Card key={s.label} className="p-3 text-center">
            <p className={cn('text-xl font-bold text-gray-900 dark:text-white', s.color)}>
              {s.value}
            </p>
      ...
      <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 gap-4">
        {filtered.map((table: any) => (
          <div
            key={table.id}
            className={cn(
              'bg-white dark:bg-gray-800 rounded-xl border-2 p-4 hover:shadow-md transition-shadow',
              STATUS_STYLES[table.status]?.split(' ')[2] || 'border-gray-200 dark:border-gray-700'
            )}
          >
              ...
              <div className="relative group">
                <span
                  className={cn(
                    ...
                    STATUS_STYLES[table.status]?.split(' ').slice(0, 2).join(' ') ||
                ...
                <div className="absolute right-0 top-full mt-1 bg-white dark:bg-gray-700 rounded-lg shadow-lg border border-gray-200 dark:border-gray-600 p-1 hidden group-hover:block z-10 min-w-[120px]">
                  {Object.entries(STATUS_LABELS).map(([key, label]) => (
                    <button
                      ...
                      className={cn(
                        'block w-full text-left px-3 py-1.5 text-xs rounded hover:bg-gray-100 dark:hover:bg-gray-600',
                        table.status === key
            ...
            <div className="flex items-center gap-2 text-xs text-gray-500 dark:text-gray-400 mb-3">
              ...
              {table.location && (
                <span className="ml-auto">{LOCATION_LABELS[table.location] || table.location}</span>
              )}
            </div>
      ...
      <Modal
        ...
        title={editTable ? 'Modifier la table' : 'Nouvelle table'}
      >
        <div className="space-y-4">
          <Input
            ...
            placeholder="Ex: 1, A2, Terrasse 3"
          />
          <Input
            label="Capacité"
            type="number"
          ...
          <div>
            ...
            >
              ...
              {LOCATIONS.filter((l) => l !== 'Tous').map((l) => (
      ...
      >
        <p className="text-sm text-gray-600 dark:text-gray-400 mb-4">
          Êtes-vous sûr de vouloir supprimer la table <strong>#{deleteConfirm?.tableNumber}</strong>{' '}
          ?
        </p>
        <div className="flex justify-end gap-3">
          ...
          <Button onClick={handleDelete} isLoading={saving} className="bg-red-500 hover:bg-red-600">
            Supprimer
          </Button>
        </div>
      </Modal>
