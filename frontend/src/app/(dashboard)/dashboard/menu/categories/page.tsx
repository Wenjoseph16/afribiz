    ...
    <>
      ...
      >
        ...
        <span className="text-xs text-gray-400 bg-gray-100 dark:bg-gray-800 px-2 py-0.5 rounded-full">
          {cat._count?.items || cat.itemsCount || 0} plats
        </span>
        <button
          onClick={() => onEdit(cat)}
  ...
  return (
    <div className="animate-fade-in space-y-6">
      ...
      <div className="grid grid-cols-3 gap-3">
        ...
        <Card className="p-3 text-center">
          <p className="text-2xl font-bold text-gray-900">
            {categories.reduce(
              (a: number, c: any) =>
                a +
                (c._count?.items || c.itemsCount || 0) +
                (c.children?.reduce(
                  (s: number, ch: any) => s + (ch._count?.items || ch.itemsCount || 0),
                  0
                ) || 0),
              0
            )}
        ...
        <Card className="p-3 text-center">
          <p className="text-2xl font-bold text-gray-900">
            {categories.reduce((a: number, c: any) => a + (c.children?.length || 0), 0)}
          </p>
      ...
      >
        <div className="space-y-4">
          <Input
            ...
            placeholder="Ex: Plats locaux, Grillades..."
          />
          <div>
          ...
          <div>
            ...
            >
              ...
              {categories
                .filter((c: any) => !editCat || c.id !== editCat.id)
                .map((c: any) => (
      ...
      >
        <p className="text-sm text-gray-600 dark:text-gray-400 mb-4">
          Êtes-vous sûr de vouloir supprimer <strong>{deleteConfirm?.name}</strong> ? Les plats de
          cette catégorie ne seront pas supprimés mais deviendront non catégorisés.
        </p>
