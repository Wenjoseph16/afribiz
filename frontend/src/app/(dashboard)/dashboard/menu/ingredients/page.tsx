...
export default function IngredientsPage() {
  ...
  const allIngredients = Array.isArray(ingredientsData)
    ? ingredientsData
    : ingredientsData?.ingredients || ingredientsData?.data || [];
  ...
  const lowStockCount = allIngredients.filter(
    (i: any) => Number(i.stock || 0) < Number(i.minStock || 0)
  ).length;
    ...
    <div className="animate-fade-in space-y-6">
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-4">
          <Link href="/dashboard/menu" className="p-2 rounded-lg hover:bg-gray-100">
            <ArrowLeft className="w-5 h-5 text-gray-500" />
          </Link>
          <div>
            <h1 className="text-2xl font-bold text-gray-900 dark:text-white">Stock cuisine</h1>
            <p className="text-sm text-gray-500">Gérez vos ingrédients</p>
          </div>
        </div>
          ...
          },
          {
            ...
            value: formatPrice(
              allIngredients.reduce(
                (a: number, i: any) => a + Number(i.stock || 0) * Number(i.unitPrice || 0),
                0
              )
            ),
            icon: Package,
      ...
      <div className="bg-white dark:bg-gray-800 rounded-xl border border-gray-200 dark:border-gray-700 overflow-hidden">
        <table className="w-full text-sm">
          <thead className="bg-gray-50 dark:bg-gray-800/50 border-b border-gray-200 dark:border-gray-700">
            <tr>
              <th className="text-left px-4 py-3 font-medium text-gray-500 dark:text-gray-400 text-xs uppercase">
                Ingrédient
              </th>
              <th className="text-left px-4 py-3 font-medium text-gray-500 dark:text-gray-400 text-xs uppercase">
                Catégorie
              </th>
              <th className="text-right px-4 py-3 font-medium text-gray-500 dark:text-gray-400 text-xs uppercase">
                Stock
              </th>
              <th className="text-right px-4 py-3 font-medium text-gray-500 dark:text-gray-400 text-xs uppercase">
                Seuil
            ...
            {filtered.map((ing: any) => {
              ...
              return (
                ...
                >
                  ...
                  <td className="px-4 py-3 text-right text-gray-500 dark:text-gray-400">
                    {formatPrice(Number(ing.unitPrice || 0))}
                  </td>
                  <td className="px-4 py-3 text-right">
      ...
      <Modal
        ...
        title={`Ajuster le stock - ${stockIng?.name || ''}`}
      >
        <div className="space-y-4">
          <div className="p-3 bg-gray-50 dark:bg-gray-800/50 rounded-lg text-sm">
            ...
            <span className="font-bold text-gray-900 dark:text-white">
              {Number(stockIng?.stock || 0)} {stockIng?.unit || ''}
            </span>
          </div>
          <div>
          ...
          <div className="flex justify-end gap-3 pt-2">
            ...
            <Button onClick={handleStockAdjust} isLoading={saving}>
              <TrendingUpDown className="w-4 h-4 mr-1.5" />
              Valider
            </Button>
          </div>
        </div>
