...
function getBadges(item: MenuItem): { label: string; className: string }[] {
  ...
  if (item.isPopular)
    b.push({ label: '🔥 Populaire', className: 'bg-emerald-100 text-emerald-700' });
...
export default function MenuPage() {
  ...
  const items: MenuItem[] = Array.isArray(itemsData)
    ? itemsData
  ...
  const handleToggleActive = async (id: string) => {
    try {
      ...
      notifyError(err, 'Erreur', 'Impossible de modifier le statut');
    }
  };
  ...
  const chartData: ModuleChartData = useMemo(() => {
    const statusDist = [
      {
        name: 'Disponibles',
        value: items.filter((i) => i.status === 'AVAILABLE' || i.isActive !== false).length,
        color: '#10b981',
      },
      {
        name: 'Indisponibles',
      ...
      { name: 'Promotions', value: items.filter((i) => i.isPromotional).length, color: '#f59e0b' },
    ].filter((d) => d.value > 0);
    return {
      trend: [
        { label: 'Plats', value: items.length },
        { label: 'Populaires', value: items.filter((i) => i.isPopular).length },
        { label: 'Vedettes', value: items.filter((i) => i.isStar).length },
      ],
      distribution:
        statusDist.length > 1
          ? statusDist
          : [{ name: 'Plats', value: items.length, color: '#f59e0b' }],
      daily: [
        {
          label: 'Vendus',
          value: items.reduce((a, i) => a + (i.soldCount || i.orderCount || 0), 0),
        },
        {
          label: 'Note moy.',
          value: Math.round(
            (items.reduce((a, i) => a + (i.rating || 0), 0) / (items.length || 1)) * 10
          ),
        },
      ],
    };
    ...
    <div className="space-y-6 animate-fade-in">
      ...
      <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
        ...
        <StatsCard
          ...
          label="Note"
          value={typeof stats.avgRating === 'number' ? stats.avgRating.toFixed(1) : '0.0'}
        />
      </div>
      ...
      <ModuleCharts
        ...
        trendLabel="Répartition"
        distributionLabel="Statut"
        dailyLabel="Performance"
        variant="menu"
      />
        ...
        <div className="flex items-center gap-3">
          ...
          <div className="flex items-center gap-1">
            <button
              ...
              className={cn(
                'p-2 rounded-lg',
                viewMode === 'grid' ? 'bg-brand/10 text-brand' : 'text-gray-400 hover:bg-gray-100'
              )}
              title="Grille"
            >
            ...
            <button
              ...
              className={cn(
                'p-2 rounded-lg',
                viewMode === 'list' ? 'bg-brand/10 text-brand' : 'text-gray-400 hover:bg-gray-100'
              )}
              title="Liste"
            >
      ...
      {selectedIds.length > 0 && (
        <div className="flex items-center gap-2 px-4 py-2 bg-brand/5 rounded-xl border border-brand/20">
          ...
          <div className="flex gap-2 ml-auto">
            <Button size="sm" variant="secondary" onClick={() => handleBulkToggle(true)}>
      ...
      {filtered.length === 0 ? (
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4">
          {filtered.map((item) => (
            <MenuItemCard key={item.id} item={item} onToggle={handleToggleActive} />
          ))}
        </div>
      ) : (
    ...
    <div className="group bg-white dark:bg-gray-800 rounded-2xl border border-gray-200 dark:border-gray-700 overflow-hidden hover:border-brand/30 transition-all">
      <Link href={`/dashboard/menu/${item.id}`} className="block p-4">
        ...
        <p className="text-xs text-gray-500 mt-0.5">
          {item.shortDescription?.slice(0, 60) || item.description?.slice(0, 60)}
        </p>
        <div className="flex items-center justify-between mt-3 pt-3 border-t border-gray-100 dark:border-gray-700">
          <p className="text-sm font-bold">{item.price.toLocaleString()} FCFA</p>
          <p className="text-xs text-gray-500">
            <Clock className="h-3 w-3 inline" /> {(item.prepTime || 0) + (item.cookTime || 0)}min
          </p>
        </div>
      </Link>
      <div className="px-4 pb-3 flex items-center gap-1">
        ...
        <Link
          ...
          className="p-1.5 rounded-lg hover:bg-gray-100 dark:hover:bg-gray-700 text-gray-400 hover:text-blue-500 ml-auto"
        >
...
function MenuRow({
  ...
  return (
    <tr
      className={cn('hover:bg-gray-50 dark:hover:bg-gray-700/30 group', selected && 'bg-brand/5')}
    >
      ...
      <td className="p-4 text-right text-sm text-gray-600 dark:text-gray-400">
        {item.orderCount || item.soldCount || 0}
      </td>
      ...
      <td className="p-4 text-right">
        <span
          className={cn(
            ...
            item.isActive !== false ? 'text-emerald-600' : 'text-gray-400'
          )}
        >
          <span
            className={cn(
              'w-2 h-2 rounded-full',
              item.isActive !== false ? 'bg-emerald-500' : 'bg-gray-300'
            )}
          />
          {item.isActive !== false ? 'Actif' : 'Inactif'}
        </span>
      ...
      <td className="p-4">
        <div className="flex items-center gap-1 opacity-0 group-hover:opacity-100">
          <button
            ...
            className="p-1.5 rounded-lg hover:bg-gray-100 dark:hover:bg-gray-700 text-gray-400 hover:text-emerald-500"
