...
const STATUS_CONFIG: Record<
  string,
  {
    ...
    color: string;
    icon: any;
    nextActions?: { status: string; label: string; color: string }[];
  }
> = {
  PENDING: {
    label: 'En attente',
    ...
    <div className="animate-fade-in space-y-6">
      ...
      <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
        {[
          {
            label: 'En attente',
            value: stats.pending || allOrders.filter((o: any) => o.status === 'PENDING').length,
            color: 'text-amber-600 bg-amber-100',
          },
          {
            label: 'En préparation',
            value: stats.preparing || allOrders.filter((o: any) => o.status === 'PREPARING').length,
            color: 'text-purple-600 bg-purple-100',
          },
          {
            label: 'Prêtes',
            value: stats.ready || allOrders.filter((o: any) => o.status === 'READY').length,
                ...
                <div className="flex-1">
                  <div className="flex items-center gap-2 flex-wrap">
                    <span className="font-semibold text-gray-900 dark:text-white">
                      {order.orderNumber || 'CMD-' + order.id.slice(0, 8)}
                    </span>
                    <span
                      className={cn(
