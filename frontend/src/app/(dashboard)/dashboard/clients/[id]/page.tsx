    ...
    <div className="space-y-6 animate-fade-in">
      ...
      <div className="flex items-center gap-4">
        ...
        <div>
          <h1 className="text-2xl font-bold text-gray-900 dark:text-gray-100">
            {bc.client?.fullName || `${bc.firstName || ''} ${bc.lastName || ''}`.trim()}
          </h1>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div className="space-y-4">
          ...
          <Card>
            <div className="text-center mb-4">
              ...
              <h2 className="text-xl font-bold text-gray-900 dark:text-gray-100">
                {bc.client?.fullName ||
                  `${bc.firstName || bc.client?.firstName} ${bc.lastName || bc.client?.lastName}`}
              </h2>
            ...
            <div className="space-y-3">
              ...
              {(bc.city || bc.client?.city) && (
                <div className="flex items-center gap-3 text-sm text-gray-600 dark:text-gray-400">
                  ...
                  <span>
                    {bc.city || bc.client?.city}
                    {bc.client?.neighborhood ? `, ${bc.client.neighborhood}` : ''}
                  </span>
                </div>
              )}
              <div className="flex items-center gap-3 text-sm text-gray-600 dark:text-gray-400">
          ...
          <Card title="Tags">
              ...
              {bc.tags?.length > 0 ? (
                bc.tags.map((t: any) => (
                  ...
                  >
                    {t.name}
                    <button
                      onClick={() => removeTag.mutate({ clientId, tagId: t.id })}
            ...
            <div className="flex gap-2">
              <select
                onChange={(e) => {
                  if (e.target.value) {
                    assignTag.mutate({ clientId, tagId: e.target.value });
                    e.target.value = '';
                  }
                }}
                value=""
              ...
              >
                ...
                {tags
                  ?.filter((t: any) => !bc.tags?.some((ct: any) => ct.id === t.id))
                  .map((t: any) => (
                    <option key={t.id} value={t.id}>
                      {t.name} ({t._count?.clients || 0})
                    </option>
                  ))}
              </select>
              <button
          ...
          <Card title="Segments">
            <div className="flex flex-wrap gap-1.5 mb-3">
              {bc.segments?.length > 0 ? (
                bc.segments.map((s: any) => (
                  <span
                    key={s.id}
                    className="inline-flex items-center gap-1 px-2.5 py-1 rounded-full text-xs font-medium"
                  ...
                  >
                    {s.name}
                    <button
                      onClick={() => removeSegment.mutate({ clientId, segmentId: s.id })}
                      className="hover:opacity-70"
                    >
                      <X className="h-3 w-3" />
                    </button>
            ...
            <select
              onChange={(e) => {
                if (e.target.value) {
                  assignSegment.mutate({ clientId, segmentId: e.target.value });
                  e.target.value = '';
                }
              }}
              value=""
            ...
            >
              ...
              {segments
                ?.filter((s: any) => !bc.segments?.some((cs: any) => cs.id === s.id))
                .map((s: any) => (
              ...
              <div className="space-y-2">
                <div className="flex justify-between items-center">
                  ...
                  <span
                    className={cn(
                      ...
                      bc.clientRisk.riskLevel === 'LOW' &&
                        'text-green-600 bg-green-50 dark:bg-green-900/20',
                      bc.clientRisk.riskLevel === 'MEDIUM' &&
                        'text-amber-600 bg-amber-50 dark:bg-amber-900/20',
                      bc.clientRisk.riskLevel === 'HIGH' &&
                        'text-orange-600 bg-orange-50 dark:bg-orange-900/20',
                      bc.clientRisk.riskLevel === 'CRITICAL' &&
                ...
                <div className="flex justify-between items-center">
                  ...
                  <span className="text-sm font-semibold">
                    {bc.clientRisk.reliabilityScore}/100
                  </span>
                </div>
                <div className="flex justify-between items-center">
          ...
          <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
            ...
            <div className="p-4 rounded-xl bg-white dark:bg-gray-800/50 border border-gray-100 dark:border-gray-700 text-center">
              <p className="text-2xl font-bold text-brand">
                {Number(bc.totalSpent).toLocaleString()}
              </p>
              ...
              <div className="space-y-2">
                {bc.bookings.slice(0, 5).map((booking: any) => (
                  ...
                  >
                    ...
                    <div className="text-right">
                      <p className="text-sm font-semibold text-gray-900 dark:text-gray-100">
                        {Number(booking.price).toLocaleString()} FCFA
                      </p>
                      <span
                        className={cn(
                          'inline-block px-2 py-0.5 rounded-full text-[10px] font-medium mt-0.5',
              ...
              <div className="space-y-2">
                {bc.debts.map((debt: any) => (
                  ...
                  >
                    <div>
                      <p className="text-sm font-medium text-gray-900 dark:text-gray-100">
                        {Number(debt.totalAmount).toLocaleString()} FCFA
                      </p>
                      <p className="text-xs text-gray-500">
                        Restant: {Number(debt.remainingAmount).toLocaleString()} FCFA
                      </p>
                    </div>
                    <span
                      className={cn(
                ...
                bc.notes.map((note: any) => (
                  <div key={note.id} className="p-3 rounded-lg bg-gray-50 dark:bg-gray-800/50">
                    ...
                    <div className="flex items-center gap-2 mt-2">
                      ...
                      {note.createdBy && (
                        <span className="text-[10px] text-gray-400">• par {note.createdBy}</span>
                      )}
                    </div>
                  </div>
                ))
