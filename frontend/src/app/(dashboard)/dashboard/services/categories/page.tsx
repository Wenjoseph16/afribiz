...
export default function ServiceCategoriesPage() {
  ...
  const stats = useMemo(
    () => ({
      total: cats.reduce((a, c) => a + 1 + (c.children?.length || 0), 0),
      totalServices: cats.reduce(
        (a, c) => a + c.serviceCount + (c.children?.reduce((s, ch) => s + ch.serviceCount, 0) || 0),
        0
      ),
    }),
    [cats]
  ...
  const filtered = useMemo(() => {
    ...
    return cats
      ...
      .map((c) => ({
        ...c,
        children: c.children?.filter((ch) => ch.name.toLowerCase().includes(q)),
      }));
  }, [cats, search]);
    ...
    try {
      if (editing) {
        await updateCat.mutateAsync({
          id: editing.id,
          data: { name: name.trim(), description: desc, icon },
        });
      } else {
  ...
  return (
    <div className="space-y-6 animate-fade-in">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
        ...
        <Card padding="sm" className="text-center">
          <p className="text-2xl font-bold text-brand">{stats.totalServices}</p>
          <p className="text-xs text-gray-500 mt-0.5">Services dans ces catégories</p>
        </Card>
      </div>
            ...
            <div className="space-y-4">
              {!editing && parentId && (
                <div className="p-3 bg-brand/5 rounded-xl border border-brand/10">
                  <p className="text-xs text-brand font-medium">Sous-catégorie</p>
                </div>
              )}
              <Input
                label="Nom *"
...
function CatRow({
  ...
  return (
    <div>
      <div
        className="flex items-center gap-3 px-4 sm:px-6 py-3 hover:bg-gray-50 dark:hover:bg-gray-700/30 transition-colors group"
      ...
      >
        ...
        <div className="w-8 h-8 rounded-xl bg-brand/5 flex items-center justify-center text-sm shrink-0">
          {cat.icon || <FolderTree className="h-4 w-4 text-brand" />}
        </div>
        ...
        <span className="text-xs font-medium text-gray-500 bg-gray-100 dark:bg-gray-700 px-2 py-0.5 rounded-full shrink-0">
          {cat.serviceCount} serv.
        </span>
        <div className="flex items-center gap-0.5 opacity-0 group-hover:opacity-100 transition-opacity shrink-0">
          ...
          <button
            ...
            disabled={cat.serviceCount > 0 || hasChildren}
            className={cn(
              'p-1 rounded-lg',
              cat.serviceCount > 0 || hasChildren
