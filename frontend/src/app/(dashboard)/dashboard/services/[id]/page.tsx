...
export default function ServiceDetailPage() {
  ...
  const isNewService =
    s?.createdAt && (Date.now() - new Date(s.createdAt).getTime()) / (1000 * 60 * 60 * 24) <= 30;
      ...
      <div className="grid grid-cols-2 sm:grid-cols-4 gap-4">
        <Card padding="sm" className="text-center">
          <p className="text-2xl font-bold text-gray-900 dark:text-gray-100">
            {s.priceType === 'VARIABLE'
              ? `${fmtPrice(s.minPrice || s.price || 0)}`
              : fmtPrice(s.price || 0)}
          </p>
          <p className="text-xs text-gray-500 mt-0.5">
            {s.isPromotional && s.promotionalPrice ? (
              <>
                <span className="line-through text-gray-400">{fmtPrice(s.price || 0)}</span> →{' '}
                <span className="text-red-500 font-bold">{fmtPrice(s.promotionalPrice)}</span>
              </>
            ) : (
              'Prix'
            )}
        ...
        <Card padding="sm" className="text-center">
          ...
          <p className="text-xs text-gray-500 mt-0.5">
            ...
            {s.durationMin && s.durationMax
              ? ` (${fmtDur(s.durationMin)} - ${fmtDur(s.durationMax)})`
              : ''}
          </p>
        </Card>
        <Card padding="sm" className="text-center">
        ...
        <div className="flex border-b border-gray-200 dark:border-gray-700 overflow-x-auto">
          {tabs.map((tab) => (
            <button
              ...
              className={cn(
                'flex items-center gap-1.5 px-4 sm:px-6 py-3 text-sm font-medium whitespace-nowrap border-b-2 transition-colors',
                activeTab === tab.id
            ...
            <div className="space-y-6">
              ...
              <div className="grid grid-cols-2 sm:grid-cols-4 gap-4">
                ...
                <div className="p-3 bg-gray-50 dark:bg-gray-800/50 rounded-xl">
                  ...
                  <p className="text-sm font-semibold mt-0.5">
                    {s.bookingRequired ? 'Obligatoire' : 'Facultative'}
                  </p>
                </div>
                <div className="p-3 bg-gray-50 dark:bg-gray-800/50 rounded-xl">
                  <p className="text-xs text-gray-500">Acompte</p>
                  <p className="text-sm font-semibold mt-0.5">
                    {s.depositRequired ? `${s.depositAmount?.toLocaleString()} FCFA` : 'Non requis'}
                  </p>
                </div>
              </div>
                ...
                <div className="p-4 bg-blue-50 dark:bg-blue-900/10 rounded-xl border border-blue-100 dark:border-blue-900/20">
                  ...
                  {s.seoTitle && (
                    <p className="text-sm text-gray-700 dark:text-gray-300">
                      <span className="text-gray-400">Titre:</span> {s.seoTitle}
                    </p>
                  )}
                  {s.seoDescription && (
                    <p className="text-sm text-gray-700 dark:text-gray-300 mt-1">
                      <span className="text-gray-400">Description:</span> {s.seoDescription}
                    </p>
                  )}
                </div>
              )}
          ...
          {activeTab === 'bookings' && (
            <div className="text-center py-8">
              ...
              <p className="text-xs text-gray-400 mt-1">
                {s.bookingCount || 0} réservation(s) au total
              </p>
            </div>
          )}
            ...
            <div className="space-y-3">
              <div className="flex items-start gap-3 p-3 bg-gray-50 dark:bg-gray-700/30 rounded-xl">
                ...
                <div className="flex-1 min-w-0">
                  ...
                  <p className="text-xs text-gray-500">
                    Le {new Date(s.createdAt).toLocaleString('fr-FR')}
                  </p>
                </div>
              </div>
              {s.updatedAt && s.updatedAt !== s.createdAt && (
                <div className="flex items-start gap-3 p-3 bg-gray-50 dark:bg-gray-700/30 rounded-xl">
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    <p className="text-xs text-gray-500">
                      Le {new Date(s.updatedAt).toLocaleString('fr-FR')}
                    </p>
                  </div>
                </div>
              )}
              <div className="flex items-start gap-3 p-3 bg-gray-50 dark:bg-gray-700/30 rounded-xl">
                ...
                <div className="flex-1 min-w-0">
                  ...
                  <p className="text-xs text-gray-500">
                    {s.bookingCount || 0} réservation(s) · {s.reviewCount || 0} avis · Note:{' '}
                    {s.rating || '-'}/5
                  </p>
                </div>
              </div>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center">
          ...
          <div className="relative bg-white dark:bg-gray-800 rounded-2xl shadow-xl border w-full max-w-md mx-4 p-6">
            <div className="flex items-center gap-3 mb-4">
              ...
              <div>
                <h2 className="text-lg font-bold text-gray-900">Supprimer ce service ?</h2>
                <p className="text-sm text-gray-500">Cette action est irréversible</p>
              </div>
            </div>
            <p className="text-sm text-gray-600 mb-6">
              Le service <strong>{s.name}</strong> sera supprimé définitivement.
            </p>
