...
export default function EditServicePage() {
  ...
  const removeEmployee = (i: number) =>
    update(
      'employees',
      form.employees.filter((_: any, idx: number) => idx !== i)
    );
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const payload: any = {
        ...
        promotionalPrice:
          form.isPromotional && form.promotionalPrice ? Number(form.promotionalPrice) : null,
        ...
        employees: form.employees
          ...
          .map((e: any) => ({ name: e.name.trim(), title: e.title.trim() })),
      };
      if (
        payload.isPromotional &&
        payload.price &&
      ...
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-4">
          ...
          <div>
            ...
            <p className="text-sm text-gray-500 dark:text-gray-400 mt-0.5">{s?.name}</p>
          </div>
        </div>
      ...
      <form onSubmit={handleSubmit} className="space-y-6">
            ...
            <div>
              ...
              <div className="flex gap-2">
                ...
                <button
                  type="button"
                  onClick={addTag}
                  className="px-3 py-2 rounded-xl bg-brand/10 text-brand text-sm font-medium hover:bg-brand/20"
                >
                  +
                </button>
              </div>
              {form.tags.length > 0 && (
                <div className="flex flex-wrap gap-1.5 mt-2">
                  {form.tags.map((t: string) => (
                    <span
                      key={t}
                      className="inline-flex items-center gap-1 px-2 py-1 text-xs font-medium bg-gray-100 dark:bg-gray-700 text-gray-600 dark:text-gray-300 rounded-full"
                    >
                      #{t}
                      <button
                        type="button"
          ...
          <div className="mt-4">
            <button
              ...
              onClick={() => {
                ...
                update('isPromotional', !showPromo);
              }}
              className={cn('text-sm font-medium', showPromo ? 'text-red-500' : 'text-brand')}
            >
              {showPromo ? '— Masquer' : '+ Promo'}
            </button>
            {showPromo && (
              <div className="mt-3 grid grid-cols-1 sm:grid-cols-3 gap-4 p-4 bg-red-50 dark:bg-red-900/10 rounded-xl border border-red-100">
                <Input
                  label="Prix promo"
                  type="number"
                  value={form.promotionalPrice}
                  onChange={(e) => update('promotionalPrice', e.target.value)}
                />
                <Input
                  label="Réduction %"
                  type="number"
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            <div>
              ...
              >
                ...
                <option value="APPOINTMENT_ONLY">Sur rendez-vous</option>
              </select>
            </div>
            <div>
          ...
          <div className="space-y-3">
            <label className="flex items-start gap-3 p-3 rounded-xl border border-gray-200 dark:border-gray-700 cursor-pointer">
              <input
                ...
                onChange={(e) => update('bookingRequired', e.target.checked)}
                className="mt-0.5 rounded border-gray-300 text-brand focus:ring-brand"
              />
              <div>
                <p className="text-sm font-medium">Réservation obligatoire</p>
              </div>
            </label>
            <label className="flex items-start gap-3 p-3 rounded-xl border border-gray-200 dark:border-gray-700 cursor-pointer">
              <input
            ...
            <label className="flex items-start gap-3 p-3 rounded-xl border border-gray-200 dark:border-gray-700 cursor-pointer">
              ...
              />
              <div>
                <p className="text-sm font-medium">Acompte obligatoire</p>
              </div>
            </label>
        ...
        <Card>
          ...
          {form.employees.map((emp: any, i: number) => (
            <div
              key={i}
              className="flex items-center gap-2 p-3 bg-gray-50 dark:bg-gray-800/50 rounded-xl mb-2"
            >
              <div className="flex-1 grid grid-cols-1 sm:grid-cols-2 gap-2">
                <input
                  value={emp.name}
                  onChange={(e) => updateEmployee(i, 'name', e.target.value)}
                  className="px-3 py-1.5 text-sm border border-gray-200 rounded-lg focus:ring-2 focus:ring-brand/20 focus:border-brand outline-none bg-white dark:bg-gray-700 dark:text-gray-100"
                  placeholder="Nom"
                />
                <input
                  value={emp.title}
        ...
        <Card>
          ...
          <div className="space-y-3">
            <label className="flex items-center gap-3 p-3 rounded-xl border border-gray-200 dark:border-gray-700 cursor-pointer">
              <input
                type="checkbox"
                checked={form.isVisibleOnPublicPage}
                onChange={(e) => update('isVisibleOnPublicPage', e.target.checked)}
                className="rounded border-gray-300 text-brand focus:ring-brand"
              />
              <span className="text-sm font-medium">Afficher page publique</span>
            </label>
            <label className="flex items-center gap-3 p-3 rounded-xl border border-gray-200 dark:border-gray-700 cursor-pointer">
              <input
                type="checkbox"
                checked={form.isVisibleOnMarketplace}
                onChange={(e) => update('isVisibleOnMarketplace', e.target.checked)}
                className="rounded border-gray-300 text-brand focus:ring-brand"
              />
              <span className="text-sm font-medium">Afficher marketplace</span>
            </label>
          </div>
        </Card>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            ...
            <div className="sm:col-span-2">
              ...
              <textarea
                ...
                className="w-full px-4 py-2 text-sm border border-gray-300 rounded-xl focus:ring-2 focus:ring-brand/20 focus:border-brand outline-none resize-none bg-transparent dark:text-gray-100"
                rows={2}
                maxLength={160}
              />
            </div>
        ...
        <div className="flex items-center justify-end gap-3 pb-8">
          ...
          <Button type="submit" isLoading={updateService.isPending}>
