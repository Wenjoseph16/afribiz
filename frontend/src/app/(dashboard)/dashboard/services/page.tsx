...
export default function ServicesPage() {
  ...
  const chartData: ModuleChartData = useMemo(() => {
    const statusDist = [
      { name: 'Actifs', value: allServices.filter((s) => s.isActive).length, color: '#10b981' },
      { name: 'Inactifs', value: allServices.filter((s) => !s.isActive).length, color: '#9ca3af' },
      {
        name: 'Promotions',
        value: allServices.filter((s) => s.isPromotional).length,
        color: '#8b5cf6',
      },
    ].filter((d) => d.value > 0);
    const cats = [...new Set(allServices.map((s) => s.category?.name).filter(Boolean))] as string[];
    return {
      distribution:
        statusDist.length > 1
          ? statusDist
          : cats.map((c, i) => ({
              name: c,
              value: allServices.filter((s) => s.category?.name === c).length,
              color: ['#8b5cf6', '#10b981', '#f59e0b', '#06b6d4'][i % 4],
            })),
      daily: [
        { label: 'Total', value: allServices.length },
        {
          label: 'Réservations',
          value: allServices.reduce((a, s) => a + (s.bookingCount || 0), 0),
        },
        { label: 'Actifs', value: allServices.filter((s) => s.isActive).length },
      ],
    };
  }, [allServices]);
  ...
  const suggestions = useMemo(() => {
    const items: {
      type: 'no_bookings' | 'popular' | 'inactive' | 'promo';
      title: string;
      description: string;
      icon: any;
  ...
  return (
    <div className="space-y-6 animate-fade-in">
      ...
      <ModuleCharts
        ...
        distributionLabel="Statut des services"
            ...
            return (
              <div key={s.type} className={`rounded-2xl border p-4 ${colorMap[s.type]}`}>
                <div className="flex items-start gap-3">
                  <div
                    className={`w-9 h-9 rounded-xl bg-white dark:bg-gray-800 flex items-center justify-center shrink-0 ${iconColorMap[s.type]}`}
                  >
                      ...
                      <div className="mt-2 space-y-1">
                        {s.services.map((svc) => (
                          ...
                          >
                            ...
                            {s.type === 'no_bookings' && (
                              <span className="text-blue-600 shrink-0 ml-2">0 réservation</span>
                            )}
                            {s.type === 'popular' && (
                              <span className="text-emerald-600 shrink-0 ml-2">
                                {svc.bookingCount} réserv.
                              </span>
                            )}
                            {s.type === 'inactive' && (
                              <span className="text-amber-600 shrink-0 ml-2">Désactivé</span>
                            )}
                            {s.type === 'promo' && (
                              <span className="text-purple-600 shrink-0 ml-2">
                                {(svc.price ?? 0).toLocaleString()} FCFA
                              </span>
                            )}
                          </Link>
                        ))}
          ...
          <div className="flex items-center gap-1 bg-gray-100 dark:bg-gray-700 rounded-lg p-0.5">
            <button
              ...
              className={cn(
                ...
                viewMode === 'grid' ? 'bg-white dark:bg-gray-600 shadow-sm' : 'text-gray-400'
              )}
            >
            ...
            <button
              ...
              className={cn(
                'p-1.5 rounded-md transition-colors',
                viewMode === 'list' ? 'bg-white dark:bg-gray-600 shadow-sm' : 'text-gray-400'
              )}
            >
              ...
              <tr className="border-b border-gray-200 dark:border-gray-700 bg-gray-50 dark:bg-gray-800/50">
                <th className="p-4 w-10">
                  <input
                    type="checkbox"
                    checked={
                      selected.size === filteredServices.length && filteredServices.length > 0
                    }
                ...
                <th className="p-4 text-left text-xs font-semibold text-gray-500 uppercase hidden sm:table-cell">
                  Catégorie
                </th>
                <th className="p-4 text-right text-xs font-semibold text-gray-500 uppercase">
                  Prix
                </th>
                <th className="p-4 text-right text-xs font-semibold text-gray-500 uppercase hidden sm:table-cell">
                  Durée
                </th>
                <th className="p-4 text-right text-xs font-semibold text-gray-500 uppercase hidden md:table-cell">
                  Réserv.
                </th>
                <th className="p-4 text-right text-xs font-semibold text-gray-500 uppercase hidden md:table-cell">
                  Note
...
function getBadges(service: ServiceItem) {
  ...
  if ((service.bookingCount || 0) >= 20)
    badges.push({
      label: '🔥 Populaire',
      className: 'bg-orange-100 text-orange-700 dark:bg-orange-900/30 dark:text-orange-400',
    });
  if ((service.rating || 0) >= 4.5 && (service.reviewCount || 0) >= 5)
    badges.push({
      label: '⭐ Recommandé',
  ...
  if (
    service.createdAt &&
    (Date.now() - new Date(service.createdAt).getTime()) / (1000 * 60 * 60 * 24) <= 30
  ) {
    badges.push({
      label: '🆕 Nouveau',
...
function ServiceCard({
  ...
  return (
    <div
      className={cn(
        'group bg-white dark:bg-gray-800 rounded-2xl border overflow-hidden hover:border-brand/30 hover:shadow-sm transition-all duration-200',
    ...
    >
      <div className="p-4">
        <div className="flex items-start justify-between gap-2 mb-3">
          ...
          <div className="flex items-center gap-1.5 flex-wrap justify-end">
            {badges.slice(0, 2).map((b, i) => (
              <span
                key={i}
                className={`px-1.5 py-0.5 text-[10px] font-bold rounded-full ${b.className}`}
              >
                {b.label}
              </span>
            ))}
            {!service.isActive && (
              <span className="px-1.5 py-0.5 text-[10px] font-bold bg-gray-100 text-gray-500 rounded-full">
                INACTIF
              </span>
            )}
          </div>
        ...
        <Link href={`/dashboard/services/${service.id}`}>
          <h3 className="text-sm font-semibold text-gray-900 dark:text-gray-100 hover:text-brand transition-colors">
            {service.name}
          </h3>
        </Link>
...
function ServiceRow({
  ...
  return (
    <tr className="hover:bg-gray-50 dark:hover:bg-gray-700/30 transition-colors group">
            ...
            {badges.length > 0 && (
              <div className="flex gap-1 mt-0.5">
                {badges.map((b, i) => (
                  <span
                    key={i}
                    className={`text-[10px] font-bold px-1 py-0.5 rounded ${b.className}`}
                  >
                    {b.label}
                  </span>
                ))}
      ...
      <td className="p-4 text-sm text-gray-600 dark:text-gray-300 hidden sm:table-cell">
        ...
        {service.isPromotional && service.promotionalPrice ? (
          <>
            ...
            <span className="text-red-500">
              {(service.promotionalPrice ?? 0).toLocaleString()} FCFA
            </span>
          </>
        ) : (
          `${(service.price ?? 0).toLocaleString()} FCFA`
      ...
      <td className="p-4 text-right text-sm font-medium text-amber-500 hidden md:table-cell">
        {service.rating || '-'}
      </td>
      <td className="p-4 text-right">
        <span
          className={cn(
            ...
            service.isActive ? 'text-emerald-600' : 'text-gray-400'
          )}
        >
          <span
            className={cn(
      ...
      <td className="p-4">
        <div className="flex items-center gap-1 opacity-0 group-hover:opacity-100 transition-opacity">
          <Link
            ...
            className="p-1.5 rounded-lg hover:bg-gray-100 dark:hover:bg-gray-700 text-gray-400 hover:text-brand"
          >
