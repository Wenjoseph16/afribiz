...
export default function EventDetailPage() {
  ...
  const handleAddPromo = async () => {
    await createPromo.mutateAsync({
      ...
      data: {
        ...
        minTickets: Number(promoForm.minTickets),
      },
    });
    setPromoForm({
      code: '',
  ...
  const handleScan = async () => {
    ...
    try {
      ...
      setScanResult({
        success: false,
        message: err?.response?.data?.error || err?.message || 'Billet invalide',
      });
    }
    setQrInput('');
  };

  const handleScanWithRef = async (ref: string) => {
    if (!ref.trim()) return;
    setScanResult(null);
    try {
      await scanTicket.mutateAsync({ eventId: id, ticketRef: ref });
      setScanResult({ success: true, message: 'Billet valide — accès autorisé' });
    } catch (err: any) {
      setScanResult({
        success: false,
  ...
  const ticketsSold =
    stats?.totalTickets || ticketList.reduce((sum: number, t: any) => sum + (t.soldCount || 0), 0);
  const checkedIn =
    stats?.checkedIn || participantList.filter((p: any) => p.status === 'CHECKED_IN').length;
  const noShow = stats?.noShow || participantList.filter((p: any) => p.status === 'NO_SHOW').length;
  const cancelled =
    stats?.cancelled ||
    participantList.filter((p: any) => p.status === 'CANCELLED' || p.status === 'REFUNDED').length;
  const totalParticipants = participantList.length;
  const revenue =
    stats?.revenue ||
    ticketList.reduce((sum: number, t: any) => sum + (t.soldCount || 0) * (t.price || 0), 0);
  const noShowRate = totalParticipants > 0 ? ((noShow / totalParticipants) * 100).toFixed(1) : '0';
  const checkInRate =
    totalParticipants > 0 ? ((checkedIn / totalParticipants) * 100).toFixed(1) : '0';
        ...
        <div className="flex items-start justify-between">
          ...
          <div className="flex items-center gap-2 flex-wrap">
            {(e.participantCount || 0) >= 50 && (
              <span className="px-2.5 py-0.5 rounded-full text-xs font-medium bg-rose-500/30 text-white">
                🔥 Populaire
              </span>
            )}
            {(e.rating || 0) >= 4.5 && (
              <span className="px-2.5 py-0.5 rounded-full text-xs font-medium bg-amber-500/30 text-white">
                ⭐ Bien noté
              </span>
            )}
            {e.createdAt &&
            ...
            {e.isFeatured && (
              <span className="px-2.5 py-0.5 rounded-full text-xs font-medium bg-blue-500/30 text-white">
                🏆 Vedette
              </span>
            )}
            <span
      ...
      {activeTab === 'info' && (
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
          <Card padding="lg" className="md:col-span-2">
            ...
            {e.category && (
              <div className="mt-4">
                <span className="text-xs font-medium px-2 py-1 rounded-full bg-brand-50 text-brand">
                  {e.category}
                </span>
              </div>
            )}
              ...
              <div className="space-y-3 text-sm">
                <div className="flex justify-between">
                  ...
                  <span className="font-medium text-gray-900 dark:text-gray-100">
                    {e.capacity || 'Illimitée'}
                  </span>
                </div>
                <div className="flex justify-between">
                  <span className="text-gray-500">Billets vendus</span>
                  <span className="font-medium text-gray-900 dark:text-gray-100">
                    {e.ticketCount || 0}
                  </span>
                </div>
                ...
                {e.price > 0 && (
                  <div className="flex justify-between">
                    ...
                    <span className="font-medium text-gray-900 dark:text-gray-100">
                      {Number(e.price).toLocaleString()} FCFA
                    </span>
                  </div>
                )}
                {e.rating > 0 && (
                  <div className="flex justify-between">
                    ...
                    <span className="flex items-center gap-1 font-medium text-amber-600">
                      {e.rating.toFixed(1)} <Star className="h-3.5 w-3.5 fill-current" />
                    </span>
                  </div>
                )}
              </div>
      ...
      {activeTab === 'billetterie' && (
        <div className="space-y-4">
          <div className="flex items-center justify-between">
            ...
            <Button size="sm" onClick={() => setShowTicketForm(!showTicketForm)}>
            ...
            <Card padding="lg">
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
                ...
                <Input
                  ...
                  placeholder="Accès coupe-file, goodie bag"
                />
                <Input
                  label="Fin de vente"
                  type="date"
                  value={ticketForm.saleEndAt}
                  onChange={(e) => setTicketForm({ ...ticketForm, saleEndAt: e.target.value })}
                />
              </div>
              <div className="flex items-center gap-3 mt-4">
                <Button size="sm" onClick={handleAddTicket} isLoading={createTicket.isPending}>
                  Créer le billet
                </Button>
                <Button size="sm" variant="ghost" onClick={() => setShowTicketForm(false)}>
                  Annuler
                </Button>
              </div>
            </Card>
                ...
                <tbody className="divide-y divide-gray-100 dark:divide-gray-700">
                  ...
                  {ticketList.map((t: any) => (
                    ...
                    >
                      ...
                      <td className="px-4 py-3 text-gray-600 dark:text-gray-300">
                        {t.soldCount || 0} / {t.quantity || '-'}
                      </td>
                      <td className="px-4 py-3 text-gray-600 dark:text-gray-300">
                        {t.saleEndAt ? new Date(t.saleEndAt).toLocaleDateString('fr-FR') : '—'}
                      </td>
                      <td className="px-4 py-3">
                        <Badge variant={t.isActive !== false ? 'success' : 'danger'} size="xs">
                          {t.isActive !== false ? 'Actif' : 'Inactif'}
                        </Badge>
                      </td>
                      <td className="px-4 py-3 text-right">
                        <button
                          onClick={() => deleteTicket.mutateAsync({ eventId: id, ticketId: t.id })}
                ...
                <tbody className="divide-y divide-gray-100 dark:divide-gray-700">
                  ...
                  {filteredParticipants.map((p: any) => (
                    ...
                    >
                      ...
                      <td className="px-4 py-3 text-gray-600 dark:text-gray-300">
                        {p.ticketName || p.ticket?.name || '—'}
                      </td>
                      <td className="px-4 py-3 text-gray-600 dark:text-gray-300">
                        {p.price
                          ? `${Number(p.price).toLocaleString()} FCFA`
                          : p.ticket?.price
                            ? `${Number(p.ticket.price).toLocaleString()} FCFA`
                            : '—'}
                      </td>
                      <td className="px-4 py-3">
                        <Badge
                        ...
                        <div className="flex items-center justify-end gap-1">
                          {p.status !== 'CHECKED_IN' &&
                            ...
                            p.status !== 'REFUNDED' && (
                              <button
                                onClick={() =>
                                  updateParticipantStatus.mutateAsync({
                                    eventId: id,
                                    participantId: p.id,
                                    status: 'CHECKED_IN',
                                  })
            ...
            <Card padding="lg">
              ...
              <div className="flex items-center gap-3">
                <Input
                  ...
                  placeholder="Code du billet ou référence"
                  className="flex-1"
                />
                <Button onClick={handleScan} isLoading={scanTicket.isPending}>
                ...
                <div className="mt-4">
                  <QrScannerComponent
                    onScan={(data) => {
                      ...
                      const ref = data.replace('EVENT:', '').split(':').pop() || data;
                      setQrInput(ref);
                      handleScanWithRef(ref);
                    }}
                        ...
                        <td className="px-4 py-3">
                          <Badge
                            variant={
                              s.status === 'VALID'
                                ? 'success'
                                : s.status === 'ALREADY_USED'
                                  ? 'warning'
                                  : s.status === 'EXPIRED'
                                    ? 'default'
                                    : s.status === 'FRAUD_SUSPECTED'
                                      ? 'danger'
                                      : 'warning'
                            }
                            size="xs"
      ...
      {activeTab === 'promotions' && (
        <div className="space-y-4">
            ...
            <Card padding="lg">
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
                ...
                <Input
                  ...
                  placeholder={promoForm.discountType === 'PERCENTAGE' ? '10' : '5000'}
                />
                <Input
                ...
                <Input
                  label="Billets min"
                  type="number"
                  value={promoForm.minTickets}
                  onChange={(e) => setPromoForm({ ...promoForm, minTickets: e.target.value })}
                />
                <Input
                  label="Début"
                  type="date"
              ...
              <div className="flex items-center gap-3 mt-4">
                <Button size="sm" onClick={handleAddPromo} isLoading={createPromo.isPending}>
                  Créer la promotion
                </Button>
          ...
          {promoList.length === 0 && !showPromoForm && (
            <Card padding="lg">
              <p className="text-center text-gray-400">Aucune promotion créée</p>
            </Card>
          )}
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
            {promoList.map((p: any) => (
              <Card key={p.id} padding="md" className="relative">
                <button
                  onClick={() => deletePromo.mutateAsync({ eventId: id, promoId: p.id })}
                ...
                <div className="space-y-2">
                  <div className="flex items-center gap-2">
                    ...
                    <Badge variant={p.isActive !== false ? 'success' : 'danger'} size="xs">
                  ...
                  <div className="flex items-center gap-2 text-sm">
                    <Badge variant="brand" size="xs">
                      {p.discountType === 'PERCENTAGE'
                        ? `${p.discountValue}%`
                        : `${Number(p.discountValue).toLocaleString()} FCFA`}
                    </Badge>
                  </div>
                  <div className="text-xs text-gray-400 space-y-1">
                    <p>
                      Utilisations : {p.currentUses || 0}
                      {p.maxUses ? ` / ${p.maxUses}` : ''}
                    </p>
        ...
        <div className="space-y-4">
          ...
          {showGalleryForm && (
            <Card padding="lg">
              ...
              <div className="flex items-center gap-3 mt-4">
                <Button size="sm" onClick={handleAddGallery} isLoading={addGalleryItem.isPending}>
                  Ajouter
                </Button>
                <Button size="sm" variant="ghost" onClick={() => setShowGalleryForm(false)}>
                  Annuler
                </Button>
              </div>
            </Card>
          ...
          {galleryList.length === 0 && !showGalleryForm && (
            <Card padding="lg">
              <p className="text-center text-gray-400">Aucun média dans la galerie</p>
            </Card>
          )}
            ...
            {galleryList.map((item: any) => (
              <Card key={item.id} padding="none" className="relative group overflow-hidden">
                ...
                <div className="absolute inset-0 bg-black/50 opacity-0 group-hover:opacity-100 transition-opacity flex items-center justify-center gap-2">
                  <button
                    onClick={() => deleteGalleryItem.mutateAsync({ eventId: id, itemId: item.id })}
      ...
      {activeTab === 'partenaires' && (
        <div className="space-y-4">
          <div className="flex items-center justify-between">
            ...
            <Button size="sm" onClick={() => setShowPartnerForm(!showPartnerForm)}>
              <Plus className="h-4 w-4 mr-1.5" />
          ...
          {showPartnerForm && (
            <Card padding="lg">
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                ...
                <div className="flex items-end pb-2.5">
                  <label className="flex items-center gap-2 cursor-pointer">
                    <input
                      type="checkbox"
                      checked={partnerForm.isSponsor}
                      onChange={(e) =>
                        setPartnerForm({ ...partnerForm, isSponsor: e.target.checked })
                      }
              ...
              <div className="flex items-center gap-3 mt-4">
                <Button size="sm" onClick={handleAddPartner} isLoading={addPartner.isPending}>
                  Ajouter
                </Button>
                <Button size="sm" variant="ghost" onClick={() => setShowPartnerForm(false)}>
                  Annuler
                </Button>
              </div>
            </Card>
          ...
          {partnerList.length === 0 && !showPartnerForm && (
            <Card padding="lg">
              <p className="text-center text-gray-400">Aucun partenaire associé</p>
            </Card>
          )}
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
            {partnerList.map((p: any) => (
              <Card key={p.id} padding="md" className="relative flex items-center gap-4">
                <button
                  onClick={() => removePartner.mutateAsync({ eventId: id, partnerId: p.id })}
                  className="absolute top-3 right-3 text-gray-400 hover:text-red-500 transition-colors"
                >
                  <Trash2 className="h-4 w-4" />
                </button>
                      ...
                      <div className="w-full h-2 bg-gray-100 dark:bg-gray-700 rounded-full overflow-hidden">
                        <div
                          className={cn(
                            ...
                            pct >= 90 ? 'bg-emerald-500' : pct >= 50 ? 'bg-brand' : 'bg-amber-500'
                          )}
