...
export default function EventsPage() {
  ...
  const allEvents: EventItem[] = Array.isArray(eventsData)
    ? eventsData
  ...
  const stats = {
    ...
    avgRating:
      allEvents.length > 0
        ? allEvents.reduce((a, e) => a + (e.rating || 0), 0) / allEvents.length
        : 0,
    totalRevenue: 0,
    fillRate: 0,
    ...(statsData || {}),
  ...
  const chartData: ModuleChartData = useMemo(() => {
    const now2 = new Date();
    const upcoming = allEvents.filter((e) => new Date(e.startDate) > now2);
    const ongoing = allEvents.filter(
      (e) => new Date(e.startDate) <= now2 && new Date(e.endDate) >= now2
    );
    const completed = allEvents.filter((e) => new Date(e.endDate) < now2);
    const cancelled = allEvents.filter((e) => e.status === 'CANCELLED');
    const published = allEvents.filter((e) => e.status === 'PUBLISHED' || e.status === 'UPCOMING');
    return {
      trend: [
        { label: 'Total', value: allEvents.length },
        { label: 'À venir', value: upcoming.length },
        { label: 'En cours', value: ongoing.length },
        { label: 'Terminés', value: completed.length },
        { label: 'Annulés', value: cancelled.length },
      ],
      distribution: [
        { name: 'À venir', value: upcoming.length, color: '#3b82f6' },
        { name: 'En cours', value: ongoing.length, color: '#10b981' },
        { name: 'Terminés', value: completed.length, color: '#6b7280' },
        { name: 'Annulés', value: cancelled.length, color: '#ef4444' },
      ].filter((d) => d.value > 0),
      daily: [
        {
          label: 'Participants',
          value: allEvents.reduce((a, e) => a + (e.participantCount || 0), 0),
        },
        { label: 'Actifs', value: published.length },
        {
          label: 'Note moy.',
          value: Math.round(
            (allEvents.reduce((a, e) => a + (e.rating || 0), 0) / (allEvents.length || 1)) * 10
          ),
        },
      ],
    };
  ...
  const filtered = useMemo(() => {
    ...
    switch (activeTab) {
      case 'upcoming':
        f = f.filter((e) => new Date(e.startDate) > now);
        break;
      case 'ongoing':
        f = f.filter((e) => new Date(e.startDate) <= now && new Date(e.endDate) >= now);
        break;
      case 'completed':
        f = f.filter((e) => new Date(e.endDate) < now);
        break;
      case 'cancelled':
        f = f.filter((e) => e.status === 'CANCELLED');
        break;
    }
    if (searchQuery) {
    ...
    <div className="space-y-6 animate-fade-in">
      ...
      <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-6 gap-4">
        ...
        <StatsCard
          ...
          value={stats.fillRate ? stats.fillRate + '%' : '—'}
        />
      </div>
        ...
        {badges.length > 0 && (
          <div className="absolute top-2 right-2 flex flex-wrap gap-1">
            {badges.map((b, i) => (
              <span
                key={i}
                className="text-[10px] font-medium px-1.5 py-0.5 rounded-full bg-white/90 dark:bg-gray-900/90 shadow-sm"
              >
                {b.label}
              </span>
            ))}
            ...
            {badges.length > 0 && (
              <div className="flex flex-wrap gap-1 mt-0.5">
                {badges.map((b, i) => (
                  <span
                    key={i}
                    className="text-[10px] font-medium px-1.5 py-0.5 rounded-full bg-gray-100 dark:bg-gray-700"
                  >
                    {b.label}
                  </span>
                ))}
