...
const statusConfig: Record<string, { label: string; class: string }> = {
  ACTIVE: {
    label: 'Active',
    class: 'text-blue-600 bg-blue-50 dark:bg-blue-900/20 dark:text-blue-300',
  },
  PARTIALLY_PAID: {
    label: 'Partiellement payée',
    class: 'text-amber-600 bg-amber-50 dark:bg-amber-900/20 dark:text-amber-300',
  },
  OVERDUE: {
    label: 'En retard',
  ...
  CRITICAL: {
    label: 'Critique',
    class: 'text-red-800 bg-red-100 dark:bg-red-950/30 dark:text-red-400',
  },
  DISPUTED: {
    label: 'Contestée',
  ...
  SETTLED: {
    label: 'Réglée',
    class: 'text-emerald-600 bg-emerald-50 dark:bg-emerald-900/20 dark:text-emerald-300',
  },
  CANCELLED: {
    label: 'Annulée',
    class: 'text-gray-500 bg-gray-100 dark:bg-gray-800 dark:text-gray-400',
  },
};
  ...
  return (
    <div className="animate-fade-in space-y-6">
            ...
            <div className="flex-1">
              <div className="flex items-center gap-3 flex-wrap">
                <h1 className="text-xl sm:text-2xl font-bold text-white">
                  {debt.title || debt.description || `Dette #${id.slice(0, 8)}`}
                </h1>
                <span className={cn('text-xs font-medium px-2.5 py-1 rounded-full', s.class)}>
                  {s.label}
                </span>
                <span className={cn('text-xs font-medium px-2.5 py-1 rounded-full', p.class)}>
                  {p.label}
                </span>
              </div>
              {debt.dueDate && (
            ...
            <Card className="p-5">
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                <div className="space-y-1">
                  ...
                  <p className="text-lg font-bold text-gray-900 dark:text-white">
                    {Number(debt.totalAmount || debt.amount).toLocaleString()} FCFA
                  </p>
                </div>
                <div className="space-y-1">
                  <p className="text-xs text-gray-500">Montant payé</p>
                  <p className="text-lg font-bold text-emerald-600">
                    {Number(debt.amountPaid || 0).toLocaleString()} FCFA
                  </p>
                </div>
                <div className="space-y-1">
                  ...
                  <p className="text-lg font-bold text-red-600">
                    {Number(
                      (debt.totalAmount || debt.amount) - (debt.amountPaid || 0)
                    ).toLocaleString()}{' '}
                    FCFA
                  </p>
                </div>
              ...
              <div className="mt-4 pt-4 border-t border-gray-100 dark:border-gray-700">
                ...
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    <p className="text-sm font-semibold text-gray-900 dark:text-white">
                      {debt.clientName || debt.client?.name || '—'}
                    </p>
                    <p className="text-xs text-gray-500">
                      {debt.clientEmail || debt.client?.email || ''}
                    </p>
                    {debt.clientPhone && (
                      <p className="text-xs text-gray-400">{debt.clientPhone}</p>
                    )}
                  </div>
                </div>
              </div>
              ...
              <div className="space-y-2">
                <div className="flex justify-between text-sm">
                  <span className="text-gray-500">
                    {Number(debt.amountPaid || 0).toLocaleString()} FCFA payés
                  </span>
                ...
                <div className="w-full h-3 bg-gray-100 dark:bg-gray-700 rounded-full overflow-hidden">
                  <div
                    className={cn(
                      ...
                      progress >= 1 ? 'bg-emerald-500' : 'bg-brand'
                    )}
                    ...
                    <tbody>
                      {(debt.payments || debt.paymentHistory || []).map((pmt: any, i: number) => (
                        <tr key={i} className="border-b border-gray-50 dark:border-gray-800">
                          <td className="py-2.5 text-gray-900 dark:text-white">
                            {new Date(pmt.date || pmt.createdAt).toLocaleDateString('fr-FR')}
                          </td>
                          ...
                          <td className="py-2.5 text-xs text-gray-400">
                            {pmt.reference || pmt.transactionId || '—'}
                          </td>
                        </tr>
                      ))}
                    </tbody>
                ...
                <Button
                  ...
                  onClick={() => {
                    ...
                    paymentMutation.mutate({
                      id,
                      data: { amount: Number(paymentAmount), method: paymentMethod },
                    });
                    setPaymentAmount('');
                  }}
                >
            ...
            <Card className="p-5">
              ...
              <div className="space-y-3 text-sm">
                <div className="flex items-center gap-2 text-gray-600 dark:text-gray-400">
                  ...
                  <span>
                    Créée le{' '}
                    {debt.createdAt ? new Date(debt.createdAt).toLocaleDateString('fr-FR') : '—'}
                  </span>
                </div>
                <div className="flex items-center gap-2 text-gray-600 dark:text-gray-400">
                  ...
                  <span>
                    ...
                    {debt.updatedAt ? new Date(debt.updatedAt).toLocaleDateString('fr-FR') : '—'}
                  </span>
                </div>
                <div className="flex items-center gap-2 text-gray-600 dark:text-gray-400">
      ...
      <ConfirmationModal
        ...
        description="Êtes-vous sûr de vouloir supprimer cette dette ? Cette action est irréversible."
        ...
        <div className="space-y-4">
          ...
          {(debt.reminders || []).length === 0 ? (
            <Card className="text-center py-10">
              ...
              <p className="text-xs text-gray-500">
                Aucun rappel n'a encore été envoyé pour cette dette
              </p>
            </Card>
          ) : (
            <div className="space-y-3">
              {(debt.reminders || []).map((rem: any, i: number) => {
                const rt = reminderTypeConfig[rem.type || rem.channel] || {
                  label: rem.type || rem.channel || '—',
                  class: 'text-gray-500 bg-gray-100 dark:bg-gray-800',
                };
                return (
                    ...
                    <div className="flex items-start justify-between gap-3">
                      <div className="flex-1 min-w-0">
                        ...
                        <p className="text-sm text-gray-900 dark:text-white">
                          {rem.content || rem.message || 'Aucun contenu'}
                        </p>
                        {rem.sentAt && (
                          <p className="text-xs text-gray-400 mt-1">
                            Envoyé le {new Date(rem.sentAt).toLocaleString('fr-FR')}
                          </p>
                        )}
                      </div>
                    </div>
        ...
        <div>
          {escrowData ? (
                ...
                <Card className="p-5 space-y-4">
                  ...
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                    ...
                    <div>
                      ...
                      <p className="text-sm font-semibold text-gray-900 dark:text-white">
                        {escrow.releasedAt
                          ? new Date(escrow.releasedAt).toLocaleDateString('fr-FR')
                          : '—'}
                      </p>
                    </div>
                  </div>
                  {escrow.status === 'HELD' && (
                    <div className="flex flex-wrap gap-2 pt-2 border-t border-gray-100 dark:border-gray-700">
                      <Button
                        size="sm"
                        variant="primary"
                        onClick={() => escrowReleaseMutation.mutate(escrow.id)}
                        isLoading={escrowReleaseMutation.isPending}
                      >
                        <CheckCircle2 className="h-4 w-4 mr-1.5" />
                        Libérer
                      </Button>
                      <Button
                        size="sm"
                        variant="secondary"
                        onClick={() => escrowRefundMutation.mutate(escrow.id)}
                        isLoading={escrowRefundMutation.isPending}
                      >
                      ...
                      <Button
                        size="sm"
                        variant="danger"
                        onClick={() => escrowDisputeMutation.mutate(escrow.id)}
                        isLoading={escrowDisputeMutation.isPending}
                      >
            ...
            <Card className="text-center py-10">
              ...
              <p className="text-xs text-gray-500">
                Aucun compte séquestre n'est associé à cette dette
              </p>
            </Card>
          )}
        </div>
        ...
        <div className="space-y-4">
          {riskData ? (
                ...
                <>
                  <Card className="p-5 space-y-4">
                    <div className="flex items-center justify-between">
                      ...
                      <span className={cn('text-xs font-medium px-3 py-1 rounded-full', rl.class)}>
                        {rl.label}
                      </span>
                    </div>
                  ...
                  <Card className="p-5">
                    ...
                    <p className="text-sm text-gray-600 dark:text-gray-400">
                      {risk.analysis ||
                        risk.comment ||
                        'Aucune analyse détaillée disponible pour ce client.'}
                    </p>
                  </Card>
                  <Link href="/dashboard/clients">
                    <Button variant="secondary">
                      <User className="h-4 w-4 mr-1.5" />
                      Voir la gestion des risques clients
                    </Button>
                  </Link>
                </>
              );
            ...
            <Card className="text-center py-10">
              ...
              <p className="text-xs text-gray-500">
                Les informations de risque ne sont pas disponibles pour ce client
              </p>
            </Card>
          )}
        </div>
            ...
            <div className="flex items-center gap-2">
              ...
              <select
                ...
                onChange={(e) => {
                  ...
                  priorityMutation.mutate({ id, priority: e.target.value });
                }}
