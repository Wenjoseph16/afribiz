        ...
        <Card padding="sm" className="text-center">
          ...
          {product.isPromotional && (
            <p className="text-[10px] text-red-500 font-medium">
              Promo: {(product.promotionalPrice || 0).toLocaleString()}
            </p>
          )}
        </Card>
        <Card padding="sm" className="text-center">
          ...
          <p className="text-[10px] text-gray-500">
            {stockStatus === 'out'
              ? 'Rupture'
              : stockStatus === 'low'
                ? `Stock faible (min: ${product.lowStockThreshold || 5})`
                : 'En stock'}
          </p>
        </Card>
        <Card padding="sm" className="text-center">
            ...
            <div className="space-y-6">
                ...
                <div>
                  ...
                  <div className="grid grid-cols-4 sm:grid-cols-6 gap-2">
                    {product.images.map((img: string, i: number) => (
                      <div
                        key={i}
                        className="aspect-square rounded-xl bg-gray-100 dark:bg-gray-700 flex items-center justify-center text-gray-400 text-sm"
                      >
                        🖼️
                      </div>
                    ))}
              ...
              <div className="grid grid-cols-2 sm:grid-cols-3 gap-4">
                ...
                <div className="p-3 bg-gray-50 dark:bg-gray-700/30 rounded-xl">
                  ...
                  <p className="text-sm font-semibold text-gray-900 dark:text-gray-100">
                    {product.isPhysical ? 'Physique' : 'Numérique'}
                  </p>
                </div>
                <div className="p-3 bg-gray-50 dark:bg-gray-700/30 rounded-xl">
                ...
                <div className="flex items-center gap-4 text-sm">
                  <span
                    className={cn(
                      ...
                      product.isVisibleOnPublicPage !== false ? 'text-emerald-600' : 'text-gray-400'
                    )}
                  >
                    <span
                      className={cn(
                        'w-2 h-2 rounded-full',
                        product.isVisibleOnPublicPage !== false ? 'bg-emerald-500' : 'bg-gray-300'
                      )}
                    />
                    Page publique
                  </span>
                  <span
                    className={cn(
                      'flex items-center gap-1.5',
                      product.isVisibleOnMarketplace !== false
                        ? 'text-emerald-600'
                        : 'text-gray-400'
                    )}
                  >
                    <span
                      className={cn(
                        'w-2 h-2 rounded-full',
                        product.isVisibleOnMarketplace !== false ? 'bg-emerald-500' : 'bg-gray-300'
                      )}
                    />
                    Marketplace
                  </span>
                  <span
                    className={cn(
                      'flex items-center gap-1.5',
                      product.isActive ? 'text-emerald-600' : 'text-gray-400'
                    )}
                  >
                    <span
                      className={cn(
                ...
                <div className="p-4 bg-gray-50 dark:bg-gray-700/30 rounded-xl">
                  ...
                  {product.seoTitle && (
                    <p className="text-sm text-gray-900 dark:text-gray-100">
                      <span className="text-gray-500">Titre :</span> {product.seoTitle}
                    </p>
                  )}
                  {product.seoDescription && (
                    <p className="text-sm text-gray-500 mt-1">{product.seoDescription}</p>
                  )}
                </div>
              )}
            </div>
              ...
              {(product.variants || []).length === 0 ? (
                ...
                <div className="space-y-2">
                  ...
                  {(product.variants || []).map((v: any, i: number) => (
                    <div
                      key={v.id || i}
                      className="grid grid-cols-5 gap-3 p-3 bg-gray-50 dark:bg-gray-700/30 rounded-xl items-center"
                    >
                      ...
                      <div className="text-right">
                        <span
                          className={cn(
                            'text-sm font-medium',
                            v.stock === 0 ? 'text-red-500' : 'text-gray-900 dark:text-gray-100'
                          )}
                        >
                          {v.stock}
                        </span>
          ...
          {activeTab === 'orders' && (
            <div className="text-center py-8">
              ...
              <p className="text-sm text-gray-400 mb-1">
                {product.orderCount || 0} commande(s) pour ce produit
              </p>
              ...
              <div className="grid grid-cols-2 sm:grid-cols-4 gap-4">
                ...
                <div className="p-4 bg-gradient-to-br from-purple-50 to-purple-100 dark:from-purple-900/20 dark:to-purple-800/20 rounded-xl text-center">
                  <p className="text-2xl font-bold text-purple-600">
                    {((product.price || 0) * (product.orderCount || 0)).toLocaleString()}
                  </p>
                  <p className="text-xs text-purple-500 mt-1">
                    Revenu estimé ({product.currency || 'FCFA'})
                  </p>
                </div>
              </div>
            </div>
              ...
              <div className="space-y-3">
                <div className="flex items-start gap-3 p-3 bg-gray-50 dark:bg-gray-700/30 rounded-xl">
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    <p className="text-xs text-gray-500">
                      Le {new Date(product.createdAt).toLocaleString('fr-FR')}
                    </p>
                  </div>
                </div>
                {product.updatedAt && product.updatedAt !== product.createdAt && (
                  <div className="flex items-start gap-3 p-3 bg-gray-50 dark:bg-gray-700/30 rounded-xl">
                    ...
                    <div className="flex-1 min-w-0">
                      ...
                      <p className="text-xs text-gray-500">
                        Le {new Date(product.updatedAt).toLocaleString('fr-FR')}
                      </p>
                ...
                <div className="flex items-start gap-3 p-3 bg-gray-50 dark:bg-gray-700/30 rounded-xl">
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    <p className="text-xs text-gray-500">
                      {product.orderCount || 0} commande(s) · {product.reviewCount || 0} avis ·
