  ...
  const stats = useMemo(
    () => ({
      ...
      totalProducts: categories.reduce(
        (acc, c) =>
          acc + c.productCount + (c.children?.reduce((s, ch) => s + ch.productCount, 0) || 0),
        0
      ),
    }),
    [categories]
  ...
  return (
    <div className="space-y-6 animate-fade-in">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
        ...
        <Card padding="sm" className="text-center">
          <p className="text-2xl font-bold text-gray-900 dark:text-gray-100">{stats.parents}</p>
          <p className="text-xs text-gray-500 mt-0.5">Principales</p>
        </Card>
        <Card padding="sm" className="text-center">
          <p className="text-2xl font-bold text-brand">{stats.totalProducts}</p>
      ...
      <ConfirmationModal
        ...
        title={`Supprimer la catégorie "${deleteCategoryTarget?.name}" ?`}
        description="Cette action est irréversible. Tous les produits de cette catégorie seront désaffectés."
    ...
    <div>
      ...
      >
        ...
        <div className="w-8 h-8 rounded-xl bg-brand/5 flex items-center justify-center text-sm shrink-0">
          {category.icon || <FolderTree className="h-4 w-4 text-brand" />}
        </div>
        ...
        <div className="flex items-center gap-0.5 opacity-0 group-hover:opacity-100 shrink-0">
          <button
            ...
            className="p-1 rounded-lg hover:bg-gray-100 text-gray-400 hover:text-brand"
          >
            <Plus className="h-3.5 w-3.5" />
          </button>
          <button
            onClick={() => onEdit(category)}
            className="p-1 rounded-lg hover:bg-gray-100 text-gray-400 hover:text-blue-500"
          >
