...
export default function ProductsPage() {
  ...
  const [deleteProductTarget, setDeleteProductTarget] = useState<boolean>(false);
  ...
  const TABS = [
    { id: 'all' as TabType, label: 'Tous', count: allProducts.length },
    {
      ...
      label: 'Actifs',
      count: allProducts.filter((p) => p.isActive).length,
    },
    {
    ...
    },
    {
      ...
      label: 'Stock faible',
      count: allProducts.filter((p) => p.stock > 0 && p.stock <= (p.lowStockThreshold || 5)).length,
    },
    {
      id: 'out-of-stock' as TabType,
      label: 'Rupture',
      count: allProducts.filter((p) => p.stock === 0).length,
    },
    {
      id: 'promotional' as TabType,
      label: 'Promotions',
      count: allProducts.filter((p) => p.isPromotional).length,
    },
  ];
  ...
  const chartData: ModuleChartData = useMemo(() => {
    ...
    const statusDist = [
      { name: 'Actifs', value: allProducts.filter((p) => p.isActive).length, color: '#10b981' },
      { name: 'Inactifs', value: allProducts.filter((p) => !p.isActive).length, color: '#9ca3af' },
      {
        name: 'Stock faible',
        value: allProducts.filter((p) => p.stock > 0 && p.stock <= (p.lowStockThreshold || 5))
          .length,
        color: '#f59e0b',
      },
      { name: 'Rupture', value: allProducts.filter((p) => p.stock === 0).length, color: '#ef4444' },
      {
        name: 'Promotions',
        value: allProducts.filter((p) => p.isPromotional).length,
        color: '#8b5cf6',
      },
    ].filter((d) => d.value > 0);
    const catDist = cats.map((cat, i) => ({
      name: cat,
      value: allProducts.filter((p) => p.category === cat).length,
      color: ['#6366f1', '#10b981', '#f59e0b', '#8b5cf6', '#06b6d4', '#ef4444'][i % 6],
    }));
    return {
      distribution: statusDist.length > 1 ? statusDist : catDist,
      daily: [
        { label: 'Total', value: allProducts.length },
        { label: 'Actifs', value: allProducts.filter((p) => p.isActive).length },
        { label: 'Promos', value: allProducts.filter((p) => p.isPromotional).length },
      ],
    };
  }, [allProducts]);
  ...
  const suggestions = useMemo(() => {
    const items: {
      type: 'low_stock' | 'no_sales' | 'popular' | 'promo' | 'new';
      title: string;
      description: string;
      icon: any;
    ...
    const lowStockItems = allProducts.filter(
      (p) => p.isActive && p.stock > 0 && p.stock <= (p.lowStockThreshold || 5)
    );
  ...
  const handleBatchExport = () => {
    ...
    const rows = selected.map(
      (p) => `"${p.name}",${p.price},${p.stock},"${p.sku || ''}","${p.category || ''}"`
    );
    const csv = header + '\n' + rows.join('\n');
  ...
  return (
    <div className="space-y-6 animate-fade-in">
      ...
      <ModuleCharts
        ...
        trendLabel="Ventes (7 jours)"
        distributionLabel="État des stocks"
        dailyLabel="Ventes par jour"
        variant="products"
      />
      ...
      <ConfirmationModal
        ...
        description={`Êtes-vous sûr de vouloir supprimer ${selectedProducts.length} produit(s) ? Cette action est irréversible.`}
...
function getBadges(product: ProductItem) {
  ...
  if ((product.soldCount || 0) >= 20)
    badges.push({
      label: '🏆 Top vente',
      className: 'bg-amber-100 text-amber-700 dark:bg-amber-900/30 dark:text-amber-400',
    });
  else if ((product.soldCount || 0) >= 10)
    badges.push({
      label: '🔥 Populaire',
        ...
        <Link href={`/dashboard/products/${product.id}`} className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-xs text-gray-500">
              {(product.rating ?? 0).toFixed(1)} ★ · {product.soldCount ?? 0} vendus
            </p>
            {badges.length > 0 && (
