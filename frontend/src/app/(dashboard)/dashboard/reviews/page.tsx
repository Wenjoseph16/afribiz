...
import { Star, Edit3, Trash2, TrendingUp, Award, Sparkles, AlertTriangle } from 'lucide-react';
...
export default function ReviewsPage() {
  ...
  const renderStars = (note: number, interactive = false, onRate?: (n: number) => void) => {
    return Array.from({ length: 5 }).map((_, i) => (
      ...
      >
        <Star
          className={cn('h-4 w-4', i < note ? 'text-amber-400 fill-current' : 'text-gray-200')}
        />
      </button>
    ));
  };
  ...
  const confirmDelete = async () => {
    ...
    setDeletingId(deleteConfirmTarget);
    try {
      await apiClient.delete(`/reviews/${deleteConfirmTarget}`);
      qc.invalidateQueries({ queryKey: ['reviews'] });
    } catch {
  ...
  const avgRating =
    reviews.length > 0
      ? Math.round(
          (reviews.reduce((a: number, r: any) => a + (r.rating || r.note || 0), 0) /
            reviews.length) *
            10
        ) / 10
      : 0;
  ...
  return (
    <div className="animate-fade-in">
      ...
      {reviews.length > 0 && (
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 mb-6">
          {[
            {
              icon: Star,
              title: `${reviews.length} avis publiés`,
              desc: `Note moyenne : ${avgRating}/5`,
              color: 'emerald' as const,
            },
            recentCount > 0 && {
              icon: Sparkles,
              title: `${recentCount} avis récent${recentCount > 1 ? 's' : ''}`,
              desc: 'Publiés au cours des 30 derniers jours',
              color: 'blue' as const,
            },
            highRated > 0 && {
              icon: Award,
              title: `${highRated} avis positif${highRated > 1 ? 's' : ''}`,
                ...
                <div className="flex-1">
                  <div className="flex items-center gap-2 mb-1">
                    <span className="text-sm font-semibold text-gray-900">
                      {review.businessName || review.business}
                    </span>
                    <span className="text-xs text-gray-400">
                      · {review.type || review.reviewType || 'Avis'}
                    </span>
                  </div>
                  {editingId === review.id ? (
                    <div className="space-y-2">
                      ...
                      <div className="flex gap-2">
                        ...
                        <Button variant="outline" size="xs" onClick={() => setEditingId(null)}>
                          Annuler
                        </Button>
                      </div>
                    </div>
                    ...
                    <>
                      ...
                      <p className="text-sm text-gray-600">
                        {review.comment || review.commentaire || ''}
                      </p>
      ...
      <ConfirmationModal
        ...
        description="Cette action est irréversible. L'avis sera définitivement supprimé."
