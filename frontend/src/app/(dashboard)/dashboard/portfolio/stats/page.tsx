...
export default function PortfolioStatsPage() {
  ...
  const topItems = [...items]
    .sort((a: any, b: any) => (b.views || b.interactions || 0) - (a.views || a.interactions || 0))
    .slice(0, 5);
      ...
      <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
        <Card className="p-3 sm:p-4">
          <div className="flex items-center gap-3">
            ...
            <div>
              <p className="text-[10px] text-gray-500 uppercase font-semibold">Projets</p>
              <p className="text-lg font-bold">{stats.totalItems ?? items.length}</p>
            </div>
          </div>
        </Card>
        <Card className="p-3 sm:p-4">
          <div className="flex items-center gap-3">
            ...
            <div>
              <p className="text-[10px] text-gray-500 uppercase font-semibold">Vues totales</p>
        ...
        <Card className="p-3 sm:p-4">
          <div className="flex items-center gap-3">
            ...
            <div>
              <p className="text-[10px] text-gray-500 uppercase font-semibold">Catégories</p>
              <p className="text-lg font-bold">{stats.totalCategories ?? categories.length}</p>
            </div>
          </div>
        </Card>
      </div>
                ...
                <div key={item.id} className="flex items-center gap-3">
                  ...
                  <div className="text-right">
                    <p className="text-xs font-bold text-gray-900 dark:text-white">
                      {item.views || item.interactions || 0}
                    </p>
            ...
            <div className="space-y-3">
              {categories.map((cat: any) => {
                const count = items.filter(
                  (i: any) => i.category === cat.name || i.categoryId === cat.id
                ).length;
                const maxCount = Math.max(
                  ...categories.map(
                    (c: any) =>
                      items.filter((i: any) => i.category === c.name || i.categoryId === c.id)
                        .length
                  ),
                  1
                );
                  ...
                  <div key={cat.id}>
                    <div className="flex justify-between text-xs mb-1">
                      ...
                      <span className="font-medium text-gray-900 dark:text-white">
                        {count} ({items.length > 0 ? Math.round((count / items.length) * 100) : 0}%)
                      </span>
                    </div>
        ...
        <Card className="p-4">
          ...
          <div className="grid grid-cols-2 gap-3">
            <div className="p-3 rounded-xl bg-gray-50 dark:bg-gray-800/50 text-center">
              ...
              <p className="text-[10px] text-gray-500">Vues / projet</p>
            </div>
            <div className="p-3 rounded-xl bg-gray-50 dark:bg-gray-800/50 text-center">
              <p className="text-2xl font-bold text-gray-900 dark:text-white">
                {totalViews > 0 ? ((totalLikes / totalViews) * 100).toFixed(1) : '0'}%
              </p>
              <p className="text-[10px] text-gray-500">Taux engagement</p>
            </div>
            <div className="p-3 rounded-xl bg-gray-50 dark:bg-gray-800/50 text-center">
              <p className="text-2xl font-bold text-gray-900 dark:text-white">
                {items.length > 0 && categories.length > 0
                  ? (items.length / categories.length).toFixed(1)
                  : '—'}
              </p>
              <p className="text-[10px] text-gray-500">Projets / catégorie</p>
            </div>
            <div className="p-3 rounded-xl bg-gray-50 dark:bg-gray-800/50 text-center">
              <p className="text-2xl font-bold text-gray-900 dark:text-white">
                {items.filter((i: any) => i.clientName).length}
              </p>
              <p className="text-[10px] text-gray-500">Avec client</p>
            </div>
          </div>
        </Card>
        ...
        <Card className="p-4 bg-gradient-to-r from-blue-50 to-indigo-50 dark:from-blue-900/20 dark:to-indigo-900/20 border-blue-200 dark:border-blue-800/30">
          ...
          <div className="space-y-2">
            ...
            {categories.length === 0 && items.length > 0 && (
              <div className="p-3 rounded-xl bg-white/60 dark:bg-gray-800/60">
                <p className="text-xs font-medium text-blue-800 dark:text-blue-300">
                  🏷️ Organisez vos projets
                </p>
                <p className="text-[10px] text-blue-600 dark:text-blue-400 mt-1">
                  Créez des catégories pour mieux organiser votre portfolio.
                </p>
              </div>
            )}
            {items.length > 0 && (
              <div className="p-3 rounded-xl bg-white/60 dark:bg-gray-800/60">
                ...
                <p className="text-[10px] text-blue-600 dark:text-blue-400 mt-1">
                  {totalViews > 0
                    ? `${totalViews} vues au total. Les projets les plus performants génèrent le plus d'engagement.`
                    : 'Ajoutez des images et vidéos pour augmenter les interactions.'}
                </p>
              </div>
            )}
          </div>
