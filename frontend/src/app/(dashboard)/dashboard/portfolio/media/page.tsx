...
export default function PortfolioMediaPage() {
  ...
  const loadMedias = async (itemId: string) => {
    ...
    try {
      ...
      if (item?.images) {
        existingMedias.push(
          ...item.images.map((url: string) => ({ id: url, url, type: 'image', caption: '' }))
        );
      }
      if (item?.videos) {
        existingMedias.push(
          ...item.videos.map((url: string) => ({ id: url, url, type: 'video', caption: '' }))
        );
      }
  ...
  const handleUpload = async () => {
    ...
    setUploading(true);
    try {
      await apiClient.post('/business/portfolio/media', {
        itemId: form.itemId,
        url: form.url,
        type: form.type,
  ...
  const handleDelete = async (itemId: string, mediaUrl: string) => {
    ...
    try {
      ...
      setMedias((prev) => ({
        ...prev,
        [itemId]: (prev[itemId] || []).filter((m: any) => m.url !== mediaUrl),
      }));
  ...
  const filteredItems = items.filter(
    (i: any) => !search || i.title?.toLowerCase().includes(search.toLowerCase())
  );
      ...
      <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
        <Card className="p-3">
          <div className="flex items-center gap-2">
            ...
            <div>
              <p className="text-[10px] text-gray-500">Projets</p>
        ...
        <Card className="p-3">
          <div className="flex items-center gap-2">
            ...
            <div>
              <p className="text-[10px] text-gray-500">Avec médias</p>
              <p className="text-lg font-bold">
                {items.filter((i: any) => i.image || i.images?.length || i.videos?.length).length}
              </p>
            </div>
          </div>
        </Card>
        <Card className="p-3">
          <div className="flex items-center gap-2">
            ...
            <div>
              <p className="text-[10px] text-gray-500">Vidéos</p>
              <p className="text-lg font-bold">
                {items.filter((i: any) => i.videos?.length).length}
              </p>
            </div>
          </div>
        </Card>
        <Card className="p-3">
          <div className="flex items-center gap-2">
            ...
            <div>
              <p className="text-[10px] text-gray-500">Fichiers</p>
              <p className="text-lg font-bold">{Object.values(medias).flat().length}</p>
            </div>
          </div>
        </Card>
      </div>
        ...
        <div className="space-y-3">
          ...
          <div className="space-y-1 max-h-[500px] overflow-y-auto">
            {filteredItems.map((item: any) => (
              <button
                key={item.id}
                onClick={() => loadMedias(item.id)}
                className={cn(
                  'w-full flex items-center gap-3 p-2.5 rounded-xl text-left text-sm transition-colors',
        ...
        <div className="lg:col-span-2">
          {!selectedItem ? (
            <Card className="text-center py-12">
              ...
              <p className="text-sm text-gray-500">Sélectionnez un projet pour voir ses médias</p>
            </Card>
          ) : loadingMedia ? (
            ...
            <div className="space-y-4">
              <div className="flex items-center justify-between">
                <h3 className="text-sm font-semibold text-gray-900 dark:text-white">
                  {items.find((i: any) => i.id === selectedItem)?.title} — {allMedias.length}{' '}
                  média(s)
                </h3>
                <div className="flex border border-gray-200 dark:border-gray-700 rounded-lg overflow-hidden">
                  <button
                    ...
                    className={cn(
                      'p-1.5',
                      viewMode === 'grid' ? 'bg-brand text-white' : 'text-gray-500'
                    )}
                  >
                  ...
                  <button
                    ...
                    className={cn(
                      'p-1.5',
                      viewMode === 'list' ? 'bg-brand text-white' : 'text-gray-500'
                    )}
                  >
                  ...
                  {allMedias.map((media: any, i: number) => (
                    ...
                    >
                      ...
                      <div className="absolute inset-0 bg-black/0 group-hover:bg-black/30 transition-all flex items-center justify-center opacity-0 group-hover:opacity-100">
                        <button
                          onClick={() => handleDelete(selectedItem, media.url)}
                      ...
                      {media.caption && (
                        <p className="absolute bottom-0 left-0 right-0 p-1.5 text-[10px] text-white bg-gradient-to-t from-black/60 bg-transparent">
                          {media.caption}
                        </p>
                      )}
                    </div>
                  ...
                  {allMedias.map((media: any, i: number) => (
                    ...
                    >
                      ...
                      <div className="flex-1 min-w-0">
                        ...
                        {media.caption && (
                          <p className="text-[10px] text-gray-400">{media.caption}</p>
                        )}
                      </div>
