...
export default function PortfolioTestimonialsPage() {
  ...
  const handleDelete = async (id: string) => {
    try {
      await apiClient.deletePortfolioTestimonial(id);
      setTestimonials((prev: any[]) => prev.filter((t: any) => t.id !== id));
    } catch (err) {
      console.error(err);
    }
  };
    ...
    <div className="animate-fade-in space-y-6">
      ...
      <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
        <Card className="p-3">
          <div className="flex items-center gap-2">
            ...
            <div>
              ...
              <p className="text-lg font-bold">{testimonials.length}</p>
            </div>
          </div>
        </Card>
        <Card className="p-3">
            ...
            <div>
              ...
              <p className="text-lg font-bold">
                {testimonials.length > 0
                  ? (
                      testimonials.reduce((s: number, t: any) => s + (t.rating || 5), 0) /
                      testimonials.length
                    ).toFixed(1)
                  : '—'}
              </p>
        ...
        <Card className="p-3">
          <div className="flex items-center gap-2">
            ...
            <div>
              <p className="text-[10px] text-gray-500">5 étoiles</p>
              <p className="text-lg font-bold">
                {testimonials.filter((t: any) => t.rating === 5).length}
              </p>
            </div>
          </div>
        </Card>
        <Card className="p-3">
          <div className="flex items-center gap-2">
            ...
            <div>
              <p className="text-[10px] text-gray-500">Auteurs</p>
              <p className="text-lg font-bold">
                {new Set(testimonials.map((t: any) => t.authorName)).size}
              </p>
            </div>
          </div>
        </Card>
      ...
      {filtered.length === 0 ? (
        <Card className="text-center py-12">
          <MessageSquare className="h-12 w-12 text-gray-200 dark:text-gray-700 mx-auto mb-3" />
          <p className="text-sm text-gray-500">{search ? 'Aucun résultat' : 'Aucun témoignage'}</p>
        </Card>
      ) : (
          ...
          {filtered.map((t: any) => (
            <Card key={t.id} className="p-4 relative">
              <button
                ...
                className="absolute top-3 right-3 p-1 rounded-lg hover:bg-gray-100 dark:hover:bg-gray-700 text-gray-300 hover:text-red-500"
              >
                  ...
                  <div className="flex gap-0.5 mt-0.5">
                    {[1, 2, 3, 4, 5].map((s) => (
                      <Star
                        ...
                        className={cn(
                          'w-3 h-3',
                          s <= (t.rating || 5) ? 'text-yellow-400 fill-yellow-400' : 'text-gray-300'
                        )}
                      />
                    ))}
                  </div>
              ...
              {t.projectId && items.find((i: any) => i.id === t.projectId) && (
                <p className="text-xs text-gray-400 mt-2">
                  — Projet : {items.find((i: any) => i.id === t.projectId)?.title}
                </p>
              )}
            </Card>
          ))}
            ...
            <div className="space-y-4">
              ...
              <div className="flex gap-2 pt-2">
                ...
                <button
                  ...
                  disabled={creating || !form.content || !form.authorName}
