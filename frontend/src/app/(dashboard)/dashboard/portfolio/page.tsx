...
const displayCategory = (
  cat: string | { id: string; name: string; slug: string } | null
): string => {
  if (!cat) return '-';
...
export default function PortfolioPage() {
  ...
  const allItems: PortfolioItem[] = Array.isArray(itemsData)
    ? itemsData
    ...
    if (searchQuery) {
      ...
      f = f.filter(
        (i) =>
          ...
          (typeof i.category === 'string' ? i.category.toLowerCase().includes(q) : false) ||
      ...
      {allItems.length > 0 &&
        (() => {
          ...
          const recent = allItems.filter(
            (i) =>
              i.projectDate &&
              new Date(i.projectDate) > new Date(Date.now() - 180 * 24 * 60 * 60 * 1000)
          );
          ...
          const suggestions = [
            ...
            categories.length > 0 && {
              ...
              desc:
                ...
                (categories.length > 3 ? ` et +${categories.length - 3}` : ''),
              color: 'purple',
            },
            recent.length > 0 && {
              type: 'recent',
          ...
          return (
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
              {suggestions.map((s: any, i) => (
                <Link
                  key={i}
                  href={s.link || '/dashboard/portfolio'}
      ...
      {filtered.length === 0 ? (
        <Card className="text-center py-12">
          <EmptyState
            ...
            description={
              searchQuery ? 'Essayez une autre recherche' : 'Ajoutez votre première réalisation'
            }
            action={
              <Link href="/dashboard/portfolio/new">
                <Button>
    ...
    <div className="group bg-white dark:bg-gray-800 rounded-2xl border border-gray-200 dark:border-gray-700 overflow-hidden hover:border-brand/30 hover:shadow-sm transition-all duration-200">
      ...
      <div className="p-4 space-y-2">
        ...
        <div className="flex items-center justify-between pt-1">
          <span className="text-xs text-gray-400">{item.interactions || 0} interactions</span>
...
function PortfolioRow({ item, onDelete }: { item: PortfolioItem; onDelete: (id: string) => void }) {
  const badges = getBadges(item);
  return (
