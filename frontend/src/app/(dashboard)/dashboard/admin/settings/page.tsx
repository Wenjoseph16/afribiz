...
export default function AdminSettingsPage() {
  ...
  const { data: settings, isLoading } = useQuery({
    ...
    queryFn: async () => {
      try {
        ...
        console.warn('Erreur chargement parametres:', error);
        return {};
      }
    },
    enabled: isAdmin,
  ...
  const { data: verificationSettings } = useQuery({
    ...
    queryFn: async () => {
      try {
        const res = await apiClient.adminGetVerificationSettings();
        return res.data.data || { mode: 'badge_only' };
      } catch (error) {
        console.warn('Erreur chargement verification settings:', error);
        return { mode: 'badge_only' };
      }
    },
    enabled: isAdmin,
    retry: false,
  ...
  const updateMutation = useMutation({
    mutationFn: (data: any) => {
      ...
      if (mode) {
        return apiClient.adminUpdateSettings({ mode });
      }
      return apiClient.adminUpdateSettings(data);
    },
    onSuccess: () => {
  ...
  const monetizationMutation = useMutation({
    mutationFn: (data: any) => {
      ...
      return apiClient.adminUpdateSettings(flatData);
    },
    onSuccess: () => {
      qc.invalidateQueries({ queryKey: ['admin', 'settings'] });
    ...
    <div className="space-y-6 animate-fade-in">
      {toast && (
        ...
        >
          ...
          {toast.message}
          <button onClick={() => setToast(null)} className="ml-auto font-bold">
            &times;
          </button>
        </div>
      )}
        ...
        {activeTab === 'general' && (
          <Card>
            <div className="space-y-5">
              ...
              <div>
                <label className="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1.5">
                  Pays actifs (séparés par des virgules)
                </label>
                <input
                  type="text"
              ...
              <div>
                ...
                <p className="text-xs text-gray-500 dark:text-gray-400 mb-2">
                  Définit le niveau de vérification requis. En mode badge, le KYC est optionnel et
                ...
                <div className="space-y-3">
                  {[
                    {
                      value: 'badge_only',
                      label: 'Badge uniquement',
                      desc: 'KYC optionnel – les vérifiés reçoivent un badge de confiance. Aucun blocage.',
                    },
                    {
                      value: 'recommended',
                      label: 'Recommandé',
                      desc: 'KYC recommandé avec rappels, mais jamais bloquant.',
                    },
                    {
                      value: 'required',
                      label: 'Obligatoire',
                      desc: 'KYC requis pour publier des modules ou recevoir des paiements.',
                    },
                  ].map((opt) => (
                    <label
                      key={opt.value}
                      className="flex items-start gap-3 cursor-pointer p-3 rounded-lg border-2 border-gray-200 dark:border-gray-700 has-[:checked]:border-brand has-[:checked]:bg-brand/5"
                    >
                      <input
                        type="radio"
            ...
            <div className="space-y-5">
              ...
              <div>
                ...
                <input
                  ...
                  onChange={(e) =>
                    updateField('finances', 'platformCommission', Number(e.target.value))
                  }
                  min="0"
                  max="100"
                  step="0.1"
              ...
              <div>
                <label className="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1.5">
                  Seuil paiement escrow
                </label>
                <input
                  type="number"
                  value={f.escrowThreshold || 0}
                  onChange={(e) =>
                    updateField('finances', 'escrowThreshold', Number(e.target.value))
                  }
                  min="0"
                  className="w-full px-4 py-2.5 rounded-xl border-2 border-gray-200 dark:border-gray-700 bg-white dark:bg-gray-800 text-gray-900 dark:text-gray-100 focus:border-brand focus:ring-brand/20 focus:outline-none"
                />
              ...
              <div>
                <label className="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1.5">
                  Délai validation paiement (heures)
                </label>
                <input
                  type="number"
                  value={f.paymentValidationDelay || 24}
                  onChange={(e) =>
                    updateField('finances', 'paymentValidationDelay', Number(e.target.value))
                  }
                  min="1"
                  className="w-full px-4 py-2.5 rounded-xl border-2 border-gray-200 dark:border-gray-700 bg-white dark:bg-gray-800 text-gray-900 dark:text-gray-100 focus:border-brand focus:ring-brand/20 focus:outline-none"
                />
          ...
          <Card>
            <div className="space-y-5">
              ...
              <p className="text-sm text-gray-500 dark:text-gray-400 -mt-3">
                Ces taux sont lus dynamiquement par le backend. Modifiez-les ici pour impacter toute
                la plateforme sans toucher au code.
              </p>
              ...
              <div>
                ...
                <p className="text-xs text-gray-500 dark:text-gray-400 mb-1">
                  Sur chaque paiement réussi (Mobile Money, Stripe, etc.)
                </p>
                <div className="flex items-center gap-3">
                  <input
                    ...
                    step="0.1"
                    value={
                      mo.transactionCommissionRate != null ? mo.transactionCommissionRate * 100 : 1
                    }
                    onChange={(e) =>
                      updateField(
                        'monetization',
                  ...
                  <span className="text-lg font-bold text-brand min-w-[4rem] text-right">
                    {(mo.transactionCommissionRate != null
                      ? mo.transactionCommissionRate * 100
                      : 1
                    ).toFixed(1)}
                    %
                  </span>
              ...
              <div>
                ...
                <p className="text-xs text-gray-500 dark:text-gray-400 mb-1">
                  Frais de tiers de confiance sur chaque libération d'escrow
                </p>
                <div className="flex items-center gap-3">
                  <input
                    type="range"
                  ...
                  />
                  <span className="text-lg font-bold text-brand min-w-[4rem] text-right">
                    {(mo.escrowCommissionRate != null ? mo.escrowCommissionRate * 100 : 2).toFixed(
                      1
                    )}
                    %
                  </span>
              ...
              <div>
                <label className="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1.5">
                  Commission modules développeur (%)
                </label>
                <p className="text-xs text-gray-500 dark:text-gray-400 mb-1">
                  Part d'AfriBiz sur les ventes de modules développeurs
                </p>
                <div className="flex items-center gap-3">
                  <input
                    type="range"
                    ...
                    step="1"
                    value={
                      mo.developerModuleCommissionRate != null
                        ? mo.developerModuleCommissionRate * 100
                        : 20
                    }
                    onChange={(e) =>
                      updateField(
              ...
              <div className="grid grid-cols-2 gap-4">
                <div>
                  <label className="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1.5">
                    Frais escrow minimum (FCFA)
                  </label>
                  <input
                    type="number"
                    value={mo.minimumEscrowFee || 0}
                    onChange={(e) =>
                      updateField('monetization', 'minimumEscrowFee', Number(e.target.value))
                    }
                    min="0"
                    className="w-full px-4 py-2.5 rounded-xl border-2 border-gray-200 dark:border-gray-700 bg-white dark:bg-gray-800 text-gray-900 dark:text-gray-100 focus:border-brand focus:ring-brand/20 focus:outline-none"
                  />
              ...
              <div className="bg-blue-50 dark:bg-blue-900/20 border border-blue-200 dark:border-blue-800 rounded-xl p-4 text-sm text-blue-700 dark:text-blue-300">
                ...
                <ul className="list-disc list-inside space-y-1 text-xs">
                  <li>
                    Les taux sont stockés en base de données et lus dynamiquement par le backend.
                  </li>
                  <li>
                    Vous pouvez aussi les gérer via la page <strong>Commissions</strong> pour des
                    règles plus fines (par type de business, etc.).
                  </li>
                  <li>
                    Les valeurs par défaut (1% transactions, 2% escrow) s'appliquent si rien n'est
                    configuré.
                  </li>
                </ul>
              </div>
            ...
            <div className="space-y-5">
              ...
              <div>
                ...
                <input
                  ...
                  onChange={(e) =>
                    updateField('subscriptions', 'businessPrice', Number(e.target.value))
                  }
                  min="0"
                  className="w-full px-4 py-2.5 rounded-xl border-2 border-gray-200 dark:border-gray-700 bg-white dark:bg-gray-800 text-gray-900 dark:text-gray-100 focus:border-brand focus:ring-brand/20 focus:outline-none"
                />
              ...
              <div>
                ...
                <input
                  ...
                  onChange={(e) =>
                    updateField('subscriptions', 'developerPrice', Number(e.target.value))
                  }
                  min="0"
                  className="w-full px-4 py-2.5 rounded-xl border-2 border-gray-200 dark:border-gray-700 bg-white dark:bg-gray-800 text-gray-900 dark:text-gray-100 focus:border-brand focus:ring-brand/20 focus:outline-none"
                />
              ...
              <div>
                <label className="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1.5">
                  Prix Module Marketplace
                </label>
                <input
                  type="number"
                  value={s.marketplaceModulePrice || 0}
                  onChange={(e) =>
                    updateField('subscriptions', 'marketplaceModulePrice', Number(e.target.value))
                  }
                  min="0"
                  className="w-full px-4 py-2.5 rounded-xl border-2 border-gray-200 dark:border-gray-700 bg-white dark:bg-gray-800 text-gray-900 dark:text-gray-100 focus:border-brand focus:ring-brand/20 focus:outline-none"
                />
              ...
              <div>
                <label className="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1.5">
                  Prix Data Hub Partner
                </label>
                <input
                  type="number"
                  value={s.dataHubPartnerPrice || 0}
                  onChange={(e) =>
                    updateField('subscriptions', 'dataHubPartnerPrice', Number(e.target.value))
                  }
                  min="0"
                  className="w-full px-4 py-2.5 rounded-xl border-2 border-gray-200 dark:border-gray-700 bg-white dark:bg-gray-800 text-gray-900 dark:text-gray-100 focus:border-brand focus:ring-brand/20 focus:outline-none"
                />
                  ...
                  {['banner', 'video', 'native', 'popup', 'sidebar'].map((fmt) => (
                    <label key={fmt} className="flex items-center gap-2 cursor-pointer">
                      <input
                        ...
                        onChange={(e) => {
                          const current = Array.isArray(a.allowedFormats)
                            ? [...a.allowedFormats]
                            : [];
              ...
              <div>
                ...
                <div className="grid grid-cols-2 gap-3">
                  <div>
                    ...
                    <input
                      type="number"
                      value={af.excellentThreshold || 90}
                      onChange={(e) =>
                        updateField('afriscore', 'excellentThreshold', Number(e.target.value))
                      }
                      min="0"
                      max="100"
                      className="w-full px-3 py-2 rounded-xl border-2 border-gray-200 dark:border-gray-700 bg-white dark:bg-gray-800 text-gray-900 dark:text-gray-100 focus:border-brand focus:ring-brand/20 focus:outline-none"
                    />
                  </div>
                  <div>
                    ...
                    <input
                      type="number"
                      value={af.goodThreshold || 70}
                      onChange={(e) =>
                        updateField('afriscore', 'goodThreshold', Number(e.target.value))
                      }
                      min="0"
                      max="100"
                      className="w-full px-3 py-2 rounded-xl border-2 border-gray-200 dark:border-gray-700 bg-white dark:bg-gray-800 text-gray-900 dark:text-gray-100 focus:border-brand focus:ring-brand/20 focus:outline-none"
                  ...
                  <div>
                    ...
                    <input
                      type="number"
                      value={af.averageThreshold || 40}
                      onChange={(e) =>
                        updateField('afriscore', 'averageThreshold', Number(e.target.value))
                      }
                      min="0"
                      max="100"
                      className="w-full px-3 py-2 rounded-xl border-2 border-gray-200 dark:border-gray-700 bg-white dark:bg-gray-800 text-gray-900 dark:text-gray-100 focus:border-brand focus:ring-brand/20 focus:outline-none"
                  ...
                  <div>
                    ...
                    <input
                      type="number"
                      value={af.lowThreshold || 20}
                      onChange={(e) =>
                        updateField('afriscore', 'lowThreshold', Number(e.target.value))
                      }
                      min="0"
                      max="100"
                      className="w-full px-3 py-2 rounded-xl border-2 border-gray-200 dark:border-gray-700 bg-white dark:bg-gray-800 text-gray-900 dark:text-gray-100 focus:border-brand focus:ring-brand/20 focus:outline-none"
            ...
            <div className="space-y-5">
              ...
              <div>
                ...
                <input
                  ...
                  onChange={(e) =>
                    updateField('notifications', 'dailySendLimit', Number(e.target.value))
                  }
                  min="1"
                  className="w-full px-4 py-2.5 rounded-xl border-2 border-gray-200 dark:border-gray-700 bg-white dark:bg-gray-800 text-gray-900 dark:text-gray-100 focus:border-brand focus:ring-brand/20 focus:outline-none"
                />
            ...
            <div className="space-y-5">
              ...
              <div>
                ...
                <input
                  ...
                  onChange={(e) =>
                    updateField('marketplace', 'saleCommission', Number(e.target.value))
                  }
                  min="0"
                  max="100"
                  step="0.1"
              ...
              <div>
                ...
                <input
                  ...
                  onChange={(e) =>
                    updateField('marketplace', 'featuredFee', Number(e.target.value))
                  }
                  min="0"
                  className="w-full px-4 py-2.5 rounded-xl border-2 border-gray-200 dark:border-gray-700 bg-white dark:bg-gray-800 text-gray-900 dark:text-gray-100 focus:border-brand focus:ring-brand/20 focus:outline-none"
                />
