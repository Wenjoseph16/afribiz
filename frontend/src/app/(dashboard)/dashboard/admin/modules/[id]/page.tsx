...
export default function AdminModuleDetailPage() {
  ...
  const [statusActionTarget, setStatusActionTarget] = useState<string | null>(null);
  ...
  const updateMutation = useMutation({
    ...
    onSuccess: () => {
      ...
      setEditMode(false);
      setToast({ message: 'Module mis à jour', type: 'success' });
    },
  ...
  const enableEdit = () => {
    if (mod) {
      setEditForm({
        pricingType: mod.pricingType || '',
        price: mod.price || 0,
        commissionRate: mod.commissionRate || 0,
      });
      setEditMode(true);
    }
  };
  ...
  if (!mod)
    return (
      <EmptyState
        ...
        description="Ce module n'existe pas."
      />
    );
    ...
    <div className="space-y-6 animate-fade-in">
      ...
      <Card>
        <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
          <div className="flex items-center gap-4">
            ...
            <div>
              <div className="flex items-center gap-2">
                ...
                <span
                  className={`text-xs font-medium px-1.5 py-0.5 rounded-full ${STATUS_STYLES[mod.status] || 'bg-gray-100 text-gray-600'}`}
                >
                  {STATUS_LABELS[mod.status] || mod.status}
                </span>
              </div>
              <p className="text-sm text-gray-500">
                par {mod.developer?.name || mod.developerId || 'N/A'} · v{mod.version || '1.0.0'}
              </p>
            </div>
          </div>
          ...
          <div className="flex items-center gap-2">
            ...
            {!editMode && (
              <Button variant="secondary" size="sm" onClick={enableEdit}>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
            ...
            <div>
              ...
              <div className="flex items-center gap-2">
                ...
                />
                <span className="text-sm font-bold text-brand min-w-[3rem]">
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
        {[
          ...
          {
            label: 'Revenus',
            value: mod.revenue ? `${Number(mod.revenue).toLocaleString()} FCFA` : '0 FCFA',
            icon: TrendingUp,
            color: 'bg-brand-50 dark:bg-brand-900/30 text-brand',
          },
          {
            label: 'Note',
            value: mod.rating ? `${mod.rating.toFixed(1)}/5` : '-',
            icon: Star,
            ...
            <div className="grid grid-cols-2 gap-4">
              ...
              <div>
                ...
                <p className="text-gray-900 dark:text-gray-100 font-medium">
                  {mod.pricingType || 'FREE'} · {mod.price ? `${mod.price} FCFA` : 'Gratuit'}
                </p>
              </div>
            </div>
        ...
        <Card>
          <div className="text-center py-12">
            ...
            <p className="text-sm text-gray-400 mt-1">
              Les graphiques de revenus seront disponibles prochainement.
            </p>
          </div>
        </Card>
      )}
                  ...
                  <table className="w-full text-sm">
                    ...
                    <tbody>
                      {commissions.transactions.map((tx: any) => (
                        <tr
                          key={tx.id}
                          className="border-b border-gray-100 dark:border-gray-800 text-gray-600 dark:text-gray-400"
                        >
        ...
        <Card>
          {mod.reviews?.length > 0 ? (
            ...
            <EmptyState
              ...
              title="Aucun avis"
              description="Ce module n'a pas encore reçu d'avis."
            />
          )}
        </Card>
      )}
      <ConfirmationModal
        ...
        onConfirm={async () => {
          ...
          await statusMutation.mutateAsync({ action: statusActionTarget });
          setStatusActionTarget(null);
        }}
        title="Confirmer l'action"
        description={`Êtes-vous sûr de vouloir ${statusActionTarget === 'publish' ? 'publier' : statusActionTarget === 'archive' ? 'archiver' : statusActionTarget === 'reject' ? 'rejeter' : ''} ce module ?`}
