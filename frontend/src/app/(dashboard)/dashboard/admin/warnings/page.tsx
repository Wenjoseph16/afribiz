...
function useWarnings(params?: any) {
  return useQuery({
    ...
    queryFn: async () => {
      try {
        const res = await apiClient.adminGetWarnings(params);
        return res.data.data;
      } catch (error) {
        console.warn('Erreur chargement avertissements:', error);
        return { warnings: [], totalPages: 1 };
      }
    },
    retry: false,
  });
...
function useWarningsStats() {
  return useQuery({
    ...
    queryFn: async () => {
      try {
        const res = await apiClient.adminGetWarningStats();
        return res.data.data;
      } catch (error) {
        console.warn('Erreur chargement stats avertissements:', error);
...
function useUsersSearch() {
  return useMutation({
    mutationFn: async (query: string) => {
      const res = await apiClient.adminSearchUsers({ search: query, limit: 10 });
      return res.data.data;
    },
  });
}
...
export default function AdminWarningsPage() {
  ...
  const createMutation = useMutation({
    mutationFn: (data: any) => apiClient.adminCreateWarning(warnForm.userId, data),
    onSuccess: () => {
  ...
  const revokeMutation = useMutation({
    mutationFn: (id: string) => apiClient.adminDeleteWarning(id),
    onSuccess: () => {
      qc.invalidateQueries({ queryKey: ['admin', 'warnings'] });
  ...
  const handleIssueWarn = () => {
    if (!warnForm.userId || !warnForm.reason) {
      setToast({
        message: 'Veuillez sélectionner un utilisateur et saisir un motif',
        type: 'error',
      });
      return;
    }
      ...
      <div className="flex flex-col sm:flex-row items-start justify-between gap-4">
        <div>
          ...
          <p className="text-sm text-gray-500 dark:text-gray-400 mt-1">
            Émettez et gérez les avertissements utilisateurs
          </p>
        </div>
        <Button
          onClick={() => {
        ...
        <Card padding="sm">
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-xs text-gray-500">Total émis</p>
            </div>
          </div>
        </Card>
        <Card padding="sm">
      ...
      <Card padding="none">
        {isLoading ? (
                  ...
                  >
                    <td className="p-4">
                      <span className="font-semibold text-gray-900 dark:text-gray-100">
                        {w.user?.name || w.user?.email || w.userId?.slice(0, 8) || '-'}
                      </span>
                    </td>
                    <td className="p-4 text-gray-500">
                      {w.issuedBy?.name || w.issuedById?.slice(0, 8) || '-'}
                    </td>
                    <td className="p-4">
                      <div>
                    ...
                    <td className="p-4">
                      <span
                        className={`text-[10px] font-medium px-1.5 py-0.5 rounded-full ${STATUS_STYLES[w.status] || ''}`}
                      >
          ...
          <EmptyState
            ...
            description={
              hasActiveFilters ? 'Aucun résultat pour ces filtres.' : 'Aucun avertissement émis.'
            }
          />
        )}
      </Card>
      ...
      >
        <div className="space-y-4">
          ...
          <Input
            ...
            placeholder="Ex: Non-respect des CGU"
          />
          <div>
            <label className="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1.5">
              Description
          ...
          <Input
            ...
            value={warnForm.action}
            onChange={(e) => setWarnForm({ ...warnForm, action: e.target.value })}
            placeholder="Ex: Suspension temporaire"
          />
          <Input
            label="Expiration (optionnelle)"
            type="date"
            value={warnForm.expiresAt}
            onChange={(e) => setWarnForm({ ...warnForm, expiresAt: e.target.value })}
          />
        </div>

        <div className="flex justify-end gap-3 mt-6 pt-4 border-t border-gray-100 dark:border-gray-700">
          <Button variant="secondary" onClick={() => setIssuModal(false)}>
            Annuler
          </Button>
          <Button
            onClick={handleIssueWarn}
            isLoading={createMutation.isPending}
            disabled={!warnForm.userId || !warnForm.reason}
          >
        ...
        <ConfirmationModal
          ...
          title="Révoquer l'avertissement"
          description="Êtes-vous sûr de vouloir révoquer cet avertissement ?"
