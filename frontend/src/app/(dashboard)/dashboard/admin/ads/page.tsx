...
export default function AdminAdsPage() {
  ...
  const {
    ...
    queryFn: async () => {
      const res = await apiClient.adminGetAllAdCampaigns({
        search,
  ...
  const campaigns = Array.isArray(campaignsData?.data || campaignsData)
  ...
  const statsCards = [
    ...
    {
      label: 'Suspendues',
      value: campaigns.filter((c: any) => c.status === 'SUSPENDED').length,
      icon: PauseCircle,
      color: 'text-red-600',
    },
  ];
          ...
          <div className="grid grid-cols-2 sm:grid-cols-4 gap-4">
            <div>
              ...
              <p className="text-lg font-bold text-emerald-600">
                {(revenueData.totalRevenue || 0).toLocaleString()} FCFA
              </p>
            </div>
            <div>
              <p className="text-xs text-gray-500">Revenus mensuels</p>
              <p className="text-lg font-bold text-blue-600">
                {(revenueData.monthlyRevenue || 0).toLocaleString()} FCFA
              </p>
            </div>
            <div>
              <p className="text-xs text-gray-500">Factures en attente</p>
              <p className="text-lg font-bold text-amber-600">
                {(revenueData.pendingInvoices || 0).toLocaleString()} FCFA (
                {revenueData.pendingInvoiceCount || 0})
              </p>
            </div>
            <div>
              <p className="text-xs text-gray-500">Dépenses publicitaires</p>
              <p className="text-lg font-bold text-purple-600">
                {(revenueData.totalAdSpend || 0).toLocaleString()} FCFA
              </p>
            </div>
          </div>
        </Card>
      ...
      <Card padding="none">
        {isLoading ? (
          ...
          <EmptyState
            ...
            description="Il n'y a pas encore de campagnes publicitaires sur la plateforme."
          />
        ) : (
                  ...
                  >
                    ...
                    <td className="p-4">
                      <p className="text-sm text-gray-900 dark:text-gray-100">
                        {campaign.business?.name || campaign.companyName || 'N/A'}
                      </p>
                    ...
                    <td className="p-4">
                      <div className="flex items-center gap-2 text-xs">
                        ...
                        <MousePointerClick className="h-3 w-3 text-gray-400 ml-1" />
                        ...
                        {campaign.status === 'PENDING' && (
                          <>
                            <Button
                              variant="ghost"
                              size="xs"
                              onClick={() => validateMutation.mutate(campaign.id)}
