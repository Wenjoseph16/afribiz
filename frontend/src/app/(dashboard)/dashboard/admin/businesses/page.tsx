...
export default function AdminBusinessesPage() {
  ...
  const handleStatusAction = async (id: string, action: string, businessName: string) => {
    setActionTarget({ id, action, name: businessName });
  };
                    ...
                    <td className="p-4">
                      <div className="flex items-center gap-1.5">
                        <Button
                          ...
                          size="xs"
                          onClick={() =>
                            (window.location.href = `/dashboard/admin/businesses/${b.id}`)
                          }
                        >
      ...
      <ConfirmationModal
        ...
        onConfirm={async () => {
          ...
          try {
            ...
            setToast({
              message: `Business ${actionLabels[actionTarget.action]} avec succès`,
              type: 'success',
            });
          } catch {
            setToast({
        ...
        description={`Êtes-vous sûr de vouloir effectuer cette action sur le business « ${actionTarget?.name} » ?`}
