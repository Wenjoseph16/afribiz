...
export default function AdminSecurityPage() {
  ...
  const { data: stats } = useQuery({
    ...
    queryFn: async () => {
      const res = await apiClient.adminGetSecurityStats();
  ...
  const { data: adminsData, isLoading: loadingAdmins } = useQuery({
    ...
    queryFn: async () => {
      const res = await apiClient.adminGetSecurityAdmins({ page, limit });
  ...
  const { data: sessionsData, isLoading: loadingSessions } = useQuery({
    ...
    queryFn: async () => {
      const res = await apiClient.adminGetSecuritySessions({ page, limit });
  ...
  const { data: attemptsData, isLoading: loadingAttempts } = useQuery({
    ...
    queryFn: async () => {
      const res = await apiClient.adminGetSecurityAttempts({ page, limit });
  ...
  const { data: blacklistData, isLoading: loadingBlacklist } = useQuery({
    ...
    queryFn: async () => {
      const res = await apiClient.adminGetBlacklist({ page, limit });
  ...
  const { data: journalData, isLoading: loadingJournal } = useQuery({
    ...
    queryFn: async () => {
      const res = await apiClient.adminGetSecurityJournal({ page, limit });
  ...
  const revokeSessionMutation = useMutation({
    mutationFn: (sessionId: string) => apiClient.adminDeleteSecuritySession(sessionId),
    onSuccess: () => {
      qc.invalidateQueries({ queryKey: ['admin', 'security', 'sessions'] });
      setToast({ message: 'Session révoquée', type: 'success' });
    },
    onError: () => {
  ...
  const blockIpMutation = useMutation({
    mutationFn: (ip: string) => apiClient.adminAddToBlacklist({ ip }),
    onSuccess: () => {
      qc.invalidateQueries({ queryKey: ['admin', 'security', 'attempts'] });
      qc.invalidateQueries({ queryKey: ['admin', 'security', 'blacklist'] });
      setToast({ message: 'IP bloquée', type: 'success' });
    },
    onError: () => {
      setToast({ message: 'Erreur lors du blocage', type: 'error' });
    },
  });

  const unblockIpMutation = useMutation({
    mutationFn: (ip: string) => apiClient.adminRemoveFromBlacklist(ip),
    onSuccess: () => {
      qc.invalidateQueries({ queryKey: ['admin', 'security', 'blacklist'] });
      setToast({ message: 'IP débloquée', type: 'success' });
    },
    onError: () => {
      setToast({ message: 'Erreur lors du déblocage', type: 'error' });
    },
  });
  ...
  const totalPages =
    ...
    attemptsData?.totalPages ??
    blacklistData?.totalPages ??
    journalData?.totalPages ??
    1;
                ...
                <tbody>
                  {admins.map((a: any) => (
                    ...
                    >
                      ...
                      <td className="p-4">
                        <span className="text-[10px] font-medium px-1.5 py-0.5 rounded-full bg-brand-50 dark:bg-brand-900/30 text-brand">
                          {a.role || '-'}
                        </span>
                      </td>
                      <td className="p-4">
                      ...
                      <td className="p-4 text-xs text-gray-500">
                        {a.lastLogin ? new Date(a.lastLogin).toLocaleString('fr-FR') : '-'}
                      </td>
                    </tr>
                  ))}
                </tbody>
                  ...
                  {sessions.map((s: any) => (
                    ...
                    >
                      ...
                      <td className="p-4 text-gray-500 text-xs">
                        {s.device || s.userAgent?.slice(0, 40) || '-'}
                      </td>
                      <td className="p-4 text-xs text-gray-500">
        ...
        <Card padding="none">
          {loadingAttempts ? (
                  ...
                  {attempts.map((att: any) => (
                    ...
                    >
                      <td className="p-4 text-xs text-gray-900 dark:text-gray-100">
                        {att.date ? new Date(att.date).toLocaleString('fr-FR') : '-'}
                      </td>
                      <td className="p-4 text-gray-500">{att.email || '-'}</td>
                      <td className="p-4 text-xs text-gray-500">{att.ip || '-'}</td>
                      <td className="p-4">
                        <span className="text-xs font-medium px-1.5 py-0.5 rounded-full bg-red-50 dark:bg-red-900/30 text-red-600">
                          {att.count || 0}
                        </span>
                      </td>
                      <td className="p-4">
                        <Button
                          variant="ghost"
                          size="xs"
                          onClick={() => blockIpMutation.mutate(att.ip)}
                        >
            ...
            <EmptyState
              ...
              description="Aucune tentative suspecte détectée."
            />
          )}
        </Card>
      )}
        ...
        <Card padding="none">
          {loadingBlacklist ? (
                ...
                <tbody>
                  {ips.map((ip: any) => (
                    ...
                    >
                      ...
                      <td className="p-4 text-xs text-gray-500">
                        {ip.blockedAt ? new Date(ip.blockedAt).toLocaleString('fr-FR') : '-'}
                      </td>
                      <td className="p-4">
                        <Button
                          variant="ghost"
            ...
            <EmptyState
              ...
              description="La liste noire est vide."
            />
          )}
        </Card>
      )}
        ...
        <Card padding="none">
          {loadingJournal ? (
            ...
            <EmptyState
              ...
              title="Aucune entrée"
              description="Le journal de sécurité est vide."
            />
          )}
        </Card>
      )}
