...
import {
  ...
  useAdminSuspendPartner,
  useAdminRevokePartner,
  useAdminGetDataAccessLogs,
  useAdminGetReports,
  useAdminGetPlatformAnalytics,
  useAdminRecomputeAllScores,
...
export default function AdminDataHubPage() {
  ...
  const { data: partnersData } = useAdminGetPartners({ page, limit: PAGE_SIZE });
  const { data: logsData } = useAdminGetDataAccessLogs({ page, limit: PAGE_SIZE });
  const { data: reportsData } = useAdminGetReports({ page, limit: PAGE_SIZE });
  ...
  const partners = Array.isArray(partnersData)
    ? partnersData
    : (partnersData?.partners ?? partnersData?.items ?? []);
  const logs = Array.isArray(logsData) ? logsData : (logsData?.logs ?? logsData?.items ?? []);
  const reports = Array.isArray(reportsData)
    ? reportsData
    : (reportsData?.reports ?? reportsData?.items ?? []);
  const partnerTotal =
    (partnersData?.total ?? partnersData?.totalPages) ? partnersData?.total : partners.length;
  const logsTotal = (logsData?.total ?? logsData?.totalPages) ? logsData?.total : logs.length;
  const reportsTotal =
    (reportsData?.total ?? reportsData?.totalPages) ? reportsData?.total : reports.length;
    ...
    <div className="space-y-6 animate-fade-in">
      ...
      {activeTab === 'partners' && (
        <div className="space-y-4">
                    ...
                    <div>
                      ...
                      <div className="flex items-center gap-3 mt-1">
                        <span className="text-[10px] text-gray-400">
                          API: {partner.apiKey?.slice(0, 8)}...
                        </span>
                        <span className="text-[10px] text-gray-400">
                          {partner.requestCount ?? 0} requêtes
                        </span>
                      </div>
                    </div>
                  </div>
          ...
          {partners.length > 0 && Math.ceil(partners.length / PAGE_SIZE) > 1 && (
            <div className="flex items-center justify-center gap-2">
              ...
              {Array.from(
                { length: Math.min(Math.ceil(partners.length / PAGE_SIZE), 10) },
                (_, i) => i + 1
              ).map((p) => (
                <button
                  key={p}
              ...
              <button
                onClick={() =>
                  setPage((p) => Math.min(Math.ceil(partners.length / PAGE_SIZE), p + 1))
                }
                disabled={page === Math.ceil(partners.length / PAGE_SIZE)}
                className="flex items-center gap-1 px-3 py-1.5 text-sm rounded-lg border border-gray-200 dark:border-gray-700 bg-white dark:bg-gray-800 text-gray-700 dark:text-gray-300 disabled:opacity-40 hover:bg-gray-50 dark:hover:bg-gray-700 transition-colors"
              >
                Suivant
      ...
      {activeTab === 'logs' && (
        <div className="space-y-4">
                  ...
                  {logs.map((log: any) => (
                    ...
                    >
                      ...
                      <td className="py-2.5 text-gray-500">
                        {log.business?.name || log.businessId?.slice(0, 8) || '-'}
                      </td>
                      <td className="py-2.5">
                        <span
          ...
          {logs.length > 0 && Math.ceil(logs.length / PAGE_SIZE) > 1 && (
            <div className="flex items-center justify-center gap-2">
              <button
                onClick={() => setPage((p) => Math.max(1, p - 1))}
                disabled={page === 1}
              ...
              {Array.from(
                { length: Math.min(Math.ceil(logs.length / PAGE_SIZE), 10) },
                (_, i) => i + 1
              ).map((p) => (
                <button
                  key={p}
      ...
      {activeTab === 'reports' && (
        <div className="space-y-4">
          ...
          {reports.length > 0 && Math.ceil(reports.length / PAGE_SIZE) > 1 && (
            <div className="flex items-center justify-center gap-2">
              <button
                onClick={() => setPage((p) => Math.max(1, p - 1))}
                disabled={page === 1}
              ...
              {Array.from(
                { length: Math.min(Math.ceil(reports.length / PAGE_SIZE), 10) },
                (_, i) => i + 1
              ).map((p) => (
                <button
                  key={p}
              ...
              <button
                onClick={() =>
                  setPage((p) => Math.min(Math.ceil(reports.length / PAGE_SIZE), p + 1))
                }
