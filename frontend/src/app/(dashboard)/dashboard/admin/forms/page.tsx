...
export default function AdminFormsPage() {
  ...
  const createMutation = useMutation({
    mutationFn: (data: TemplateForm) =>
      apiClient.post('/admin/forms/templates', {
        ...data,
  ...
  const updateMutation = useMutation({
    mutationFn: ({ id, data }: { id: string; data: Partial<TemplateForm> }) =>
      apiClient.put(`/admin/forms/templates/${id}`, {
        ...data,
  ...
  const confirmDeleteTemplate = async () => {
    ...
    try {
      await deleteMutation.mutateAsync(deleteTemplateTarget.id);
  ...
  const handleActivate = async (id: string, activate: boolean) => {
    try {
      ...
      setToast({
        message: `Template ${activate ? 'activé' : 'désactivé'} avec succès`,
        type: 'success',
      });
    } catch {
    ...
    <div className="space-y-6 animate-fade-in">
              ...
              <div>
                            ...
                            <div className="flex items-center gap-1.5">
                              {tpl.status === 'ACTIVE' ? (
                                <Button
                                  variant="ghost"
                                  size="xs"
                                  onClick={() => handleActivate(tpl.id, false)}
                                >
                                ...
                                <Button
                                  variant="ghost"
                                  size="xs"
                                  onClick={() => handleActivate(tpl.id, true)}
                                >
                ...
                {templateTotalPages > 1 && (
                  <div className="flex items-center justify-center gap-2 p-4 border-t border-gray-200 dark:border-gray-700">
                    ...
                    {Array.from({ length: Math.min(templateTotalPages, 10) }, (_, i) => i + 1).map(
                      (p) => (
                        <button
                          key={p}
                          onClick={() => setPage(p)}
                    ...
                    <button
                      onClick={() => setPage((p) => Math.min(templateTotalPages, p + 1))}
              ...
              <EmptyState
                ...
                description={
                  search
                    ...
                    : 'Créez votre premier template de formulaire.'
                }
                action={
                  !search ? (
        ...
        <>
          <div className="w-full sm:max-w-xs">
            ...
            <Select
              ...
              options={templateList.map((t) => ({ value: t.id, label: t.name }))}
            />
          </div>
      ...
      >
        ...
        <ConfirmationModal
          ...
          description={
            deleteTemplateTarget ? `Supprimer le template « ${deleteTemplateTarget.name} » ?` : ''
          }
