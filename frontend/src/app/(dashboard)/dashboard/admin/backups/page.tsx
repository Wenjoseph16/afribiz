...
export default function AdminBackupsPage() {
  ...
  const { data, isLoading } = useQuery({
    ...
    queryFn: async () => {
      const res = await apiClient.adminGetBackups({ page, limit });
  ...
  const runBackupMutation = useMutation({
    mutationFn: () => apiClient.adminCreateBackup(),
    onSuccess: () => {
      qc.invalidateQueries({ queryKey: ['admin', 'backups'] });
      setToast({ message: 'Sauvegarde lancée avec succès', type: 'success' });
    },
    onError: () => {
  ...
  const restoreMutation = useMutation({
    mutationFn: (id: string) => apiClient.adminRestoreBackup(id),
    onSuccess: () => {
      setShowRestoreConfirm(null);
      qc.invalidateQueries({ queryKey: ['admin', 'backups'] });
      setToast({ message: 'Restauration effectuée', type: 'success' });
    },
    onError: () => {
  ...
  const toggleAutoBackupMutation = useMutation({
    mutationFn: (enabled: boolean) => apiClient.adminToggleAutoBackup(enabled),
    onSuccess: () => {
      qc.invalidateQueries({ queryKey: ['admin', 'backups'] });
      setToast({ message: 'Sauvegarde automatique mise à jour', type: 'success' });
    },
    onError: () => {
  ...
  const downloadMutation = useMutation({
    mutationFn: (id: string) => apiClient.adminDownloadBackup(id),
    onSuccess: () => {
      setToast({ message: 'Téléchargement démarré', type: 'success' });
    },
    onError: () => {
      setToast({ message: 'Erreur de téléchargement', type: 'error' });
    },
  });
    ...
    <div className="space-y-6 animate-fade-in">
      ...
      <div className="flex flex-col sm:flex-row items-start justify-between gap-4">
        <div>
          ...
          <p className="text-sm text-gray-500 dark:text-gray-400 mt-1">
            Gestion des sauvegardes de la plateforme
          </p>
        </div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-sm font-semibold text-gray-900 dark:text-gray-100">
                {status.lastBackup ? new Date(status.lastBackup).toLocaleString('fr-FR') : '-'}
              </p>
            </div>
          </div>
        </Card>
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-sm font-semibold">
                <span className={status.autoBackup ? 'text-emerald-600' : 'text-gray-400'}>
                  {status.autoBackup ? 'Activée' : 'Désactivée'}
                </span>
              </p>
            </div>
          </div>
        ...
        <Card padding="sm">
          <div className="flex items-center gap-3">
            <div
              className={`p-2 rounded-lg ${status.integrity === 'OK' ? 'bg-emerald-50 dark:bg-emerald-900/30 text-emerald-600' : 'bg-red-50 dark:bg-red-900/30 text-red-600'}`}
            >
              {status.integrity === 'OK' ? (
                <CheckCircle className="h-4 w-4" />
              ) : (
      ...
      <Card padding="md">
        <div className="flex flex-wrap items-center gap-4">
          <label className="flex items-center gap-3 cursor-pointer">
            ...
            >
              <span
                className={`inline-block h-4 w-4 transform rounded-full bg-white transition-transform ${status.autoBackup ? 'translate-x-6' : 'translate-x-1'}`}
              />
            </button>
      ...
      <Card padding="none">
        {isLoading ? (
                ...
                {history.map((b: any) => (
                  ...
                  >
                    <td className="p-4 text-xs text-gray-900 dark:text-gray-100">
                      {b.createdAt || b.date
                        ? new Date(b.createdAt || b.date).toLocaleString('fr-FR')
                        : '-'}
                    </td>
                    <td className="p-4 text-xs text-gray-500">
                      {b.size ? `${(b.size / (1024 * 1024)).toFixed(2)} Mo` : '-'}
                    </td>
                    <td className="p-4">
                      <span
                    ...
                    <td className="p-4">
                      <div className="flex items-center gap-1">
                        <Button
                          variant="ghost"
                          size="xs"
                          onClick={() => setShowRestoreConfirm(b.id)}
                        >
          ...
          <EmptyState
            ...
            description="Lancer une sauvegarde pour voir l'historique."
          />
        )}
      </Card>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 backdrop-blur-sm">
          <Card className="w-full max-w-sm mx-4">
            ...
            <p className="text-sm text-gray-500 dark:text-gray-400 mb-6">
              Êtes-vous sûr de vouloir restaurer cette sauvegarde ? Cette action peut entraîner une
