...
export default function AdminApiKeysPage() {
  ...
  const confirmDeleteApiKey = async () => {
    ...
    await deleteMutation.mutateAsync(deleteApiKeyTarget);
  ...
  const regenerateMutation = useMutation({
    ...
    onSuccess: () => {
      ...
      setToast({ message: 'Clé régénérée avec succès', type: 'success' });
    },
    onError: () => {
      setToast({ message: 'Erreur lors de la régénération', type: 'error' });
    },
  });
  ...
  const suspendMutation = useMutation({
    ...
    onSuccess: () => {
      qc.invalidateQueries({ queryKey: ['admin', 'api-keys'] });
      setToast({ message: 'Clé suspendue', type: 'success' });
    },
    onError: () => {
  ...
  const activateMutation = useMutation({
    ...
    onSuccess: () => {
      qc.invalidateQueries({ queryKey: ['admin', 'api-keys'] });
      setToast({ message: 'Clé activée', type: 'success' });
    },
    onError: () => {
      setToast({ message: "Erreur lors de l'activation", type: 'error' });
    },
  });
  ...
  const createMutation = useMutation({
    ...
    onSuccess: () => {
      ...
      setNewKey({ partnerName: '', type: 'READ' });
      setToast({ message: 'Clé API créée', type: 'success' });
    },
    onError: () => {
  ...
  return (
    <div className="space-y-6 animate-fade-in">
      ...
      <div className="flex flex-col sm:flex-row items-start justify-between gap-4">
        <div>
          ...
          <p className="text-sm text-gray-500 dark:text-gray-400 mt-1">
            Gestion des clés d&apos;accès API partenaires
          </p>
        </div>
      ...
      <Card padding="none">
        {isLoading ? (
                  ...
                  >
                    ...
                    <td className="p-4">
                      <span className="font-mono text-xs text-gray-500">
                        {k.apiKey ? `${k.apiKey.slice(0, 12)}...${k.apiKey.slice(-4)}` : '-'}
                      </span>
                    </td>
                    <td className="p-4">
                      <span
                    ...
                    <td className="p-4 text-xs text-gray-500">
                      {k.usedQuota ?? 0}/{k.quota ?? '-'}
                    </td>
                    <td className="p-4 text-xs text-gray-500">
                      {k.lastCall ? new Date(k.lastCall).toLocaleString('fr-FR') : '-'}
                    </td>
                    <td className="p-4">
                      <div className="flex items-center gap-1">
                        ...
                        {k.status === 'ACTIVE' ? (
                          <Button
                            variant="ghost"
                            size="xs"
                            onClick={() => suspendMutation.mutate(k.id)}
                          >
                          ...
                          <Button
                            variant="ghost"
                            size="xs"
                            onClick={() => activateMutation.mutate(k.id)}
                          >
                        ...
                        <Button
                          variant="ghost"
                          size="xs"
                          onClick={() => setDeleteApiKeyTarget(k.id)}
                        >
          ...
          <EmptyState
            ...
            description="Les clés API partenaires apparaîtront ici."
          />
        )}
      </Card>
      ...
      <ConfirmationModal
        ...
        onClose={() => setDeleteApiKeyTarget(null)}
