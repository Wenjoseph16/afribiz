...
import { Percent, Plus, Edit3, Trash2, Shield } from 'lucide-react';
...
function Toggle({ enabled, onChange }: { enabled: boolean; onChange: (value: boolean) => void }) {
  return (
    ...
    >
      <span
        className={`inline-block h-3.5 w-3.5 transform rounded-full bg-white transition-transform ${enabled ? 'translate-x-[18px]' : 'translate-x-0.5'}`}
      />
    </button>
  );
}
  ...
  return useQuery({
    ...
    queryFn: async () => {
      try {
        ...
        console.warn('Erreur chargement commissions:', error);
        return [];
      }
    },
    retry: false,
...
export default function AdminCommissionsPage() {
  ...
  const createMutation = useMutation({
    ...
    onSuccess: () => {
      ...
      setModal({ open: false });
      setToast({ message: 'Commission créée', type: 'success' });
    },
  ...
  const updateMutation = useMutation({
    mutationFn: ({ id, data }: { id: string; data: any }) =>
      apiClient.put(`/admin/commissions/${id}`, data),
    onSuccess: () => {
      qc.invalidateQueries({ queryKey: ['admin', 'commissions'] });
      setModal({ open: false });
      setToast({ message: 'Commission mise à jour', type: 'success' });
    },
  ...
  const deleteMutation = useMutation({
    ...
    onSuccess: () => {
      qc.invalidateQueries({ queryKey: ['admin', 'commissions'] });
      setToast({ message: 'Commission supprimée', type: 'success' });
    },
  ...
  const toggleMutation = useMutation({
    mutationFn: ({ id, isActive }: { id: string; isActive: boolean }) =>
      apiClient.put(`/admin/commissions/${id}`, { isActive }),
    onSuccess: () => {
      qc.invalidateQueries({ queryKey: ['admin', 'commissions'] });
  ...
  const [deleteCommissionTarget, setDeleteCommissionTarget] = useState<{
    id: string;
    label: string;
  } | null>(null);
  ...
  const confirmDeleteCommission = async () => {
    ...
    await deleteMutation.mutateAsync(deleteCommissionTarget.id);
    setDeleteCommissionTarget(null);
  };

  const handleDelete = (id: string, label: string) => {
    setDeleteCommissionTarget({ id, label });
  };
    ...
    <div className="space-y-6 animate-fade-in">
      ...
      <div className="flex flex-col sm:flex-row items-start justify-between gap-4">
        <div>
          ...
          <p className="text-sm text-gray-500 dark:text-gray-400 mt-1">
            Gérez les taux de commission de la plateforme
          </p>
        </div>
        <Button onClick={openCreate}>
          <Plus className="h-4 w-4" /> Nouvelle commission
        </Button>
      </div>
      ...
      {isLoading ? (
        ...
        <EmptyState
          ...
          description="Créez votre première règle de commission."
          action={
            <Button onClick={openCreate}>
                ...
                >
                  ...
                  <td className="p-3">
                    <Badge variant="info" size="xs">
                      {SCOPES.find((s) => s.value === c.scope)?.label || c.scope}
                    </Badge>
                  </td>
                  ...
                  <td className="p-3">
                    <Toggle
                      enabled={c.isActive}
                      onChange={(v) => toggleMutation.mutate({ id: c.id, isActive: v })}
                    />
                  </td>
                  <td className="p-3">
                    <div className="flex items-center gap-1">
                      ...
                      <Button variant="ghost" size="xs" onClick={() => handleDelete(c.id, c.label)}>
      ...
      <Modal
        ...
        title={modal.edit ? 'Modifier la commission' : 'Nouvelle commission'}
        size="lg"
      >
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
          <Input
            ...
            placeholder="ex: BUSINESS_COMMISSION"
          />
          <Input
            label="Libellé"
            value={form.label}
            onChange={(e) => setForm({ ...form, label: e.target.value })}
            placeholder="Commission business"
          />
          ...
          <Input
            ...
            value={form.minFee ?? ''}
            onChange={(e) =>
              setForm({ ...form, minFee: e.target.value ? Number(e.target.value) : null })
            }
          />
          <Input
            label="Frais max"
            ...
            value={form.maxFee ?? ''}
            onChange={(e) =>
              setForm({ ...form, maxFee: e.target.value ? Number(e.target.value) : null })
            }
          />
          <Input
            label="Devise"
        ...
        <ConfirmationModal
          ...
          description={
            deleteCommissionTarget
              ? `Supprimer la commission « ${deleteCommissionTarget.label} » ?`
              : ''
          }
