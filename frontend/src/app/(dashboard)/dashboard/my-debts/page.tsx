...
import { DollarSign, Search, Loader2, CreditCard } from 'lucide-react';
...
export default function MyDebtsPage() {
  ...
  const [payModal, setPayModal] = useState<{ open: boolean; debtId: string; remaining: number }>({
    open: false,
    debtId: '',
    remaining: 0,
  });
  ...
  const stats = useMemo(() => {
    ...
    return {
      ...
      active: all.filter(
        (d: any) => d.status === 'ACTIVE' || d.status === 'PARTIALLY_PAID' || d.status === 'OVERDUE'
      ).length,
  ...
  return (
    <div className="space-y-6">
      <PageHeader title="Mes dettes" description="Suivez et gérez vos dettes auprès des vendeurs" />
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
        ...
        <Card className="p-4">
          ...
          <p className="text-2xl font-bold text-amber-600">
            {stats.remaining.toLocaleString()} FCFA
          </p>
        </Card>
        <Card className="p-4">
                  ...
                  <div className="flex items-start gap-4">
                    ...
                    <div>
                      ...
                      <p className="text-sm text-gray-500 mt-1">
                        Réf: {debt.reference || debt.id.slice(0, 8)} ·{' '}
                        {debt.sourceType || 'Commande'}
                      </p>
                      {debt.dueDate && (
                  ...
                  <div className="flex flex-col items-end gap-2">
                    <div className="text-right">
                      ...
                      <p className="text-sm text-gray-500">
                        Payé: {Number(debt.amountPaid).toLocaleString()} · Reste:{' '}
      ...
      <Modal
        open={payModal.open}
        onClose={() => setPayModal({ open: false, debtId: '', remaining: 0 })}
      >
        <div className="p-6">
          ...
          <div className="space-y-4">
            <div>
              <label className="block text-sm font-medium mb-1">
                Montant (max: {payModal.remaining.toLocaleString()} FCFA)
              </label>
              <input
                type="number"
                min={1}
          ...
          <div className="flex justify-end gap-2 mt-6">
            <Button
              variant="ghost"
              onClick={() => setPayModal({ open: false, debtId: '', remaining: 0 })}
            >
              Annuler
            </Button>
            <Button
              ...
              disabled={
                !payAmount ||
                Number(payAmount) <= 0 ||
                Number(payAmount) > payModal.remaining ||
