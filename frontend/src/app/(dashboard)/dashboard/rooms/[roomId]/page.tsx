...
function isPopularRoom(r: any) {
  return (r._count?.bookings || 0) >= 10;
}
function isHighDemandRoom(r: any) {
  return (r._count?.bookings || 0) >= 5;
}
...
export default function RoomDetailPage() {
  ...
  const [activeTab, setActiveTab] = useState<'overview' | 'bookings' | 'stats' | 'history'>(
    'overview'
  );
  ...
  if (!r)
    return (
      <div className="flex items-center justify-center min-h-[400px]">
        <p className="text-gray-500">Chambre non trouvée</p>
      </div>
    );
        ...
        <div className="flex items-center gap-4">
          ...
          <div>
            <div className="flex items-center gap-2 flex-wrap">
              ...
              {r.roomNumber && (
                <span className="text-sm text-gray-400 font-mono">N° {r.roomNumber}</span>
              )}
              <span
                className={cn(
              ...
              {isHighDemandRoom(r) && !isPopularRoom(r) && (
            ...
            <p className="text-sm text-gray-500 dark:text-gray-400 mt-0.5">
              {ROOM_TYPES[r.type] || r.type} · {r.capacity} pers. · {r.beds} lit(s) ·{' '}
              {r.bathroom === 'PRIVATE'
                ? 'SDB privée'
                : r.bathroom === 'SHARED'
                  ? 'SDB partagée'
      ...
      <div className="grid grid-cols-2 sm:grid-cols-4 gap-4">
        <Card padding="sm" className="text-center">
          ...
          <p className="text-xs text-gray-500 mt-0.5">
            Par nuit
            {r.isPromotional && r.promotionalPrice && (
              <span className="text-red-500 ml-1">→ {fmtPrice(r.promotionalPrice)}</span>
            )}
          </p>
        </Card>
        <Card padding="sm" className="text-center">
        ...
        <div className="flex border-b border-gray-200 dark:border-gray-700">
          {[
            ...
            <button
              ...
              className={cn(
                'flex items-center gap-1.5 px-4 sm:px-6 py-3 text-sm font-medium border-b-2 transition-colors',
                activeTab === tab.id
            ...
            <div className="space-y-6">
              ...
              <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
                {r.priceWeekend && (
                  <div className="p-3 bg-gray-50 dark:bg-gray-800/50 rounded-xl">
                    ...
                    <p className="text-sm font-semibold mt-0.5">{fmtPrice(r.priceWeekend)}</p>
                  </div>
                )}
                {r.priceHighSeason && (
                  <div className="p-3 bg-gray-50 dark:bg-gray-800/50 rounded-xl">
                    <p className="text-xs text-gray-500">Haute saison</p>
                    <p className="text-sm font-semibold mt-0.5">{fmtPrice(r.priceHighSeason)}</p>
                  </div>
                )}
                {r.priceLowSeason && (
                  <div className="p-3 bg-gray-50 dark:bg-gray-800/50 rounded-xl">
                    <p className="text-xs text-gray-500">Basse saison</p>
                    <p className="text-sm font-semibold mt-0.5">{fmtPrice(r.priceLowSeason)}</p>
                  </div>
                )}
                {r.size && (
                  <div className="p-3 bg-gray-50 dark:bg-gray-800/50 rounded-xl">
                    <p className="text-xs text-gray-500">Surface</p>
                    <p className="text-sm font-semibold mt-0.5">{r.size} m²</p>
                  </div>
                )}
              </div>
              ...
              <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
                <div className="p-3 bg-gray-50 dark:bg-gray-800/50 rounded-xl">
                  ...
                  <p className="text-sm font-semibold mt-0.5">{r.checkInTime || '14:00'}</p>
                </div>
                <div className="p-3 bg-gray-50 dark:bg-gray-800/50 rounded-xl">
                  <p className="text-xs text-gray-500">
                    <Clock className="h-3 w-3 inline mr-1" />
                  ...
                  <p className="text-sm font-semibold mt-0.5">{r.checkOutTime || '12:00'}</p>
                </div>
                <div className="p-3 bg-gray-50 dark:bg-gray-800/50 rounded-xl">
                  <p className="text-xs text-gray-500">
                ...
                <div className="p-3 bg-gray-50 dark:bg-gray-800/50 rounded-xl">
                  ...
                  <p className="text-sm font-semibold mt-0.5">
                    {r.breakfastIncluded ? 'Inclus ✓' : 'Non inclus'}
                  </p>
                </div>
              </div>
                ...
                <div>
                  ...
                  <div className="flex flex-wrap gap-2">
                    {r.amenities.map((a: string) => (
                      <span
                        key={a}
                        className="inline-flex items-center gap-1 px-3 py-1.5 text-xs font-medium bg-gray-100 dark:bg-gray-700 text-gray-600 dark:text-gray-300 rounded-lg"
                      >
                ...
                <div className="p-4 bg-blue-50 dark:bg-blue-900/10 rounded-xl border border-blue-100">
                  ...
                  {r.seoTitle && (
                    <p className="text-sm text-gray-700">
                      <span className="text-gray-400">Titre:</span> {r.seoTitle}
                    </p>
                  )}
                  {r.seoDescription && (
                    <p className="text-sm text-gray-700 mt-0.5">
                      <span className="text-gray-400">Description:</span> {r.seoDescription}
                    </p>
                  )}
                </div>
              )}
          ...
          {activeTab === 'bookings' && (
            <div className="text-center py-8">
              ...
              <p className="text-xs text-gray-400 mt-1">
                {r._count?.bookings || 0} réservation(s) au total
              </p>
            </div>
          )}
          ...
          {activeTab === 'stats' && (
            <div className="grid grid-cols-2 sm:grid-cols-4 gap-4">
              <div className="text-center p-4 bg-brand/5 rounded-xl">
                <p className="text-2xl font-bold text-brand">{r._count?.bookings || 0}</p>
                <p className="text-xs text-gray-500 mt-0.5">Réservations</p>
              </div>
              <div className="text-center p-4 bg-emerald-50 rounded-xl">
                <p className="text-2xl font-bold text-emerald-600">{r.rating || '-'}</p>
                <p className="text-xs text-gray-500 mt-0.5">Note</p>
              </div>
              <div className="text-center p-4 bg-amber-50 rounded-xl">
                <p className="text-2xl font-bold text-amber-600">{r.capacity}</p>
                <p className="text-xs text-gray-500 mt-0.5">Capacité</p>
              </div>
              <div className="text-center p-4 bg-blue-50 rounded-xl">
                <p className="text-2xl font-bold text-blue-600">{r.beds}</p>
                <p className="text-xs text-gray-500 mt-0.5">Lits</p>
              </div>
            </div>
          )}
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center">
          ...
          <div className="relative bg-white dark:bg-gray-800 rounded-2xl shadow-xl border w-full max-w-md mx-4 p-6">
            <div className="flex items-center gap-3 mb-4">
              ...
              <div>
                <h2 className="text-lg font-bold">Supprimer cette chambre ?</h2>
                <p className="text-sm text-gray-500">Action irréversible</p>
              </div>
            </div>
            <p className="text-sm text-gray-600 mb-6">
              La chambre <strong>{r.name}</strong> sera supprimée définitivement.
            </p>
