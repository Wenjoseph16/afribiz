...
function getBadges(room: RoomItem): { label: string; className: string }[] {
  ...
  if (isPopular(room))
    badges.push({ label: '🔥 Populaire', className: 'bg-emerald-100 text-emerald-700' });
  if (isHighDemand(room) && !isPopular(room))
    badges.push({ label: '📈 Haute demande', className: 'bg-blue-100 text-blue-700' });
  if (isNewRoom(room))
    badges.push({ label: '🆕 Nouveau', className: 'bg-purple-100 text-purple-700' });
...
export default function RoomsPage() {
  ...
  const allRooms: RoomItem[] = Array.isArray(roomsData)
    ? roomsData
  ...
  const updatedTabs = TABS.map((t) => ({
    ...
    count:
      t.id === 'all'
        ? allRooms.length
        : t.id === 'available'
          ? allRooms.filter((r) => r.status === 'AVAILABLE' && r.isActive).length
          : t.id === 'occupied'
            ? allRooms.filter((r) => r.status === 'OCCUPIED').length
            : t.id === 'maintenance'
              ? allRooms.filter((r) => r.status === 'MAINTENANCE' || r.status === 'CLEANING').length
              : allRooms.filter((r) => !r.isActive).length,
  }));

  // Charts data
  const chartData: ModuleChartData = useMemo(() => {
    const available = allRooms.filter((r) => r.status === 'AVAILABLE' && r.isActive).length;
    const occupied = allRooms.filter((r) => r.status === 'OCCUPIED').length;
    const reserved = allRooms.filter((r) => r.status === 'RESERVED').length;
    const cleaning = allRooms.filter(
      (r) => r.status === 'CLEANING' || r.status === 'MAINTENANCE'
    ).length;
    const closed = allRooms.filter((r) => r.status === 'CLOSED' || r.status === 'BLOCKED').length;
    const inactive = allRooms.filter((r) => !r.isActive).length;
    return {
      distribution: [
        { name: 'Disponibles', value: available, color: '#10b981' },
        { name: 'Occupées', value: occupied, color: '#8b5cf6' },
        { name: 'Réservées', value: reserved, color: '#3b82f6' },
        { name: 'Maintenance', value: cleaning, color: '#f59e0b' },
        { name: 'Fermées', value: closed, color: '#6b7280' },
        { name: 'Inactives', value: inactive, color: '#ef4444' },
      ].filter((d) => d.value > 0),
      daily: [
        { label: 'Total', value: allRooms.length },
        { label: 'Réservations', value: allRooms.reduce((a, r) => a + (r.bookingCount || 0), 0) },
        {
          label: 'Note moy.',
          value: Math.round(
            (allRooms.reduce((a, r) => a + (r.rating || 0), 0) / (allRooms.length || 1)) * 10
          ),
        },
      ],
    };
  ...
  const filtered = useMemo(() => {
    ...
    if (activeTab === 'available') f = f.filter((r) => r.status === 'AVAILABLE' && r.isActive);
      ...
      f = f.filter((r) => r.status === 'MAINTENANCE' || r.status === 'CLEANING');
    ...
    if (searchQuery) {
      ...
      f = f.filter(
        (r) =>
          ...
          ROOM_TYPES[r.type]?.toLowerCase().includes(q)
      );
    }
    f.sort((a, b) => {
    ...
    <div className="space-y-6 animate-fade-in">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-4 gap-4">
        <StatsCard
          ...
          label="Total"
          value={stats.total || allRooms.length}
        />
        <StatsCard
      ...
      <ModuleCharts
        data={chartData}
        title="ANALYTICS LOGEMENTS"
        distributionLabel="Statut des chambres"
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
          {suggestions.map((s) => (
            <Link
              key={s.type}
              href={s.link}
              className="block p-4 rounded-2xl border transition-all hover:shadow-sm"
            ...
            >
              <div className="flex items-center gap-3">
                ...
                <span className="text-xs font-bold" style={{ color: s.countColor }}>
                  {s.count}
                </span>
              </div>
            </Link>
        ...
        <div className="flex flex-col sm:flex-row items-start sm:items-center gap-3">
          ...
          <div className="flex items-center gap-2">
            ...
            <button
              ...
              className="p-2 rounded-xl border border-gray-200 dark:border-gray-700 hover:bg-gray-50 dark:hover:bg-gray-700"
            >
              <ArrowUpDown
                className={cn('h-4 w-4 text-gray-500', sortOrder === 'desc' && 'rotate-180')}
              />
            </button>
            <div className="flex border border-gray-200 dark:border-gray-700 rounded-xl overflow-hidden">
              <button
                ...
                className={cn(
                  'p-2',
                  viewMode === 'grid' ? 'bg-brand text-white' : 'hover:bg-gray-50 text-gray-500'
                )}
              >
              ...
              <button
                ...
                className={cn(
                  'p-2',
                  viewMode === 'list' ? 'bg-brand text-white' : 'hover:bg-gray-50 text-gray-500'
                )}
              >
...
function RoomCard({ room }: { room: RoomItem }) {
  ...
  return (
    <Link
      ...
      className="group bg-white dark:bg-gray-800 rounded-2xl border border-gray-200 dark:border-gray-700 overflow-hidden hover:border-brand/30 hover:shadow-sm transition-all"
    >
      <div className="aspect-[4/3] bg-gradient-to-br from-brand/5 to-blue-50 dark:from-brand/10 dark:to-blue-900/10 flex flex-col items-center justify-center relative">
        ...
        {room.roomNumber && (
          <span className="text-xs font-mono text-gray-400">N° {room.roomNumber}</span>
        )}
        {badges.length > 0 && (
      ...
      <div className="p-4">
        ...
        <p className="text-xs text-gray-500 mt-0.5">
          {ROOM_TYPES[room.type] || room.type} · {room.capacity} pers.
        </p>
  ...
  return (
    <tr className="hover:bg-gray-50 dark:hover:bg-gray-700/30 group">
        ...
        <Link href={`/dashboard/rooms/${room.id}`} className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-xs text-gray-500">
              {room.roomNumber ? `N° ${room.roomNumber} · ` : ''}
              {room.rating || '-'} ★
            </p>
            {badges.length > 0 && (
      ...
      <td className="p-4 text-right text-sm font-semibold">
        {(room.price || 0).toLocaleString()} FCFA
      </td>
      ...
      <td className="p-4 text-right">
        <span
          className={cn(
            ...
            room.isActive ? 'text-gray-700' : 'text-gray-400'
          )}
        >
      ...
      <td className="p-4">
        <div className="flex items-center gap-1 opacity-0 group-hover:opacity-100">
          <Link
            href={`/dashboard/rooms/${room.id}`}
            className="p-1.5 rounded-lg hover:bg-gray-100 text-gray-400 hover:text-brand"
          >
