...
export default function PlanningPage() {
  ...
  const getRoomStatusForDate = (
    roomId: string,
    dateStr: string
  ): { status: string; label: string } => {
    const block = blocks.find(
      (b: any) => b.roomId === roomId && dateStr >= b.startDate && dateStr <= b.endDate
    );
    if (block)
      return {
        status: block.reason,
        label: BLOCK_REASONS.find((r) => r.value === block.reason)?.label || block.reason,
      };
    const booking = events.find(
      (b: any) => b.roomId === roomId && dateStr >= b.checkIn && dateStr <= b.checkOut
    );
    if (booking) return { status: 'RESERVED', label: booking.clientName || 'Réservé' };
    const room = rooms.find((r) => r.id === roomId);
    return {
      status:
        room?.status === 'MAINTENANCE' || room?.status === 'BLOCKED' ? room.status : 'AVAILABLE',
      label: '',
    };
  };
    ...
    <div className="space-y-6 animate-fade-in">
      ...
      <div className="grid grid-cols-2 sm:grid-cols-6 gap-3">
        ...
        <Card className="p-3 text-center border-blue-200 dark:border-blue-800">
          <p className="text-xs text-blue-600 font-medium">Réservées</p>
          <p className="text-lg font-bold text-blue-600">{stats.reserved}</p>
        </Card>
        <Card className="p-3 text-center border-purple-200 dark:border-purple-800">
          <p className="text-xs text-purple-600 font-medium">Occupées</p>
          <p className="text-lg font-bold text-purple-600">{stats.occupied}</p>
        </Card>
        ...
        <Card className="p-3 text-center border-red-200 dark:border-red-800">
          <p className="text-xs text-red-600 font-medium">Bloquées</p>
      ...
      <div className="flex items-center justify-between bg-white dark:bg-gray-800 rounded-2xl border border-gray-200 dark:border-gray-700 p-4">
        <div className="flex items-center gap-3">
          ...
          <button
            onClick={goToday}
            className="px-3 py-1.5 text-xs font-medium bg-brand text-white rounded-lg hover:bg-brand-700 transition-colors"
          >
            Aujourd'hui
          </button>
        </div>
      ...
      <div className="bg-white dark:bg-gray-800 rounded-2xl border border-gray-200 dark:border-gray-700 overflow-hidden">
        <div className="grid grid-cols-[200px_repeat(31,minmax(28px,1fr))] overflow-x-auto scrollbar-hide">
          ...
          {days.map((d, i) => (
            <div
              key={i}
              className={cn(
                'border-b border-r border-gray-100 dark:border-gray-700 p-1.5 text-center',
                isWeekend(year, month, d) && 'bg-gray-50 dark:bg-gray-700/30'
              )}
            >
              {d && (
                <div
        ...
        {rooms.map((room) => (
          <div
            key={room.id}
            className="grid grid-cols-[200px_repeat(31,minmax(28px,1fr))] overflow-x-auto scrollbar-hide border-t border-gray-100 dark:border-gray-700"
          >
            ...
            {days.map((d, i) => {
              if (!d)
                return (
                  <div key={i} className="border-r border-b border-gray-100 dark:border-gray-700" />
                );
              ...
              return (
                <button
                  key={i}
                  onClick={() => handleCellClick(room.id, d)}
                  className={cn(
                    'border-r border-b border-gray-100 dark:border-gray-700 p-0.5 relative group cursor-pointer transition-all duration-100 hover:ring-2 hover:ring-brand/30 hover:z-10',
                    style?.bg || 'bg-white dark:bg-gray-800',
                    isWeekend(year, month, d) && 'bg-opacity-70',
                    selectedCell?.roomId === room.id &&
                ...
                >
                  {cellInfo.status !== 'AVAILABLE' && (
                    <div className="absolute inset-0.5 rounded flex items-center justify-center">
                      {cellInfo.status === 'RESERVED' && (
                        <div className="w-full h-full bg-blue-200/60 dark:bg-blue-500/20 rounded" />
                      )}
                      {cellInfo.status === 'OCCUPIED' && (
                        <div className="w-full h-full bg-purple-200/60 dark:bg-purple-500/20 rounded" />
                      )}
                      {cellInfo.status === 'CLEANING' && (
                        <div className="w-1.5 h-1.5 bg-amber-400 rounded-full mx-auto" />
                      )}
                      {cellInfo.status === 'MAINTENANCE' && (
                        <Wrench className="h-3 w-3 text-orange-500 mx-auto" />
                      )}
                      {cellInfo.status === 'BLOCKED' && (
                        <Ban className="h-3 w-3 text-red-500 mx-auto" />
                      )}
                      {cellInfo.status === 'CLOSED' && (
      ...
      {showBlockModal && (
        ...
        >
          <div
            className="bg-white dark:bg-gray-800 rounded-2xl shadow-2xl border border-gray-200 dark:border-gray-700 w-full max-w-md p-6 space-y-5"
            ...
            <form onSubmit={handleBlockSubmit} className="space-y-4">
              ...
              <div>
                ...
                <textarea
                  ...
                  className="w-full px-4 py-2 border border-gray-300 dark:border-gray-700 rounded-xl focus:ring-2 focus:ring-brand/20 focus:border-brand outline-none resize-none bg-transparent dark:text-gray-100 text-sm"
                  rows={3}
                />
              </div>
