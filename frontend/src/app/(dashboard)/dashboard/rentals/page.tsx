    ...
    <div className="space-y-6 animate-fade-in">
      ...
      {allRentals.length > 0 &&
        (() => {
          ...
          const lowStock = allRentals.filter(
            (r) => r.isActive && r.availableQty < r.quantity && r.availableQty <= 2
          );
          const inactive = allRentals.filter((r) => !r.isActive);
          const highDemand = allRentals.filter(
            (r) => r.isActive && r.availableQty < r.quantity / 2
          );
        ...
        <Card className="text-center py-12">
          ...
          <p className="text-sm text-gray-500 dark:text-gray-400 mb-4">
            {searchQuery
              ...
              : 'Ajoutez votre premier article en location'}
          </p>
          <Link href="/dashboard/rentals/new">
            <Button>
              <Plus className="h-4 w-4 mr-1.5" />
...
function getBadge(rental: RentalItem): { label: string; class: string } | null {
  ...
  const isLowStock =
    rental.isActive && rental.availableQty <= 2 && rental.availableQty < rental.quantity;
    ...
    <tr className="hover:bg-gray-50 dark:hover:bg-gray-700/30 transition-colors group">
      ...
      <td className="p-4 text-center">
        <span className="text-sm text-gray-600 dark:text-gray-300">
          {rental.availableQty}/{rental.quantity}
        </span>
      </td>
      <td className="p-4 text-center">
        <span
