...
export default function BookingRemindersPage() {
  ...
  const [sentLog, setSentLog] = useState<
    { bookingId: string; channel: string; success: boolean }[]
  >([]);
  ...
  const pendingReminders = allBookings
    .filter((b: any) => {
  ...
  // Recently reminded
  const remindedBookings = allBookings
    .filter((b: any) => b.reminderSent)
    .sort(
      (a: any, b: any) =>
        new Date(b.remindedAt || b.updatedAt).getTime() -
        new Date(a.remindedAt || a.updatedAt).getTime()
    )
    .slice(0, 10);
      ...
      <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
        <Card className="p-3">
          <div className="flex items-center gap-2">
            ...
            <div>
              <p className="text-[10px] text-gray-500">À rappeler</p>
        ...
        <Card className="p-3">
          <div className="flex items-center gap-2">
            ...
            <div>
              <p className="text-[10px] text-gray-500">Rappelés</p>
              <p className="text-sm font-bold text-gray-900 dark:text-white">
                {remindedBookings.length}
              </p>
            </div>
          </div>
        </Card>
              ...
              <p className="text-sm font-bold text-gray-900 dark:text-white">
                {
                  allBookings.filter(
                    (b: any) =>
                      (b.startDate || '').startsWith(todayStr) &&
        ...
        <Card className="p-3">
          <div className="flex items-center gap-2">
            ...
            <div>
              ...
              <p className="text-sm font-bold text-gray-900 dark:text-white">
                {pendingReminders.filter((b: any) => b.customerPhone).length}
              </p>
            </div>
          </div>
        </Card>
            ...
            {pendingReminders.map((booking: any) => {
              ...
              return (
                <Card
                  ...
                  className={cn(
                    'p-3 sm:p-4',
                    isToday && 'ring-2 ring-amber-200 dark:ring-amber-800'
                  )}
                >
                    ...
                    <div className="flex-1 min-w-0">
                      <div className="flex items-center gap-2 flex-wrap">
                        <h3 className="text-sm font-semibold text-gray-900 dark:text-white">
                          {booking.title || booking.customerName || 'Réservation'}
                        </h3>
                        <Badge
                          variant={STATUS_CONFIG[booking.status]?.variant || 'warning'}
                          size="xs"
                        >
                        ...
                        {isTomorrow && (
                          <span className="text-[10px] font-medium text-blue-600 bg-blue-50 dark:bg-blue-900/20 px-1.5 py-0.5 rounded-full">
                            Demain
                          </span>
                        )}
                      </div>
                      <div className="flex flex-wrap items-center gap-3 mt-1 text-xs text-gray-500">
                        ...
                        {booking.customerPhone && (
                          <span className="text-brand">{booking.customerPhone}</span>
                        )}
                      </div>
                    </div>
                ...
                <div key={booking.id} className="p-3 flex items-center justify-between text-sm">
                  ...
                  <div className="text-right">
                    ...
                    <p className="text-[10px] font-medium text-gray-600 dark:text-gray-300">
                      {new Date(booking.remindedAt || booking.updatedAt).toLocaleDateString(
                        'fr-FR',
            ...
            {sentLog.map((log, i) => (
              <div key={i} className="flex items-center gap-2 text-xs">
                ...
                <span className="text-gray-600 dark:text-gray-400">
                  Rappel {log.success ? 'envoyé' : 'échoué'} via {log.channel}
                </span>
              </div>
            ))}
          </div>
