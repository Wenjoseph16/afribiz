...
export default function BookingStatsPage() {
  ...
  const allBookings = useMemo(() => {
    const raw = Array.isArray(bookingsData)
      ? bookingsData
  ...
  const stats = useMemo(() => {
    ...
    const revenueChart = Object.entries(last30Days).map(([date, revenue]) => ({
      date,
              ...
              >
                ...
                <div
                  className={cn(
                    ...
                    day.revenue > 0 ? 'bg-brand' : 'bg-gray-100 dark:bg-gray-800'
                  )}
                  style={{
                    height: `${Math.max((day.revenue / stats.maxRevenue) * 100, day.revenue > 0 ? 2 : 0)}%`,
                  }}
                />
                    ...
                    <div className="w-full h-2 bg-gray-100 dark:bg-gray-800 rounded-full overflow-hidden">
                      <div
                        className={cn(
                          ...
                          STATUS_COLORS[s.key] || 'bg-gray-400'
                        )}
                ...
                <div key={h.hour} className="flex items-center gap-2">
                  ...
                  <div className="flex-1 h-2 bg-gray-100 dark:bg-gray-800 rounded-full overflow-hidden">
                    <div
                      ...
                      style={{
                        width: `${(h.count / Math.max(...stats.hourDist.map((x) => x.count))) * 100}%`,
                      }}
                    />
                  </div>
                  <span className="text-[10px] font-medium text-gray-900 dark:text-white w-6 text-right">
                    {h.count}
                  </span>
                </div>
              ))}
            {stats.hourDist.filter((h) => h.count > 0).length === 0 && (
              ...
              >
                ...
                <div className="text-right">
                  ...
                  <p className="text-[9px] text-gray-400">
                    {formatPrice(Math.round(c.revenue / c.count))}
                  </p>
                </div>
              </div>
            ))}
      ...
      <Card className="p-4">
        <div className="flex flex-wrap items-center gap-6 text-sm">
          ...
          <div>
            ...
            <span className="font-semibold text-rose-600">{stats.noShow}</span>
          </div>
          <div>
            <span className="text-gray-500">Taux complétion :</span>{' '}
