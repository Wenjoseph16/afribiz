...
export default function ResourcesPage() {
  ...
  const openCreate = () => {
    ...
    setForm({ name: '', type: 'ROOM', capacity: 1, description: '' });
    setModalOpen(true);
  };
  ...
  const handleSave = async () => {
    ...
    try {
      if (editResource) {
        await apiClient.put(`${RESOURCE_URL}/${editResource.id}`, {
          name: form.name,
          type: form.type,
        ...
        await apiClient.post(RESOURCE_URL, {
          name: form.name,
          type: form.type,
          capacity: Number(form.capacity),
          description: form.description || undefined,
    ...
    <div className="animate-fade-in space-y-6">
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-4">
          ...
          <div>
            ...
            <p className="text-sm text-gray-500">Salles, employés, équipements...</p>
          </div>
        </div>
      ...
      <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
        {[
          ...
          { label: 'Salles', value: resources.filter((r: any) => r.type === 'ROOM').length },
          { label: 'Employés', value: resources.filter((r: any) => r.type === 'EMPLOYEE').length },
          {
            label: 'Équipements',
            value: resources.filter((r: any) => r.type === 'EQUIPMENT').length,
          },
        ].map((s) => (
              ...
              <div className="flex items-start gap-3">
                ...
                <div className="flex-1 min-w-0">
                  ...
                  <div className="flex items-center gap-3 mt-2 text-xs text-gray-500">
                    ...
                    {res._count && (
                      <span>{res._count.bookings || res.bookingsCount || 0} résa.</span>
                    )}
                  </div>
                </div>
      ...
      <Modal
        ...
        title={editResource ? 'Modifier la ressource' : 'Nouvelle ressource'}
      >
        <div className="space-y-4">
          <Input
            ...
            placeholder="Ex: Salle de réunion, Jean..."
          />
          <div>
            ...
            >
              {RESOURCE_TYPES.filter((t) => t !== 'Tous').map((t) => (
