...
export default function NewBookingPage() {
  ...
  const resources = Array.isArray(resourcesData)
    ? resourcesData
    : resourcesData?.items || resourcesData?.data || [];
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const startDateTime =
        startDate && startTime ? new Date(`${startDate}T${startTime}`).toISOString() : undefined;
      const endDateTime =
        endDate && endTime ? new Date(`${endDate}T${endTime}`).toISOString() : undefined;
        ...
        <div className="lg:col-span-2 space-y-6">
          ...
          <Card className="p-6 space-y-4">
            <h2 className="font-semibold text-gray-900 dark:text-white flex items-center gap-2">
              <Calendar className="w-4 h-4 text-brand" /> Informations
            </h2>
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
              <div className="sm:col-span-2">
                ...
                <Input
                  ...
                  placeholder="Ex: Coupe + Barbe, Consultation médicale..."
                  required
                />
              </div>
              <div>
                ...
                <select
                  ...
                  onChange={(e) => {
                    ...
                    if (e.target.value !== 'ROOM') setRoomId('');
                  }}
                  ...
                  >
                    ...
                    {services.map((s: any) => (
                      <option key={s.id} value={s.id}>
                        {s.name} - {Number(s.price || 0).toLocaleString()} FCFA
                      </option>
                    ))}
                  </select>
                </div>
                  ...
                  >
                    ...
                    {rooms.map((r: any) => (
                      <option key={r.id} value={r.id}>
                        {r.name} - {Number(r.price || 0).toLocaleString()} FCFA
                      </option>
                    ))}
                  </select>
                </div>
          ...
          <Card className="p-6 space-y-4">
            <h2 className="font-semibold text-gray-900 dark:text-white flex items-center gap-2">
              <Clock className="w-4 h-4 text-brand" /> Date & heure
            </h2>
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
              <div>
          ...
          <Card className="p-6 space-y-4">
            <h2 className="font-semibold text-gray-900 dark:text-white flex items-center gap-2">
              <User className="w-4 h-4 text-brand" /> Client
            </h2>
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
              <div className="sm:col-span-2">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
              ...
              <div className="flex items-center gap-2 pt-6">
                <label className="flex items-center gap-2 cursor-pointer">
                  ...
                  />
                  <span className="text-sm text-gray-600 dark:text-gray-300">Acompte payé</span>
                </label>
              </div>
            </div>
          </Card>
          ...
          <Card className="p-6 space-y-4">
            ...
            <div>
              ...
              <textarea
                ...
                className="w-full px-4 py-2 border border-gray-200 dark:border-gray-700 rounded-xl resize-none bg-transparent dark:text-gray-100"
                rows={2}
                placeholder="Info pour le staff..."
              />
            </div>
          </Card>
        </div>
        ...
        <div className="space-y-6">
          <Card className="p-6 text-center">
            ...
            <p className="text-xs text-gray-500">
              Les dates sont pré-remplies avec la date et l'heure actuelles.
            </p>
          </Card>
          <Card className="p-6">
