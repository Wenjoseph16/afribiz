...
import {
  useHubPlatformStats,
  useHubSectorBenchmarks,
  useHubGeographicStats,
  useHubGrowthStats,
  useHubPaymentTrends,
  usePartnerReports,
  useOrderPartnerReport,
...
export default function DataHubPage() {
  ...
  const { data: businessesData } = usePartnerBusinesses(
    isPartner ? { q: searchQuery || undefined, limit: 20 } : undefined
  );
  ...
  const sectors = Array.isArray(sectorBenchmarks)
    ? sectorBenchmarks
    : (sectorBenchmarks?.sectors ?? []);
  ...
  return (
    <div className="space-y-6 animate-fade-in">
      <PageHeader
        title="Data Hub"
        description="Données agrégées et analyses du marché africain"
        breadcrumbs={[{ label: 'Data Hub' }]}
      />
      ...
      {activeTab === 'analytics' && (
        <div className="space-y-6">
          {/* Conversion Funnel */}
          <Card
            title="Entonnoir de conversion (30 jours)"
              ...
              <div className="space-y-6">
                ...
                <div className="grid grid-cols-2 sm:grid-cols-5 gap-3">
                  {[
                    {
                      label: 'Visite → Clic',
                      value: funnel.conversionRates?.viewToClick,
                      color: 'text-brand',
                    },
                    {
                      label: 'Clic → Panier',
                      value: funnel.conversionRates?.clickToCart,
                      color: 'text-purple-600',
                    },
                    {
                      label: 'Panier → Commande',
                      value: funnel.conversionRates?.cartToOrder,
                      color: 'text-emerald-600',
                    },
                    {
                      label: 'Commande → Paiement',
                      value: funnel.conversionRates?.orderToPayment,
                      color: 'text-blue-600',
                    },
                    {
                      label: 'Global',
                      value: funnel.conversionRates?.overall,
            ...
            {cohorts ? (
              Array.isArray(cohorts) && cohorts.length > 0 ? (
                  ...
                  <table className="w-full text-sm">
                    <thead>
                      <tr className="border-b border-gray-200 dark:border-gray-700">
                        ...
                        <th className="text-right py-2 px-4 font-medium text-gray-500 text-xs uppercase tracking-wider">
                          Nouveaux
                        </th>
                        <th className="text-right py-2 px-4 font-medium text-gray-500 text-xs uppercase tracking-wider">
                          J+7
                ...
                <EmptyState
                  ...
                  description="Les cohorts seront disponibles avec plus de clients."
                />
              )
            ) : (
              <Loader className="py-12" />
            ...
            {engagement
              ? [
                  ...
                  },
                  {
                    ...
                    value: engagement.activeClients,
                    icon: Activity,
                    color: 'text-emerald-600',
                  ...
                  },
                  {
                    ...
                    value: engagement.conversations30d,
                    icon: MessageCircle,
                    color: 'text-amber-600',
      ...
      {activeTab === 'copilot' && (
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
          ...
          <Card
            ...
            titleIcon={<HeartHandshake className="h-4 w-4" />}
            ...
            {tips ? (
              Array.isArray(tips.tips) && tips.tips.length > 0 ? (
                ...
                <EmptyState
                  ...
                  title="Tout va bien !"
                  description="Aucun conseil pour le moment."
                />
              )
            ) : (
              <Loader className="py-12" />
          ...
          {recommendations ? (
            Array.isArray(recommendations) && recommendations.length > 0 ? (
              ...
              <EmptyState
                ...
                description="Les recommandations apparaîtront quand les clients interagiront avec vos produits."
              />
            )
          ) : (
              ...
              <Card key={sector.sector || sector.id}>
                <div className="flex items-center justify-between mb-3">
                  ...
                  <span className="text-xs font-medium text-brand">
                    {sector.businessCount} businesses
                  </span>
                </div>
                <div className="space-y-2">
                  ...
                  <div className="flex justify-between text-xs">
                    ...
                    <span
                      className={`font-semibold ${sector.growth >= 0 ? 'text-emerald-600' : 'text-red-600'}`}
                    >
                      {sector.growth ?? 0}%
                    </span>
                  </div>
              ...
              {reports.map((report: any) => (
                <Card key={report.id}>
                  ...
                  <p className="text-sm font-semibold text-gray-900 dark:text-gray-100">
                    {report.title || `Rapport ${report.type}`}
                  </p>
                  {report.summary && (
                  ...
                  <div className="flex items-start gap-3">
                    ...
                    <div className="min-w-0 flex-1">
                      ...
                      <p className="text-[11px] text-gray-500">
                        {b.type || b.sector} · {b.country || b.city}
                      </p>
                      <div className="flex items-center gap-2 mt-1.5">
                        <span className="text-xs font-medium text-brand">
                          {b.afriScore ?? b.score ?? '-'} pts
                        </span>
                        {b.badges?.slice(0, 2).map((badge: string, i: number) => (
                          <span
                            key={i}
                            className="text-[10px] px-1.5 py-0.5 rounded bg-amber-50 dark:bg-amber-900/20 text-amber-700 dark:text-amber-400"
                          >
                            {badge}
                          </span>
                        ))}
            ...
            <EmptyState
              ...
              description={
                searchQuery
                  ...
                  : 'Les données des businesses seront disponibles sous condition de consentement.'
              }
            />
          )}
        </div>
