    ...
    (async () => {
      try {
        ...
        const data = Array.isArray(res.data.data)
          ? res.data.data
          : res.data.data?.logs || res.data.data?.activities || [];
        setLogs(data);
      } catch (err) {
        console.error(err);
      }
  ...
  const filtered = logs.filter((log: any) => {
    ...
    if (search) {
      ...
      return (
        ...
        (log.clientName || log.subscriberName || '').toLowerCase().includes(q)
      );
    }
    return true;
  });
  ...
  return (
    <div className="animate-fade-in space-y-6">
      ...
      <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
        <Card className="p-3">
          <Activity className="w-4 h-4 text-brand mb-1" />
          <p className="text-[10px] text-gray-500">Total activités</p>
          <p className="text-lg font-bold">{logs.length}</p>
        </Card>
        <Card className="p-3">
          <User className="w-4 h-4 text-blue-600 mb-1" />
          <p className="text-[10px] text-gray-500">Souscriptions</p>
          <p className="text-lg font-bold">
            {logs.filter((l: any) => l.action === 'SUBSCRIBED').length}
          </p>
        </Card>
        <Card className="p-3">
          <Repeat className="w-4 h-4 text-emerald-600 mb-1" />
          <p className="text-[10px] text-gray-500">Renouvellements</p>
          <p className="text-lg font-bold">
            {logs.filter((l: any) => l.action === 'RENEWED').length}
          </p>
        </Card>
        <Card className="p-3">
          <CreditCard className="w-4 h-4 text-amber-600 mb-1" />
          <p className="text-[10px] text-gray-500">Paiements</p>
          <p className="text-lg font-bold">
            {logs.filter((l: any) => l.action === 'PAYMENT').length}
          </p>
        </Card>
      </div>
                  ...
                  <div
                    className={cn(
                      ...
                      log.action === 'SUBSCRIBED'
                        ...
                        : log.action === 'RENEWED'
                          ? 'bg-blue-50'
                          : log.action === 'CANCELLED' || log.action === 'EXPIRED'
                            ? 'bg-red-50'
                            : 'bg-amber-50'
                    )}
                  >
                    ...
                    <p className="text-xs text-gray-500 mt-0.5 flex items-center gap-2">
                      ...
                      {log.amount && (
                        <Badge variant="info" size="xs">
                          {(log.amount || 0).toLocaleString()} FCFA
                        </Badge>
                      )}
                    </p>
                  </div>
