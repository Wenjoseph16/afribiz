...
export default function SubscribersPage() {
  ...
  const handleRenew = async (id: string) => {
    try {
      await apiClient.renewSubscription(id);
      refetch();
    } catch (err) {
      console.error(err);
    }
    ...
    <div className="animate-fade-in space-y-6">
      ...
      <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
        ...
        <Card className="p-3">
          ...
          <p className="text-[10px] text-gray-500">Expirés</p>
          <p className="text-lg font-bold text-amber-600">{stats.expired}</p>
        </Card>
        <Card className="p-3">
          <CreditCard className="w-4 h-4 text-blue-600 mb-1" />
          <p className="text-[10px] text-gray-500">Revenus</p>
          <p className="text-lg font-bold">{stats.revenue.toLocaleString()} FCFA</p>
        </Card>
      </div>
        ...
        <div className="space-y-2">
          {filtered.map((sub: any) => {
            ...
            const isExpiring =
              sub.status === 'ACTIVE' &&
              sub.endDate &&
              new Date(sub.endDate) < new Date(Date.now() + 7 * 24 * 60 * 60 * 1000);
            return (
              <Card key={sub.id} className="p-4">
                  ...
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      ...
                      <p className="text-xs text-gray-500">
                        {plan?.name || sub.planName || 'Abonnement'} —{' '}
                        {sub.clientEmail || sub.clientPhone || '—'}
                      </p>
                      {sub.endDate && (
                        <p
                          className={cn(
                            'text-[10px] mt-0.5 flex items-center gap-1',
                            isExpiring ? 'text-amber-500' : 'text-gray-400'
                          )}
                        >
                ...
                {sub.amount && (
                  <p className="text-xs text-gray-400 mt-2">
                    Montant : {sub.amount.toLocaleString()} FCFA
                  </p>
                )}
              </Card>
            );
