...
export default function SubscriptionPaymentsPage() {
  ...
  const filtered = payments.filter(
    (p: any) =>
      !search ||
      p.subscriberName?.toLowerCase().includes(search.toLowerCase()) ||
  ...
  const stats = useMemo(
    () => ({
      ...
      succeeded: payments.filter((p: any) => p.status === 'SUCCEEDED' || p.status === 'COMPLETED')
        .length,
    ...
    <div className="animate-fade-in space-y-6">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-sm text-gray-500">Historique des paiements récurrents</p>
        </div>
      ...
      <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
        <Card className="p-3">
          <CreditCard className="w-4 h-4 text-brand mb-1" />
        ...
        <Card className="p-3">
          ...
          <p className="text-lg font-bold text-emerald-600">{stats.succeeded}</p>
        </Card>
        <Card className="p-3">
          <XCircle className="w-4 h-4 text-red-600 mb-1" />
          <p className="text-[10px] text-gray-500">Échoués</p>
        ...
        <Card className="p-3">
          ...
          <p className="text-lg font-bold">{stats.total.toLocaleString()} FCFA</p>
        </Card>
      </div>
      ...
      {filtered.length === 0 ? (
        <Card className="text-center py-12">
          <CreditCard className="h-12 w-12 text-gray-200 dark:text-gray-700 mx-auto mb-3" />
                ...
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    <p className="text-sm font-semibold text-gray-900 dark:text-white">
                      {p.clientName || p.subscriberName || 'Client'}
                    </p>
            ...
            <div className="space-y-4">
              ...
              <div>
                ...
                <input
                  ...
                  value={form.amount}
                  onChange={(e) =>
                    setForm((f) => ({ ...f, amount: parseInt(e.target.value) || 0 }))
                  }
                  required
                  className="w-full p-2.5 text-sm border border-gray-200 dark:border-gray-700 rounded-xl bg-transparent dark:text-gray-100"
                />
