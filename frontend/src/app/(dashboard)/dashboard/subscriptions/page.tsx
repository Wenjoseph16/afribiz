...
export default function SubscriptionsPage() {
  ...
  const stats = statsData || {
    ...
    monthlyRevenue: plans.reduce(
      (a: number, p: any) => a + (p.price || 0) * (p.subscriberCount || 0),
      0
    ),
    churnRate: 0,
  };
      ...
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-gray-400 dark:text-white/30 text-sm mt-0.5">
            {plans.length} plan{plans.length > 1 ? 's' : ''} — {stats.activeSubscribers} abonné
            {stats.activeSubscribers > 1 ? 's' : ''}
          </p>
        </div>
              ...
              >
                <div className="relative rounded-[calc(1rem-0.1875rem)] bg-gradient-to-br from-white/[0.02] to-transparent p-5">
                  ...
                  <div className="relative">
                    ...
                    {plan.description && (
                      <p className="text-xs text-gray-400 dark:text-white/30 line-clamp-2 mb-3">
                        {plan.description}
                      </p>
                    )}
                    ...
                    {plan.features && plan.features.length > 0 && (
                      <div className="space-y-1 mb-3">
                        {plan.features.slice(0, 3).map((f: string, i: number) => (
                          <div
                            key={i}
                            className="flex items-center gap-1.5 text-xs text-gray-500 dark:text-white/40"
                          >
                        ...
                        {plan.features.length > 3 && (
                          <p className="text-[10px] text-gray-400 dark:text-white/20 pl-4">
                    ...
                    <div className="flex items-center justify-between pt-3 border-t border-white/5">
                      <div className="flex items-center gap-1 text-xs text-gray-400 dark:text-white/30">
                        ...
                        {plan.subscriberCount || 0} abonné
                        {(plan.subscriberCount || 0) !== 1 ? 's' : ''}
                      </div>
                      <div className="flex items-center gap-1 text-xs text-gray-400 dark:text-white/30">
