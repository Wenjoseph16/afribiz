...
export default function DisputesPage() {
  ...
  const statusBadge = (s: string) => {
    switch (s) {
      ...
      case 'EN_COURS':
        return (
          <Badge className="bg-amber-50 text-amber-700 dark:bg-amber-900/30 dark:text-amber-400">
            En cours
          </Badge>
        );
      case 'RESOLU':
  ...
  return (
    <div className="space-y-6 animate-fade-in">
      ...
      <CopilotTips moduleKey="DISPUTES" />
      ...
      <div className="grid grid-cols-2 sm:grid-cols-4 gap-4">
        ...
        <Card className="p-4">
          <p className="text-2xl font-bold text-emerald-600">{stats.resolu}</p>
      ...
      {filtered.length === 0 ? (
        <EmptyState
          ...
          title="Aucun litige"
          description="Vous n'avez pas de litige pour le moment"
        />
      ) : (
            ...
            <Card key={d.id} className="p-4 hover:border-brand/20 transition-all">
              <div className="flex items-start justify-between gap-4">
                <div className="flex items-start gap-3 flex-1">
                  <div
                    className={`p-2 rounded-xl ${d.status === 'OUVERT' ? 'bg-red-50' : d.status === 'EN_COURS' ? 'bg-amber-50' : 'bg-emerald-50'}`}
                  >
                    {d.status === 'RESOLU' ? (
                      <CheckCircle className="h-5 w-5 text-emerald-600" />
                    ) : (
                      <AlertTriangle
                        className={`h-5 w-5 ${d.status === 'OUVERT' ? 'text-red-600' : 'text-amber-600'}`}
                      />
                    )}
                  </div>
                  <div className="flex-1">
                    ...
                    {d.reference && (
                      <p className="text-xs text-gray-500 mt-0.5">Réf: {d.reference}</p>
                    )}
                    {d.description && (
