...
import { User, Plus, Pencil, Trash2, Loader, Phone, Truck, Save, X, Circle } from 'lucide-react';
...
export default function DeliveryDriversPage() {
  ...
  const [deleteDriverTarget, setDeleteDriverTarget] = useState<string | null>(null);
  ...
  return (
    <div className="space-y-6 animate-fade-in">
          ...
          {drivers.map((driver: any) => (
            <Card key={driver.id} padding="md" className="relative group">
              <div className="absolute top-3 right-3 flex items-center gap-1 opacity-0 group-hover:opacity-100 transition-opacity">
                ...
                <button
                  onClick={() => setDeleteDriverTarget(driver.id)}
              ...
              <div className="space-y-2 text-sm">
                ...
                <div className="flex items-center gap-2 text-gray-500">
                  ...
                  {driver.vehiclePlate && (
                    <span className="font-mono text-xs text-gray-400">({driver.vehiclePlate})</span>
                  )}
                </div>
              </div>
      ...
      {showModal && (
        ...
        >
          <div
            className="bg-white dark:bg-gray-800 rounded-3xl p-6 sm:p-8 max-w-md w-full shadow-2xl relative"
          ...
          >
            ...
            <p className="text-sm text-gray-500 dark:text-gray-400 mb-6">
              {editingId
                ...
                : 'Ajoutez un chauffeur à votre équipe'}
            </p>
      ...
      <ConfirmationModal
        ...
        onConfirm={async () => {
          if (deleteDriverTarget) {
            await deleteMutation.mutateAsync(deleteDriverTarget);
            setDeleteDriverTarget(null);
          }
        }}
        title="Désactiver ce chauffeur ?"
        description="Le chauffeur sera désactivé et ne pourra plus recevoir de livraisons."
