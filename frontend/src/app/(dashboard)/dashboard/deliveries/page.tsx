  ...
  const chartData: ModuleChartData = useMemo(
    () => ({
      distribution: [
        {
          name: 'En attente',
          value: allDeliveries.filter((d) => d.status === 'pending').length,
          color: '#f59e0b',
        },
        {
          name: 'En transit',
      ...
      daily: [
        { label: 'Total', value: allDeliveries.length },
        { label: 'Livrées', value: allDeliveries.filter((d) => d.status === 'delivered').length },
        {
          label: 'En cours',
          value: allDeliveries.filter((d) => d.status === 'pending' || d.status === 'in_transit')
            .length,
        },
      ],
    }),
          ...
          return (
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
              {suggestions.map((s: any, i) => (
                <Link
                  key={i}
                  href={`/dashboard/deliveries?tab=${s.type}`}
        ...
        <div className="flex items-center justify-between pt-1">
          ...
          <span
            className={cn(
              ...
              STATUS_COLORS[delivery.status] || 'text-gray-600 bg-gray-50'
            )}
          >
