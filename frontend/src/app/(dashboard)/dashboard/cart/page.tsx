    ...
    return (
      <div className="min-h-[60vh] flex items-center justify-center px-4">
        <motion.div initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} className="text-center max-w-md">
          ...
          <h2 className="text-2xl font-display font-bold text-white tracking-tight mb-2">Panier vide</h2>
          ...
          <Link href="/marketplace">
            <button className="group inline-flex items-center gap-2 px-8 py-4 rounded-full bg-gradient-to-r from-emerald-500 to-emerald-400 text-white font-bold text-base shadow-[0_0_30px_rgba(16,185,129,0.25)] hover:shadow-[0_0_50px_rgba(16,185,129,0.4)] transition-all duration-500 active:scale-[0.98]">
    ...
    <motion.div variants={containerVariants} initial="hidden" animate="visible" className="space-y-6 max-w-6xl mx-auto px-4 py-8">
      ...
      <motion.div variants={itemVariants} className="flex items-center justify-between">
        <div>
          ...
          <h1 className="text-3xl font-display font-bold text-white tracking-tight">Mon panier</h1>
          <p className="text-white/40 text-sm mt-1">{itemCount} article{itemCount > 1 ? 's' : ''} — {formatPrice(total, currency)}</p>
        </div>
                    ...
                    <div className="flex-1 min-w-0">
                      <div className="flex justify-between items-start gap-3">
                        <div className="min-w-0">
                          ...
                          <p className="text-xs text-white/30 mt-0.5">{item.businessName || 'Marchand AfriBiz'}</p>
                          <p className="text-xs text-emerald-400/70 mt-0.5 font-medium">{formatPrice(item.price, item.currency)} / unité</p>
                        </div>
                        <button onClick={() => handleRemove(item.productId, item.name)} className="shrink-0 w-8 h-8 rounded-xl flex items-center justify-center text-white/20 hover:text-red-400 hover:bg-red-400/10 transition-all active:scale-95">
                          <Trash2 className="w-4 h-4" />
                        </button>
                      </div>
                      <div className="flex items-center justify-between mt-3">
                        <div className="flex items-center gap-1">
                          <button onClick={() => updateQuantity(item.productId, item.quantity - 1)} disabled={item.quantity <= 1} className="w-8 h-8 rounded-xl bg-white/5 border border-white/10 flex items-center justify-center text-white/50 hover:text-white hover:bg-white/10 active:scale-95 transition-all disabled:opacity-30">
                            <Minus className="w-3.5 h-3.5" />
                          </button>
                          <span className="w-10 text-center text-sm font-bold text-white tabular-nums">{item.quantity}</span>
                          <button onClick={() => updateQuantity(item.productId, item.quantity + 1)} className="w-8 h-8 rounded-xl bg-white/5 border border-white/10 flex items-center justify-center text-white/50 hover:text-white hover:bg-emerald-500/20 active:scale-95 transition-all">
        ...
        <motion.div variants={itemVariants} className="space-y-4">
          ...
          <div className="glass rounded-2xl p-5 space-y-3">
            ...
            <div className="flex justify-between text-sm">
              <span className="text-white/40">Sous-total ({itemCount} article{itemCount > 1 ? 's' : ''})</span>
              <span className="text-white/70 font-medium tabular-nums">{formatPrice(total, currency)}</span>
            </div>
            <div className="flex justify-between text-sm">
            ...
            <div className="flex justify-between items-baseline pt-1">
              ...
              <span className="text-2xl font-bold text-white tabular-nums">{formatPrice(total + (total >= FREE_SHIPPING_THRESHOLD ? 0 : 2500), currency)}</span>
            </div>
          </div>
