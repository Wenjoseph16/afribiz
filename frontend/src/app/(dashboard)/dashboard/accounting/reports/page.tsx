      ...
      <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
        ...
        <div className="flex items-center gap-2">
          ...
          <Button
            ...
            size="sm"
            onClick={() =>
              window.open('/api/business/accounting/reports/export-csv?year=' + year, '_blank')
            }
          >
      ...
      {income && (
        <Card>
          <div className="flex items-center gap-2 mb-4">
            ...
            <h3 className="font-semibold">Compte de résultat {year}</h3>
          </div>
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-6">
            {[
              {
                label: 'Revenus',
                value: fmt(income.revenue?.totalRevenue || 0),
                color: 'text-emerald-600',
                bg: 'bg-emerald-50',
              },
              {
                label: 'Dépenses',
                value: fmt(income.expenses?.total || 0),
                color: 'text-red-600',
                bg: 'bg-red-50',
              },
              {
                label: 'Résultat net',
                value: fmt(income.netIncome || 0),
                color: income.netIncome >= 0 ? 'text-emerald-600' : 'text-red-600',
                bg: income.netIncome >= 0 ? 'bg-emerald-50' : 'bg-red-50',
              },
            ].map((s, i) => (
            ...
            <div className="mt-4">
              ...
              <div className="space-y-1.5">
                {income.expenses.byCategory.map((c: any) => (
                  <div
                    key={c.category}
                    className="flex items-center justify-between text-sm p-2 bg-gray-50 dark:bg-gray-800/50 rounded-lg"
                  >
      ...
      {bilan && (
        <Card>
          <div className="flex items-center gap-2 mb-4">
            ...
            <h3 className="font-semibold">Bilan comptable {year}</h3>
          </div>
            ...
            <div>
              ...
              <div className="space-y-2">
                {[
                  ...
                  {
                    label: 'Total passif',
                    value: fmt(bilan.liabilities?.totalLiabilities || 0),
                    bold: true,
                  },
                ].map((item, i) => (
                  <div
                ...
                <div className="pt-3 mt-3 border-t border-gray-200">
                  <div className="flex justify-between">
                    ...
                    <span className="text-sm font-bold text-emerald-600">
                      {fmt(bilan.equity?.totalEquity || 0)}
                    </span>
                  </div>
                </div>
              </div>
