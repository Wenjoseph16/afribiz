...
import { MessageCircle, Search, Code, Plus, X, ArrowRight, Building2 } from 'lucide-react';
...
export default function DeveloperMessagesPage() {
  ...
  // Hook socket conversation réutilisable
  ...
  const startConversation = (recipient: any) => {
    ...
    createConversation.mutate(
      {
        recipientId: ownerId,
        initialMessage: `Bonjour ${recipient.owner?.firstName || ''}, je vous contacte depuis AfriBiz (espace développeur).`,
      },
      {
        onSuccess: (data: any) => {
  ...
  const filteredConvs = conversations.filter(
    (c: any) =>
      ...
      c.otherUser?.firstName?.toLowerCase().includes(search.toLowerCase())
  );
  ...
  return (
    <div className="animate-fade-in">
      <PageHeader
        title="Messagerie développeur"
        description="Échangez avec les business, le support et la communauté"
        breadcrumbs={[
          ...
          <div className="flex-1 overflow-y-auto">
            {isLoading ? (
              ...
              <EmptyState
                ...
                description="Contactez les business et le support."
              />
            ) : (
              filteredConvs.map((conv: any) => {
                const displayName = conv.businessName || conv.otherUser?.firstName || 'Support';
                return (
                  <button
                    key={conv.id}
                  ...
                  >
                    <div className="w-10 h-10 rounded-full bg-gradient-to-br from-purple-500 to-blue-500 flex items-center justify-center text-white font-bold shrink-0">
                      {conv.businessName?.charAt(0) || <Code className="h-4 w-4" />}
                    </div>
                    ...
                    {conv.unreadCount > 0 && (
                      <div className="w-5 h-5 rounded-full bg-purple-500 text-white text-[10px] font-bold flex items-center justify-center shrink-0">
          ...
          {selectedConv ? (
                ...
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    <p className="text-sm font-semibold text-gray-900 dark:text-gray-100">
                      {selectedConv.businessName || selectedConv.otherUser?.firstName || 'Support'}
                    </p>
            ...
            <div className="flex-1 flex items-center justify-center">
              <div className="text-center">
                <Code className="h-16 w-16 text-gray-200 dark:text-gray-700 mx-auto mb-4" />
                <h3 className="text-lg font-semibold text-gray-400 dark:text-gray-500">
                  Sélectionnez une conversation
                </h3>
                <p className="text-sm text-gray-400 dark:text-gray-600 mt-1">
                  Échangez avec les business et la communauté
                </p>
              </div>
            </div>
          )}
                ...
                <div className="mt-4 max-h-64 overflow-y-auto space-y-1">
                  {searchingRecipients ? (
                    ...
                    recipientResults.map((r: any) => (
                      ...
                      >
                        <div className="w-10 h-10 rounded-full bg-gradient-to-br from-purple-500 to-blue-500 flex items-center justify-center text-white font-bold shrink-0 relative">
                          {r.logo ? (
                            <Image
                              src={r.logo}
                              alt=""
