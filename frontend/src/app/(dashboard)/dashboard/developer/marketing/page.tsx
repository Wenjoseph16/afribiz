...
const OPTIMIZATION_TIPS = [
  {
    icon: Lightbulb,
    title: 'Optimisez vos descriptions',
    description:
      'Une description claire avec des mots-clés ciblés augmente la visibilité dans la marketplace.',
    color: 'bg-amber-50 dark:bg-amber-900/30 text-amber-600',
  },
  {
    icon: Target,
    title: "Ajoutez des captures d'écran",
    description: "Les modules avec des visuels ont 40% plus de chances d'être installés.",
    color: 'bg-blue-50 dark:bg-blue-900/30 text-blue-600',
  },
  {
    icon: Star,
    title: 'Encouragez les avis',
    description: 'Les modules bien notés sont mis en avant dans les recherches.',
    color: 'bg-purple-50 dark:bg-purple-900/30 text-purple-600',
  },
  {
    icon: TrendingUp,
    title: 'Publiez des mises à jour',
    description: "Les modules activement maintenus sont favorisés par l'algorithme.",
    color: 'bg-emerald-50 dark:bg-emerald-900/30 text-emerald-600',
  },
  {
    icon: Download,
    title: 'Proposez un essai gratuit',
    description: "Les modules avec version d'essai convertissent 3x mieux.",
    color: 'bg-cyan-50 dark:bg-cyan-900/30 text-cyan-600',
  },
  {
    icon: MessageCircle,
    title: 'Répondez aux avis',
    description: 'Montrez votre réactivité pour renforcer la confiance des utilisateurs.',
...
export default function MarketingPage() {
  ...
  const rankedModules = useMemo(() => {
    ...
    if (sortBy === 'installs')
      return list.sort((a, b) => (b.totalInstalls || 0) - (a.totalInstalls || 0));
  ...
  const totalInstallsAll = moduleList.reduce(
    (s: number, m: DeveloperModule) => s + (m.totalInstalls || 0),
    0
  );
  const totalRevenueAll = moduleList.reduce(
    (s: number, m: DeveloperModule) => s + (m.totalRevenue || 0),
    0
  );
        ...
        {[
          {
            ...
            value: moduleList.filter((m: DeveloperModule) => m.status === 'PUBLISHED').length,
            color: 'bg-brand-50 dark:bg-brand-900/30 text-brand',
          },
          {
            icon: Download,
            label: 'Installations totales',
            value: formatNumber(totalInstallsAll),
            color: 'bg-blue-50 dark:bg-blue-900/30 text-blue-600',
          },
          {
            icon: Star,
            label: 'Note moyenne',
            value: overview?.averageRating ? `${overview.averageRating.toFixed(1)} / 5` : '—',
            color: 'bg-amber-50 dark:bg-amber-900/30 text-amber-600',
          },
          {
            icon: DollarSign,
        ...
        <Card padding="lg" className="lg:col-span-2">
          ...
          {rankedModules.length === 0 ? (
            <EmptyState
              ...
              title="Aucun module"
              description="Créez votre premier module pour apparaître ici."
            />
          ) : (
            <div className="space-y-2">
                ...
                >
                  ...
                  <div className="text-right shrink-0">
                    <p className="text-sm font-semibold text-gray-900 dark:text-gray-100">
                      {sortBy === 'installs'
                        ? formatNumber(mod.totalInstalls || 0)
                        : sortBy === 'rating'
                          ? `${(mod.rating || 0).toFixed(1)} ★`
        ...
        <Card padding="lg" className="bg-gradient-to-br from-brand-500 to-purple-600 text-white">
          <div className="flex flex-col items-center text-center py-4">
            ...
            <p className="text-sm text-white/80 mb-6">
              Contactez l'équipe AfriBiz pour proposer vos modules en featured sur la marketplace et
              augmentez votre visibilité.
            </p>
            <Link href="/dashboard/support">
              <Button
                variant="secondary"
                className="bg-white text-brand hover:bg-gray-100 border-0"
              >
            ...
            {[
              {
                ...
                value:
                  totalInstallsAll > 0
                    ? `${((moduleList.filter((m: DeveloperModule) => m.totalInstalls > 0).length / Math.max(moduleList.length, 1)) * 100).toFixed(0)}%`
                    : '—',
                icon: TrendingUp,
              ...
              },
              {
                ...
                value:
                  moduleList.length > 0
                    ? formatCurrency(Math.round(totalRevenueAll / moduleList.length))
                    : '—',
                icon: DollarSign,
                color: 'text-blue-600',
              ...
              },
              {
                ...
                value: overview?.totalReviews
                  ? `${Math.round((overview.averageRating / 5) * 100)}%`
                  : '—',
                icon: Star,
              ...
              {
                label: 'Moy. installations/module',
                value:
                  moduleList.length > 0
                    ? formatNumber(Math.round(totalInstallsAll / moduleList.length))
                    : '—',
                icon: Download,
      ...
      <Card padding="lg">
        ...
        <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-4">
          {OPTIMIZATION_TIPS.map((tip) => (
            <div
              key={tip.title}
              className="flex items-start gap-3 p-4 rounded-xl border border-gray-200 dark:border-gray-700 hover:bg-gray-50 dark:hover:bg-gray-800/50 transition-colors"
            >
