...
export default function DeveloperShortsPage() {
  ...
  return (
    <div className="space-y-6">
      <PageHeader
        title="Shorts"
        description="Creez et gerez vos videos courtes"
        actions={
          <button
      ...
      <div className="grid grid-cols-1 sm:grid-cols-4 gap-4">
        <Card padding="md">
          <div className="flex items-center gap-3">
            <div className="p-2 rounded-lg bg-brand-100 dark:bg-brand-900/20 text-brand">
              <Film className="h-5 w-5" />
            </div>
            <div>
              <p className="text-xs text-gray-500 dark:text-gray-400">Total</p>
              <p className="text-lg font-bold text-gray-900 dark:text-gray-100">{shorts.length}</p>
            </div>
          </div>
        </Card>
        <Card padding="md">
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-lg font-bold">{shorts.reduce((s: number, v) => s + v.views, 0)}</p>
            </div>
          </div>
        </Card>
        <Card padding="md">
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-lg font-bold">{shorts.reduce((s: number, v) => s + v.likes, 0)}</p>
            </div>
          </div>
        </Card>
        <Card padding="md">
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-lg font-bold">
                {shorts.reduce((s: number, v) => s + v.comments, 0)}
              </p>
            </div>
          </div>
        </Card>
      ...
      <EmptyState
        ...
        action={
          <Button variant="gradient" onClick={() => setShowUploader(true)}>
