...
import { Shield, Plus, Trash2, Save, Lock, Unlock, AlertTriangle } from 'lucide-react';
...
export default function ModulePermissionsPage() {
  ...
  const [removePermissionTarget, setRemovePermissionTarget] = useState<string | null>(null);

  const confirmRemovePermission = async () => {
    if (!removePermissionTarget) return;
    try {
      await removePermission.mutateAsync(removePermissionTarget);
      showToast('Permission supprimée', 'success');
    } catch {
      showToast('Erreur lors de la suppression', 'error');
    }
    setRemovePermissionTarget(null);
  };

  const handleRemove = async (permissionId: string) => {
    setRemovePermissionTarget(permissionId);
  };
  ...
  return (
    <div className="space-y-6 animate-fade-in">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
              ...
              <div>
                ...
                <Select
                  ...
                  options={ACCESS_LEVELS.map((a) => ({ value: a, label: ACCESS_LABELS[a] }))}
                />
              </div>
            </div>
            <div>
                  ...
                  {items.map((p: any) => (
                    ...
                    >
                      ...
                      <div className="flex items-center gap-2 shrink-0">
                        ...
                        {p.isRequired && (
                          <span className="text-[10px] text-amber-600 bg-amber-50 dark:bg-amber-900/30 px-1.5 py-0.5 rounded-full">
                            Requis
                          </span>
                        )}
      ...
      <ConfirmationModal
        open={!!removePermissionTarget}
        onClose={() => setRemovePermissionTarget(null)}
        onConfirm={confirmRemovePermission}
        title="Supprimer la permission"
        description="Êtes-vous sûr de vouloir supprimer cette permission ? Le module perdra l'accès à cette ressource."
