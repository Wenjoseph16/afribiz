...
export default function ModuleConfigurationPage() {
  ...
  const configList = useMemo(() => {
    ...
    if (search.trim()) {
      ...
      list = list.filter((c: ModuleConfiguration) => c.business?.name?.toLowerCase().includes(q));
    }
    return list;
  }, [configs, search]);
        ...
        <EmptyState
          ...
          description={
            search
              ...
              : "Ce module n'a pas encore été configuré par des business."
          }
        />
      ) : (
                    ...
                    >
                      {cfg.business?.logo ? (
                        ...
                        <Globe
                          className={cn(
                            'h-5 w-5',
                            cfg.isActive ? 'text-emerald-600' : 'text-gray-400'
                          )}
                        />
                      )}
                    </div>
                      ...
                      <div className="flex items-center gap-3 mt-0.5 text-xs text-gray-400">
                        {cfg.installation?.createdAt && (
                          <span className="flex items-center gap-1">
                            ...
                            Installé le{' '}
                            {new Date(cfg.installation.createdAt).toLocaleDateString('fr-FR')}
                          </span>
                        )}
                        <span className="flex items-center gap-1">
                  ...
                  <div className="pt-4">
                    ...
                    <div className="flex items-center justify-between mt-4 pt-3 border-t border-gray-100 dark:border-gray-700">
                      <div className="flex items-center gap-2 text-xs text-gray-500">
                        <Clock className="h-3 w-3" />
                        Mis à jour le{' '}
                        {cfg.updatedAt ? new Date(cfg.updatedAt).toLocaleDateString('fr-FR') : '—'}
                      </div>
                      <div className="flex items-center gap-2">
                        <span
                          className={cn(
