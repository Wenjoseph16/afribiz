...
const PRICING_OPTIONS: ModulePricingType[] = [
  'FREE',
  'ONE_TIME',
  'MONTHLY',
  'QUARTERLY',
  ...
  const handleSubmit = async () => {
    try {
      ...
      if (form.requirements.trim())
        payload.requires = form.requirements
          .split(',')
          .map((t: string) => t.trim())
          .filter(Boolean);
      const res: any = await createModule.mutateAsync(payload);
      const newModuleId = res?.data?.data?.id;
      if (newModuleId) {
        ...
        <Card padding="lg">
          <div className="flex items-center gap-3 mb-5">
            ...
            <div>
              ...
              <p className="text-xs text-gray-500">
                Ajoutez des images pour valoriser votre module sur la marketplace
              </p>
            </div>
          </div>
          ...
          <div className="space-y-5">
                ...
                {!uploadedLogo && (
                  <div>
                    <p className="text-sm text-gray-500 dark:text-gray-400">
                      Ajoutez un logo pour votre module
                    </p>
            ...
            <div className="pt-4 border-t border-gray-100 dark:border-gray-700">
              <p className="text-sm font-medium text-gray-700 dark:text-gray-300 mb-2">
                Captures d'écran ({uploadedScreenshots.length})
              </p>
              <input
                type="file"
                ...
                {uploadingScreenshots ? (
                  ...
                  <>
                    ...
                    <p className="text-xs text-gray-400 mt-1">
                      Jusqu'à 10 fichiers — PNG, JPG, WebP
                    </p>
                  </>
                )}
              </div>
              {uploadedScreenshots.length > 0 && (
                <div className="flex flex-wrap gap-2 mt-3">
                  {uploadedScreenshots.map((name, idx) => (
                    <span
                      key={idx}
                      className="text-xs bg-gray-100 dark:bg-gray-800 text-gray-600 dark:text-gray-400 px-2 py-1 rounded-lg"
                    >
                      ✓ {name}
                    </span>
                  ))}
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-5">
                <div>
                  ...
                  <Select
                    ...
                    options={MODULE_CATEGORIES.map((cat) => ({ value: cat, label: cat }))}
                  />
                </div>
                <div>
                  <label className="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1.5">
              ...
              <div className="grid grid-cols-1 sm:grid-cols-3 gap-5">
                <div>
                  ...
                  <Select
                    ...
                    options={PRICING_OPTIONS.map((pt) => ({
                      value: pt,
                      label: PRICING_LABELS[pt],
                    }))}
                  />
                </div>
