...
export default function DeveloperSubscriptionsPage() {
  ...
  const cancelMutation = useMutation({
    ...
    onSuccess: () => {
      qc.invalidateQueries({ queryKey: ['developer-subscriptions'] });
    },
  });
  ...
  return (
    <div className="space-y-6 animate-fade-in">
      ...
      {subscriptionList.length === 0 ? (
        ...
        <div className="space-y-3">
          {subscriptionList.map((sub) => {
            ...
            const pricingLabel = mod?.pricingType
              ? PRICING_LABELS[mod.pricingType] || mod.pricingType
              : '—';
            const pricingValue = mod?.price
              ? `${mod.price.toLocaleString()} FCFA / ${pricingLabel}`
              : pricingLabel;
            return (
                  ...
                  <div className="flex items-start gap-3 min-w-0 flex-1">
                    ...
                    <div className="min-w-0">
                      ...
                      <p className="text-xs text-gray-400 dark:text-gray-500 mt-0.5">
                        Installé le{' '}
                        {sub.installedAt
                          ? new Date(sub.installedAt).toLocaleDateString('fr-FR')
                          : '—'}
                      </p>
                    </div>
                  </div>
      ...
      <ConfirmationModal
        ...
        description="L'abonnement sera désactivé et l'entreprise ne pourra plus utiliser ce module."
