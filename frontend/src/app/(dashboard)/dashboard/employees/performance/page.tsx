  ...
  const handleCreate = async () => {
    ...
    try {
      ...
      await apiClient.post('/business/employees/performances', {
        ...
        periodEnd: form.date,
        punctuality: form.metric === 'PUNCTUALITY' ? form.value : undefined,
        clientSatisfaction: form.metric === 'CLIENT_SATISFACTION' ? form.value : undefined,
        efficiency: form.metric === 'PRODUCTIVITY' ? form.value : undefined,
        overallScore: form.value,
        reviewNotes: form.comment || undefined,
      });
      setShowCreate(false);
      setForm({
        employeeId: '',
      ...
      <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
        ...
        <Card className="p-3">
          <div className="flex items-center gap-2">
            ...
            <div>
              <p className="text-[10px] text-gray-500">Présences</p>
        ...
        <Card className="p-3">
          <div className="flex items-center gap-2">
            ...
            <div>
              <p className="text-[10px] text-gray-500">Ponctualité moyenne</p>
              <p className="text-lg font-bold">
                {employeeStats.length > 0
                  ? Math.round(
                      employeeStats.reduce((s: number, e: any) => s + e.punctuality, 0) /
        ...
        <Card className="p-3">
          <div className="flex items-center gap-2">
            ...
            <div>
              <p className="text-[10px] text-gray-500">Absences</p>
              <p className="text-lg font-bold">
      ...
      {filtered.length === 0 ? (
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
          {filtered.map((emp: any) => {
            const stats = employeeStats.find((e: any) => e.id === emp.id) || {
              punctuality: 0,
              totalAttendances: 0,
              lateCount: 0,
              absentCount: 0,
