...
export default function EmployeesPage() {
  ...
  const filtered = useMemo(() => {
    ...
    switch (activeTab) {
      case 'active':
        f = f.filter((e) => e.status === 'ACTIVE');
        break;
      case 'inactive':
        f = f.filter((e) => e.status === 'INACTIVE' || e.status === 'SUSPENDED');
        break;
    }
    if (searchQuery) {
    ...
    <div className="space-y-6 animate-fade-in">
      ...
      <div className="grid grid-cols-2 sm:grid-cols-4 gap-4">
        ...
        <StatsCard
          ...
          label="Rôles"
          value={stats.rolesCount ?? roles.length}
        />
        <StatsCard
          icon={<Users className="h-5 w-5" />}
          iconBg="bg-amber-50"
      ...
      {allEmployees.length > 0 &&
        (() => {
          const newHires = allEmployees.filter(
            (e) =>
              e.hireDate &&
              new Date(e.hireDate) > new Date(now.getTime() - 30 * 24 * 60 * 60 * 1000)
          );
          const suspended = allEmployees.filter((e) => e.status === 'SUSPENDED');
          const seniorEmployees = allEmployees.filter(
            (e) =>
              e.hireDate &&
              new Date(e.hireDate) < new Date(now.getTime() - 365 * 24 * 60 * 60 * 1000)
          );
          ...
          const suggestions = [
            ...
            departments.length > 0 && {
              ...
              desc:
                ...
                (departments.length > 3 ? ` et +${departments.length - 3}` : ''),
              color: 'purple',
...
function getBadges(employee: EmployeeItem) {
  ...
  if (
    ...
    employee.position?.toLowerCase().includes('chef') ||
            ...
            {badges.length > 0 && (
              <div className="flex flex-wrap gap-1 mt-1.5">
                {badges.map((b, i) => (
                  <span
                    key={i}
                    className={cn('text-[10px] font-medium px-1.5 py-0.5 rounded-full', b.class)}
                  >
                    {b.label}
                  </span>
                ))}
