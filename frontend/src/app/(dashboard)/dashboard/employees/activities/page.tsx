    ...
    (async () => {
      try {
        ...
        const data = Array.isArray(res.data.data)
          ? res.data.data
          : res.data.data?.activities || res.data.data?.logs || [];
        setLogs(data);
      } catch (err) {
        console.error(err);
      }
      ...
      <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
        ...
        <Card className="p-3">
          <div className="flex items-center gap-2">
            ...
            <div>
              <p className="text-[10px] text-gray-500">Employés actifs</p>
              <p className="text-lg font-bold">
                {new Set(logs.map((l: any) => l.employeeId).filter(Boolean)).size}
              </p>
            </div>
          </div>
        </Card>
        <Card className="p-3">
          <div className="flex items-center gap-2">
            ...
            <div>
              <p className="text-[10px] text-gray-500">Types actions</p>
              <p className="text-lg font-bold">{uniqueActions.length}</p>
            </div>
          </div>
        </Card>
        <Card className="p-3">
