...
export default function EmployeePortalPage() {
  ...
  const [employeeName, setEmployeeName] = useState(
    () => localStorage.getItem('portal_employee_name') || ''
  );
  ...
  const tasks: any[] = useMemo(() => {
    ...
    return raw.filter(
      (t: any) => !employeeName || t.assignee?.toLowerCase() === employeeName.toLowerCase()
    );
        ...
        <Card className="p-8 max-w-sm w-full text-center">
          ...
          <input
            ...
            onChange={(e) => {
              ...
              localStorage.setItem('portal_employee_name', e.target.value);
            }}
          ...
          />
          <Button className="w-full" onClick={() => {}} disabled={!employeeName}>
            Accéder
          </Button>
        </Card>
      </div>
              ...
              <div className="flex items-center gap-2">
                ...
                <div>
                  <p className="text-sm font-semibold text-red-800 dark:text-red-300">
                    ⚠️ {overdueTasks.length} tâche(s) en retard
                  </p>
                  <p className="text-xs text-red-600 dark:text-red-400">
                    Vous avez des tâches qui dépassent leur échéance. Veuillez les traiter
                    rapidement.
                  </p>
                </div>
              </div>
              ...
              <div className="flex items-center gap-2">
                ...
                <div>
                  <p className="text-sm font-semibold text-amber-800 dark:text-amber-300">
                    ⏰ {pendingTasks.length} tâches en attente
                  </p>
                  <p className="text-xs text-amber-600 dark:text-amber-400">
                    Vous accumulez des tâches non terminées. Priorisez les plus urgentes.
                  </p>
                </div>
              </div>
            </Card>
        ...
        <Card className="p-3">
          <button
            ...
            className={cn(
              ...
              clockedIn ? 'text-red-600' : 'text-emerald-600'
            )}
          >
            <Clock className="w-4 h-4" />
            <span className="text-xs font-medium">
              {clockedIn ? 'Pointer sortie' : 'Pointer arrivée'}
            </span>
          </button>
        ...
        <Card className="p-3">
          <div className="flex items-center gap-2">
            ...
            <div>
              ...
              <p className="text-sm font-bold">{completedTasks.length}</p>
            </div>
          </div>
        </Card>
        <Card className="p-3">
          <div className="flex items-center gap-2">
            ...
            <div>
              ...
              <p className="text-sm font-bold">{pendingTasks.length}</p>
            </div>
          </div>
        </Card>
        <Card className="p-3">
          <div className="flex items-center gap-2">
            ...
            <div>
              ...
              <p className="text-sm font-bold">{overdueTasks.length}</p>
            </div>
          </div>
        </Card>
      </div>
        ...
        <div className="space-y-2">
          ...
          {tasks.length === 0 ? (
            <Card className="text-center py-8">
              <CheckCircle className="h-8 w-8 text-gray-200 dark:text-gray-700 mx-auto mb-2" />
              <p className="text-xs text-gray-500">Aucune tâche assignée</p>
            </Card>
          ) : (
                ...
                <Card key={t.id} className="p-3">
                  <div className="flex items-center justify-between">
                    ...
                    <div className="flex items-center gap-2">
                      ...
                      <span className={cn('text-[10px] px-2 py-0.5 rounded-full', sc.color)}>
                        {sc.label}
                      </span>
                    </div>
                  </div>
        ...
        <div className="space-y-4">
          ...
          {myAttendances.length === 0 ? (
            <Card className="text-center py-8">
              ...
              <p className="text-xs text-gray-500">Aucun pointage</p>
            </Card>
          ) : (
            <div className="space-y-2">
                    ...
                    <div className="flex items-center gap-3">
                      ...
                      <div>
                        <p className="text-xs text-gray-500">
                          ...
                          {' — '}
                          {a.clockIn
                            ? new Date(a.clockIn).toLocaleTimeString('fr-FR', {
                                hour: '2-digit',
                                minute: '2-digit',
                              })
                            : '—'}
