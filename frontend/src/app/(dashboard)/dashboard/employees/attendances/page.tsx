...
export default function EmployeeAttendancesPage() {
  ...
  const {
    ...
    isLoading,
    refetch,
  } = useEmployeeAttendances({ limit: 200, sortBy: 'createdAt', order: 'desc' });
  ...
  const getAttendanceStatus = (att: any): string => {
    if (att.isAbsent) return 'ABSENT';
    if (att.isLate) return 'LATE';
    if (att.clockIn) return 'PRESENT';
    return 'PRESENT';
  };

  const filtered = useMemo(() => {
    let items = allAttendances.map((a: any) => ({
      ...a,
      computedStatus: a.status || getAttendanceStatus(a),
    }));
    if (statusFilter !== 'all') items = items.filter((a: any) => a.computedStatus === statusFilter);
    if (search) {
        ...
        <Card className="p-3">
          <div className="flex items-center gap-2">
            ...
            <div>
              ...
              <p className="text-sm font-bold">{stats.late}</p>
            </div>
          </div>
        </Card>
        <Card className="p-3">
        ...
        <Card className="p-3">
          <div className="flex items-center gap-2">
            ...
            <div>
              ...
              <p className="text-sm font-bold">{stats.onLeave}</p>
            </div>
          </div>
        </Card>
      </div>
      ...
      {filtered.length === 0 ? (
        <Card className="text-center py-12">
          ...
          <p className="text-sm text-gray-500">
            {search ? 'Aucun résultat' : "Aucun pointage pour l'instant"}
          </p>
        </Card>
      ) : (
        <div className="space-y-2">
          {filtered.map((att: any) => {
            const computedStatus = att.computedStatus || att.status || 'PRESENT';
            const sc = STATUS_CONFIG[computedStatus] || STATUS_CONFIG.PRESENT;
                  ...
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      <p className="text-sm font-semibold text-gray-900 dark:text-white">
                        {att.employeeName || att.employee?.name || 'Employé'}
                      </p>
                      <p className="text-xs text-gray-500 flex items-center gap-2 mt-0.5">
                        <CalendarDays className="w-3 h-3" />
                        {clockIn
                          ? clockIn.toLocaleDateString('fr-FR', {
                              day: 'numeric',
                              month: 'short',
                              year: 'numeric',
                            })
                        ...
                        {clockIn
                          ? clockIn.toLocaleTimeString('fr-FR', {
                              hour: '2-digit',
                              minute: '2-digit',
                            })
                          : '—'}
                        {clockOut && (
                          <span>
                            →{' '}
                            {clockOut.toLocaleTimeString('fr-FR', {
                              hour: '2-digit',
                              minute: '2-digit',
                            })}
                          </span>
          ...
          >
            ...
            <div className="space-y-4">
              <div>
                ...
                <input
                  type="text"
                  value={clockForm.employeeName}
                  onChange={(e) => setClockForm((f) => ({ ...f, employeeName: e.target.value }))}
                  required
              ...
              <div>
                ...
                <input
                  ...
                  onChange={(e) => setClockForm((f) => ({ ...f, employeeId: e.target.value }))}
                  className="w-full p-2.5 text-sm border border-gray-200 dark:border-gray-700 rounded-xl bg-transparent dark:text-gray-100"
              ...
              <div className="flex gap-2 pt-2">
                ...
                <button
                  ...
                  disabled={clocking || !clockForm.employeeName}
