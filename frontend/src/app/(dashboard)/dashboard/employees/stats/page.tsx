...
export default function EmployeeStatsPage() {
  ...
  const { data: enrichedData, isLoading: enrichedLoading } = useQuery({
    queryKey: ['employees', 'stats', 'enriched'],
    queryFn: async () => {
      const res = await apiClient.get('/business/employees/stats/enriched');
  ...
  const attendances: any[] = useMemo(() => {
    const raw = Array.isArray(attendancesData)
      ? attendancesData
      : attendancesData?.attendances || attendancesData?.data || [];
    return raw;
  }, [attendancesData]);
  ...
  const activeEmployees = employees.filter(
    (e: any) => e.status === 'ACTIVE' || e.status === 'ACTIF' || !e.status
  );
        ...
        employees.length > 0
          ? Math.round(
              totalAttendances /
                Math.max(
                  1,
                  new Set(attendances.map((a: any) => new Date(a.createdAt).toDateString())).size
                )
            )
          : 0,
      color: 'bg-purple-500',
  ...
  const employeesWithLowPunctuality = employees.filter((e: any) => {
    const empAtt = attendances.filter(
      (a: any) => a.employeeId === e.id || a.employeeName === e.name
    );
  ...
  return (
    <div className="animate-fade-in space-y-6">
              ...
              <div className="flex items-center gap-2">
                ...
                <div>
                  <p className="text-sm font-semibold text-amber-800 dark:text-amber-300">
                    ⏰ {todayLate.length} employé(s) en retard aujourd'hui
                  </p>
                  <p className="text-xs text-amber-600 dark:text-amber-400">
                    {todayLate
                      .slice(0, 3)
                      .map((a: any) => a.employeeName || a.employee?.name)
                      .filter(Boolean)
                      .join(', ')}
              ...
              <div className="flex items-center gap-2">
                ...
                <div>
                  <p className="text-sm font-semibold text-red-800 dark:text-red-300">
                    🚫 {todayAbsent.length} absence(s) aujourd'hui
                  </p>
              ...
              <div className="flex items-center gap-2">
                ...
                <div>
                  <p className="text-sm font-semibold text-orange-800 dark:text-orange-300">
                    📉 {employeesWithLowPunctuality.length} employé(s) avec faible ponctualité
                  </p>
                  <p className="text-xs text-orange-600 dark:text-orange-400">
                    {employeesWithLowPunctuality
                      .slice(0, 3)
                      .map((e: any) => e.name)
                      .filter(Boolean)
                      .join(', ')}
                    {employeesWithLowPunctuality.length > 3 &&
                      ` et ${employeesWithLowPunctuality.length - 3} autre(s)`}
                  </p>
                </div>
              </div>
            </Card>
              ...
              <div className="flex items-center gap-3">
                <div
                  className={cn(
                    ...
                    kpi.color === 'bg-brand'
                      ? 'bg-brand/10'
                      : `${kpi.color.replace('bg-', 'bg-').replace('bg-', 'bg-')} bg-opacity-10`,
                    'flex items-center'
                  )}
                >
                  <Icon
                    className={cn(
                      'w-4 h-4',
                      kpi.color === 'bg-brand' ? 'text-brand' : kpi.color.replace('bg-', 'text-')
                    )}
                  />
                </div>
                <div>
      ...
      {!enrichedLoading && enrichedData && (
        <Card className="p-5">
          <h3 className="font-semibold text-sm text-gray-900 dark:text-white mb-5 flex items-center gap-2">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
            ...
            <div className="p-3 rounded-xl bg-gray-50 dark:bg-gray-800/50">
              <p className="text-[10px] text-gray-500 uppercase font-semibold mb-1">Turnover</p>
              <p
                className={cn(
                  'text-2xl font-bold',
                  (enrichedData.turnoverRate ?? 0) < 5
                    ? 'text-emerald-600'
                    : (enrichedData.turnoverRate ?? 0) <= 15
                      ? 'text-amber-600'
                      : 'text-red-600'
                )}
              >
                {(enrichedData.turnoverRate ?? 0).toFixed(1)}%
              </p>
            </div>
            {/* Average tenure */}
            <div className="p-3 rounded-xl bg-gray-50 dark:bg-gray-800/50">
              ...
              <p className="text-2xl font-bold text-gray-900 dark:text-white">
                {enrichedData.averageTenureDays != null
                  ? `${(enrichedData.averageTenureDays / 30).toFixed(1)} mois`
                  : '—'}
              </p>
            </div>
            {/* Total payroll */}
            <div className="p-3 rounded-xl bg-gray-50 dark:bg-gray-800/50">
              ...
              <p className="text-2xl font-bold text-gray-900 dark:text-white">
                {enrichedData.totalPayroll != null ? formatPrice(enrichedData.totalPayroll) : '—'}
              </p>
            </div>
            {/* Average salary */}
            <div className="p-3 rounded-xl bg-gray-50 dark:bg-gray-800/50">
              ...
              <p className="text-2xl font-bold text-gray-900 dark:text-white">
                {enrichedData.averageSalary != null ? formatPrice(enrichedData.averageSalary) : '—'}
              </p>
            </div>
            {/* Attendance rate */}
            <div className="p-3 rounded-xl bg-gray-50 dark:bg-gray-800/50">
              ...
              <p
                className={cn(
                  'text-2xl font-bold',
                  (enrichedData.attendanceRate ?? 0) >= 90
                    ? 'text-emerald-600'
                    : (enrichedData.attendanceRate ?? 0) >= 75
                      ? 'text-amber-600'
                      : 'text-red-600'
                )}
              >
                {(enrichedData.attendanceRate ?? 0).toFixed(1)}%
              </p>
            </div>
            {/* Gender distribution */}
            <div className="p-3 rounded-xl bg-gray-50 dark:bg-gray-800/50 sm:col-span-2 lg:col-span-1">
              <p className="text-[10px] text-gray-500 uppercase font-semibold mb-2">
                Répartition par genre
              </p>
              <div className="flex flex-wrap gap-2">
                {(enrichedData.genderDistribution as Record<string, number>) ? (
                  Object.entries(enrichedData.genderDistribution as Record<string, number>).map(
                    ([key, count]) => (
          ...
          <div className="mt-5">
            <p className="text-[10px] text-gray-500 uppercase font-semibold mb-3">
              Répartition par département
            </p>
            <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-5 gap-2">
              {(enrichedData.departmentBreakdown as Record<string, number>) ? (
                Object.entries(enrichedData.departmentBreakdown as Record<string, number>).map(
                  ([dept, count]) => (
                    <div
                      key={dept}
                      className="p-2 rounded-lg bg-gray-50 dark:bg-gray-800/50 text-center"
                    >
                      <p className="text-xs font-medium text-gray-900 dark:text-white">{dept}</p>
                      <p className="text-lg font-bold text-brand">{count}</p>
                    </div>
                  )
                )
              ) : (
          ...
          <div className="mt-5">
            ...
            <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-5 gap-2">
              {(enrichedData.positionBreakdown as Record<string, number>) ? (
                Object.entries(enrichedData.positionBreakdown as Record<string, number>).map(
                  ([pos, count]) => (
                    <div
                      key={pos}
                      className="p-2 rounded-lg bg-gray-50 dark:bg-gray-800/50 text-center"
                    >
                      <p className="text-xs font-medium text-gray-900 dark:text-white">{pos}</p>
                      <p className="text-lg font-bold text-brand">{count}</p>
                    </div>
                  )
                )
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        ...
        <Card className="p-4">
          ...
          <div className="space-y-3">
            {[
              ...
              {
                label: 'Suspendus',
                count: employees.filter((e: any) => e.status === 'SUSPENDED').length,
                color: 'bg-red-500',
              },
              {
                label: 'En congé',
                count: employees.filter((e: any) => e.status === 'ON_LEAVE' || e.status === 'CONGE')
                  .length,
                color: 'bg-amber-500',
              },
              {
                label: 'Inactifs',
                count: employees.filter((e: any) => e.status === 'INACTIVE').length,
                color: 'bg-gray-400',
              },
            ].map((s) => (
              <div key={s.label}>
                <div className="flex justify-between text-xs mb-1">
                  ...
                  <span className="font-medium text-gray-900 dark:text-white">
                    {s.count} (
                    {employees.length > 0 ? Math.round((s.count / employees.length) * 100) : 0}%)
                  </span>
                </div>
                <div className="w-full h-2 bg-gray-100 dark:bg-gray-800 rounded-full overflow-hidden">
                  <div
                    ...
                    style={{
                      width: `${employees.length > 0 ? (s.count / employees.length) * 100 : 0}%`,
                    }}
                  />
                </div>
              </div>
            ...
            {totalAttendances > 0 && (
              <>
                <div
                  ...
                  style={{ width: `${(presentCount / totalAttendances) * 100}%` }}
                />
                <div
                  className="h-full bg-amber-500 transition-all"
                  style={{ width: `${(lateCount / totalAttendances) * 100}%` }}
                />
                <div
                  className="h-full bg-red-500 transition-all"
                  style={{ width: `${(absentCount / totalAttendances) * 100}%` }}
                />
              </>
            )}
          </div>
        ...
        <Card className="p-4">
          ...
          {employees.slice(0, 5).map((emp: any, i: number) => {
            const empAtt = attendances.filter(
              (a: any) => a.employeeId === emp.id || a.employeeName === emp.name
            );
            const empPresent = empAtt.filter((a: any) => a.status === 'PRESENT').length;
          ...
          <div className="space-y-2">
            {absentCount > totalAttendances * 0.2 && (
              <div className="p-3 rounded-xl bg-white/60 dark:bg-gray-800/60">
                <p className="text-xs font-medium text-blue-800 dark:text-blue-300">
                  ⚠️ Taux d'absence élevé
                </p>
                <p className="text-[10px] text-blue-600 dark:text-blue-400 mt-1">
                  Les absences représentent{' '}
                  {totalAttendances > 0 ? Math.round((absentCount / totalAttendances) * 100) : 0}%
                  des pointages.
                </p>
              </div>
            )}
            {lateCount > 0 && (
              <div className="p-3 rounded-xl bg-white/60 dark:bg-gray-800/60">
                ...
                <p className="text-[10px] text-blue-600 dark:text-blue-400 mt-1">
                  {lateCount} retards enregistrés. Envisagez un rappel automatique aux employés
                  concernés.
                </p>
              </div>
            )}
            <div className="p-3 rounded-xl bg-white/60 dark:bg-gray-800/60">
              ...
              <p className="text-[10px] text-blue-600 dark:text-blue-400 mt-1">
                {employees.length} employés pour {totalAttendances} présences enregistrées.
              </p>
            </div>
          </div>
        </Card>
      ...
      <div className="grid grid-cols-2 sm:grid-cols-4 gap-3 text-center">
        ...
        <div className="p-3 rounded-xl bg-gray-50 dark:bg-gray-800/50">
          <p className="text-2xl font-bold text-gray-900 dark:text-white">
            {stats.averageTenure ? `${stats.averageTenure}m` : '—'}
          </p>
