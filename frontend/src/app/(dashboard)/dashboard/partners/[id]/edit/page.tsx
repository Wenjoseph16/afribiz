...
export default function EditPartnerPage() {
  ...
  useEffect(() => {
    if (partner && !loaded) {
      ...
      setForm({
        name: p.name || '',
        category: p.category || 'FOURNISSEUR',
        ...
        city: p.city || '',
        country: p.country || '',
        servicesProposes: p.servicesProposes || '',
        produitsFournis: p.produitsFournis || '',
        zonesCouvertes: p.zonesCouvertes || '',
      ...
      <form onSubmit={handleSubmit}>
              ...
              <div className="grid grid-cols-5 gap-2">
                {CATEGORIES.map((cat) => {
                  ...
                  return (
                    ...
                    >
                      <Icon
                        className={`h-4 w-4 mx-auto mb-0.5 ${isActive ? 'text-brand' : 'text-gray-400'}`}
                      />
                      {cat.label}
                    </button>
                  );
              ...
              <div className="grid grid-cols-4 gap-2">
                {COLLAB_LEVELS.map((level) => (
                  <button
                    key={level}
                    type="button"
                    onClick={() => update('collaborationLevel', level)}
                    className={`p-2 rounded-lg border-2 text-center transition-all text-xs ${
                      form.collaborationLevel === level
                        ? 'border-brand bg-brand/5'
                        : 'border-gray-200 dark:border-gray-700'
                    }`}
                  >
                    <Star
                      className={`h-4 w-4 mx-auto mb-0.5 ${form.collaborationLevel === level ? 'text-brand' : 'text-gray-400'}`}
                    />
        ...
        <div className="flex items-center justify-between mt-6">
          ...
          <Button type="submit" disabled={updatePartner.isPending}>
            ...
            {updatePartner.isPending ? 'Enregistrement...' : 'Enregistrer'}
          </Button>
        </div>
      </form>
    </div>
