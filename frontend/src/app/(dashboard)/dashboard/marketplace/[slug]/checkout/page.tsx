...
const MOBILE_MONEY_PROVIDERS = [
  ...
  { id: 'FLOOZ', label: 'Flooz', color: 'bg-yellow-500', countries: 'Togo, Bénin' },
  {
    ...
    label: 'Orange Money',
    color: 'bg-orange-500',
    countries: "Côte d'Ivoire, Sénégal, Mali, Burkina",
  },
...
export default function CheckoutPage() {
  ...
  const handleStartTrial = async () => {
    ...
    try {
      ...
      const errorMsg =
        err?.response?.data?.error || err?.message || "Erreur lors du démarrage de l'essai";
  ...
  const handleInitiatePayment = async () => {
    ...
    try {
      const res: any = await purchaseMutation.mutateAsync({
        moduleId: mod.id,
  ...
  const handleConfirmPayment = async () => {
    ...
    try {
      const res: any = await confirmMutation.mutateAsync({ providerRef });
      setPaymentStatus('success');
      setPaymentMessage(res.data?.message || 'Paiement confirmé et module installé !');
      setStep('confirm');
    } catch (err: any) {
      const errorMsg =
        err?.response?.data?.error || err?.message || 'Erreur lors de la confirmation';
      setPaymentStatus('error');
      setPaymentMessage(errorMsg);
    }
  };
    ...
    return (
      <div className="max-w-3xl mx-auto space-y-6 animate-fade-in flex flex-col items-center justify-center py-20">
        ...
        <p className="text-sm text-gray-500 dark:text-gray-400">
          Le module que vous cherchez à acheter n&apos;existe pas.
        </p>
  ...
  if (isFree) {
    return (
      <div className="max-w-3xl mx-auto space-y-6 animate-fade-in flex flex-col items-center justify-center py-20">
        ...
        <p className="text-sm text-gray-500 dark:text-gray-400">
          Ce module est gratuit. Vous pouvez l&apos;installer directement depuis la page du module.
        </p>
          ...
          <div className="flex items-center gap-4">
            ...
            <div className="flex-1 min-w-0">
              ...
              <p className="text-sm text-gray-500 dark:text-gray-400 truncate">
                {mod.shortDescription || mod.tagline || ''}
              </p>
      ...
      {paymentStatus === 'success' && (
        <div className="p-6 rounded-xl bg-emerald-50 dark:bg-emerald-900/20 border border-emerald-200 dark:border-emerald-800 text-center">
          ...
          <div className="flex items-center justify-center gap-3">
            <Link href={`/dashboard/marketplace/${slug}`}>
              <Button variant="secondary">Voir le module</Button>
            </Link>
            <Link href="/dashboard/marketplace">
                ...
                >
                  ...
                  <div className="flex-1">
                    ...
                    <p className="text-xs text-gray-500 dark:text-gray-400">
                      Testez sans payer. Aucun risque.
                    </p>
                  </div>
            ...
            <Card>
              <div className="p-5 sm:p-6">
                <h2 className="text-sm font-semibold text-gray-500 dark:text-gray-400 uppercase tracking-wider mb-4">
                  2. Paiement{' '}
                  {selectedProvider &&
                    `via ${MOBILE_MONEY_PROVIDERS.find((p) => p.id === selectedProvider)?.label}`}
                </h2>
