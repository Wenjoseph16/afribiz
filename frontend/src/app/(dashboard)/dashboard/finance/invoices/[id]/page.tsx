...
export default function InvoiceDetailPage() {
  ...
  if (!invoice) return <ErrorState message="Facture introuvable" onRetry={refetch} />;

  return (
    <div className="animate-fade-in space-y-6 max-w-4xl">
      <PageHeader
        title={invoice.invoiceNumber || `#${id.slice(0, 8)}`}
        description={`Créée le ${new Date(invoice.createdAt).toLocaleDateString('fr-FR', {
          day: 'numeric',
          month: 'long',
          year: 'numeric',
        })}`}
        breadcrumbs={[
          ...
          { label: invoice.invoiceNumber || 'Facture' },
        ]}
        actions={
        ...
        <div className="flex flex-wrap items-center gap-2">
          ...
          {remaining > 0 && invoice.status !== 'PAID' && invoice.status !== 'CANCELLED' && (
            <button
              onClick={() => {
                setPayModal(true);
                setPayAmount(Math.min(remaining, remaining));
              }}
          ...
          {invoice.quote && (
            <Link
              href={`/dashboard/finance/quotes/${invoice.quote.id}`}
          ...
          <button
            onClick={async () => {
              try {
                const res = await apiClient.get('/business/finance/invoices/' + id + '/pdf', {
                  responseType: 'blob',
                });
                const blob =
                  res.data instanceof Blob
                    ? res.data
                    : new Blob([String(res.data ?? '')], { type: 'application/pdf' });
              ...
              <div className="py-2 border-t border-gray-200 dark:border-gray-700">
                ...
                {Number(invoice.taxAmount || 0) > 0 && (
                  <div className="flex justify-between text-sm">
                    ...
                    <span>{formatPrice(Number(invoice.taxAmount))}</span>
                  </div>
                )}
                ...
                <div className="flex justify-between text-sm border-t border-gray-200 dark:border-gray-700 pt-2 mt-2">
                  ...
                  <span className="font-bold text-gray-900 dark:text-white">
                    {formatPrice(Number(invoice.totalAmount || 0))}
                  </span>
                </div>
                {Number(invoice.amountPaid || 0) > 0 && (
                  <div className="flex justify-between text-sm">
                    ...
                    <span className="text-emerald-600 font-medium">
                      {formatPrice(Number(invoice.amountPaid))}
                    </span>
                  </div>
                )}
                {remaining > 0 && (
                  <div className="flex justify-between text-sm">
                    <span className="text-amber-600">Reste dû</span>
            ...
            <div className="space-y-2 text-sm">
              ...
              <div className="flex justify-between">
                ...
                <span className="font-medium text-emerald-600">
                  {formatPrice(Number(invoice.amountPaid || 0))}
                </span>
              </div>
              {remaining > 0 && (
                <div className="flex justify-between">
                  <span className="text-gray-500">Reste</span>
                  <span className="font-bold text-amber-600">{formatPrice(remaining)}</span>
                </div>
              )}
              {invoice.paidAt && (
              ...
              {invoice.dueDate && (
                <div className="flex justify-between">
                  ...
                  <span
                    className={cn(
                      'font-medium',
                      isOverdue ? 'text-red-600' : 'text-gray-700 dark:text-gray-300'
                    )}
                  >
        ...
        >
          ...
          >
            ...
            <p className="text-sm text-gray-500 mb-4">
              Reste dû: <strong>{formatPrice(remaining)}</strong>
            </p>
            <input
              type="number"
              value={payAmount || ''}
              onChange={(e) => setPayAmount(Math.min(Number(e.target.value) || 0, remaining))}
              placeholder="Montant"
              max={remaining}
