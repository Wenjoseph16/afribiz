...
export default function QuotesPage() {
  ...
  const stats = {
    ...
    converted: allQuotes.filter((q: any) => q.status === 'CONVERTED').length,
    rejected: allQuotes.filter((q: any) => q.status === 'REJECTED' || q.status === 'EXPIRED')
      .length,
    draft: allQuotes.filter((q: any) => q.status === 'DRAFT').length,
    totalAmount: allQuotes.reduce((s: number, q: any) => s + Number(q.totalAmount || 0), 0),
  };

  const chartData: ModuleChartData = {
    distribution: [
      { name: 'Brouillons', value: stats.draft, color: '#6b7280' },
      { name: 'Envoyés', value: stats.sent, color: '#3b82f6' },
      { name: 'Acceptés', value: stats.accepted, color: '#10b981' },
      { name: 'Convertir', value: stats.converted, color: '#14b8a6' },
  ...
  return (
    <div className="animate-fade-in space-y-6">
      ...
      {stats.total > 0 && (
        <ModuleCharts
          data={chartData}
          title="ANALYTICS DEVIS"
          distributionLabel="Statut des devis"
      ...
      {filtered.length === 0 ? (
        <Card className="text-center py-12">
          ...
          <p className="text-sm text-gray-500 mb-4">Aucun devis trouvé</p>
          <Link href="/dashboard/finance/quotes/new">
            <Button>
              <Plus className="h-4 w-4 mr-1.5" />
              Nouveau devis
              ...
              <Link key={q.id} href={`/dashboard/finance/quotes/${q.id}`} className="block">
                <Card
                  className={cn(
                    ...
                    isExpired && 'ring-1 ring-amber-200 dark:ring-amber-800'
                  )}
                >
                  <div className="flex items-start justify-between gap-4">
                    <div className="flex-1">
                      <div className="flex items-center gap-2 flex-wrap">
                        ...
                        <Badge variant={isExpired ? 'warning' : s.variant} size="xs">
                          {isExpired ? 'Expiré' : s.label}
                        </Badge>
                      </div>
                      <div className="flex items-center gap-3 mt-1 text-xs text-gray-500">
                        ...
                        {q.quoteItems?.length > 0 && (
                          <span>
                            • {q.quoteItems.length} article{q.quoteItems.length > 1 ? 's' : ''}
                          </span>
                        )}
                      </div>
                      {isExpired && (
                        <p className="text-[10px] text-amber-600 mt-1">
                          Expiré le {new Date(q.validUntil).toLocaleDateString('fr-FR')}
                        </p>
                      )}
                    </div>
                    <div className="text-right shrink-0">
                      ...
                      <p className="text-[10px] text-gray-400">
                        {new Date(q.createdAt || q.date).toLocaleDateString('fr-FR')}
                      </p>
                    </div>
