...
export default function QuoteDetailPage() {
  ...
  return (
    <div className="animate-fade-in space-y-6 max-w-4xl">
      <PageHeader
        title={quote.quoteNumber || `#${id.slice(0, 8)}`}
        description={`Créé le ${new Date(quote.createdAt).toLocaleDateString('fr-FR', {
          day: 'numeric',
          month: 'long',
          year: 'numeric',
        })}`}
        breadcrumbs={[
          ...
          { label: quote.quoteNumber || 'Devis' },
        ]}
        actions={
          <Badge variant={isExpired ? 'warning' : s.variant} size="sm">
      ...
      <Card className="p-4">
        <div className="flex flex-wrap items-center gap-2">
          ...
          {quote.invoice && (
            <Link
              href={`/dashboard/finance/invoices/${quote.invoice.id}`}
              ...
              <div className="py-2 border-t border-gray-200 dark:border-gray-700">
                ...
                {Number(quote.taxAmount || 0) > 0 && (
                  <div className="flex justify-between text-sm">
                    ...
                    <span>{formatPrice(Number(quote.taxAmount))}</span>
                  </div>
                )}
                {Number(quote.discountAmount || 0) > 0 && (
                  <div className="flex justify-between text-sm">
                    ...
                    <span className="text-red-500">
                      -{formatPrice(Number(quote.discountAmount))}
                    </span>
                  </div>
                )}
                <div className="flex justify-between text-base font-bold text-gray-900 dark:text-white mt-1 pt-2 border-t border-gray-200 dark:border-gray-700">
                  <span>Total</span>
                  <span>{formatPrice(Number(quote.totalAmount || 0))}</span>
                </div>
              </div>
            </div>
          )}
          ...
          <Card className="p-4">
            ...
            <div className="space-y-2 text-xs text-gray-500">
              <div className="flex justify-between">
                <span>Statut</span>
                <span className={cn('font-medium', s.color)}>{isExpired ? 'Expiré' : s.label}</span>
              </div>
              <div className="flex justify-between">
