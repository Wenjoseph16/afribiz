          ...
          <Card className="p-4">
            ...
            <div className="grid grid-cols-2 gap-4 text-sm">
              <div className="p-3 rounded-xl bg-amber-50 dark:bg-amber-900/20">
                <p className="text-[10px] text-amber-600 uppercase font-semibold">Reste dû</p>
                <p className="text-lg font-bold text-amber-700">{formatPrice(remaining)}</p>
              </div>
              <div className="p-3 rounded-xl bg-emerald-50 dark:bg-emerald-900/20">
                <p className="text-[10px] text-emerald-600 uppercase font-semibold">Payé</p>
                <p className="text-lg font-bold text-emerald-700">{formatPrice(paid)}</p>
              </div>
            </div>
          </Card>
                  ...
                  >
                    <div>
                      <p className="text-xs font-medium text-gray-900 dark:text-white">
                        Rappel {r.type === 'OVERDUE' ? 'retard' : 'échéance'}
                      </p>
                    ...
                    <Badge variant={r.status === 'SENT' ? 'success' : 'warning'} size="xs">
                      {r.status}
                    </Badge>
                  </div>
                ))}
          ...
          {(debt.order || debt.invoice) && (
            <Card className="p-4">
              ...
              <div className="space-y-2">
                {debt.order && (
                  <Link
                    href={`/dashboard/orders/${debt.order.id}`}
                    className="flex items-center gap-2 p-2 rounded-lg hover:bg-gray-50 dark:hover:bg-gray-800 text-xs text-brand font-medium"
                  >
                    <FileText className="w-3.5 h-3.5" />
                    Commande {debt.order.orderNumber}
                  </Link>
                )}
                {debt.invoice && (
                  <Link
                    href={`/dashboard/finance/invoices/${debt.invoice.id}`}
                    className="flex items-center gap-2 p-2 rounded-lg hover:bg-gray-50 dark:hover:bg-gray-800 text-xs text-brand font-medium"
                  >
                    <FileText className="w-3.5 h-3.5" />
                    Facture {debt.invoice.invoiceNumber}
                  </Link>
                )}
              </div>
            </Card>
