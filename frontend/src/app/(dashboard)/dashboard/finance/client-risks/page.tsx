...
export default function ClientRisksPage() {
  ...
  const highRisk = allRisks.filter(
    ...
    <div className="animate-fade-in space-y-6">
      ...
      <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
        <Card className="p-3">
          <div className="flex items-center gap-2">
            ...
            <div>
              ...
              <p className="text-sm font-bold">{allRisks.length}</p>
            </div>
          </div>
        </Card>
        <Card className="p-3">
          <div className="flex items-center gap-2">
            ...
            <div>
              <p className="text-[10px] text-gray-500">Faible risque</p>
              <p className="text-sm font-bold">
                {allRisks.filter((r: any) => r.riskLevel === 'LOW').length}
              </p>
            </div>
          </div>
        </Card>
        <Card className="p-3">
          <div className="flex items-center gap-2">
            ...
            <div>
              <p className="text-[10px] text-gray-500">Moyen</p>
              <p className="text-sm font-bold">
                {allRisks.filter((r: any) => r.riskLevel === 'MEDIUM').length}
              </p>
            </div>
          </div>
        </Card>
        <Card className="p-3">
          <div className="flex items-center gap-2">
            ...
            <div>
              <p className="text-[10px] text-gray-500">Haut risque</p>
              <p className="text-sm font-bold">{highRisk.length}</p>
            </div>
          </div>
        </Card>
      </div>
      ...
      {allRisks.length === 0 ? (
        <Card className="text-center py-12">
          ...
          <p className="text-sm text-gray-500">Aucun client évalué</p>
        </Card>
      ) : (
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3">
          {allRisks.map((risk: any) => {
            ...
            return (
              <Card
                key={risk.id}
                className={cn('p-4', risk.blacklisted && 'ring-2 ring-red-300 dark:ring-red-700')}
              >
                <div className="flex items-start justify-between mb-3">
                  <div className="flex items-center gap-2">
                    <div className={cn('p-1.5 rounded-lg', r.color)}>
                      <User className="w-4 h-4" />
                    </div>
                    <div>
                      <p className="text-sm font-semibold text-gray-900 dark:text-white">
                        {risk.customerName || 'Client'}
                      </p>
                      {risk.customerPhone && (
                        <p className="text-[10px] text-gray-400">{risk.customerPhone}</p>
                      )}
                    </div>
                  </div>
                ...
                <div className="grid grid-cols-2 gap-2 text-xs text-gray-500 mb-3">
                  <span className="flex items-center gap-1">
                    ...
                    {risk.incidentCount || 0} incidents
                  </span>
                  {risk.requireDeposit && (
                ...
                <div className="flex gap-1 flex-wrap">
                  {['LOW', 'MEDIUM', 'HIGH', 'CRITICAL'].map((level) => (
                    <button
                      key={level}
                      onClick={() => updateRiskLevel(risk.id, level)}
                      className={cn(
                  ...
                  {risk.blacklisted && (
                    <span className="px-2 py-0.5 rounded text-[9px] font-medium bg-red-100 text-red-700">
                      Blacklisté
                    </span>
                  )}
                </div>
