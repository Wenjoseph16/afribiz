...
export default function PromoStatsPage() {
  ...
  const promos = useMemo(() => {
    const raw = Array.isArray(promosData)
      ? promosData
      : ((promosData?.promotions || promosData?.data || []) as any[]);
    return raw;
  }, [promosData]);

  const coupons = useMemo(() => {
    const raw = Array.isArray(couponsData)
      ? couponsData
      : ((couponsData?.coupons || couponsData?.data || []) as any[]);
    return raw;
  }, [couponsData]);

  const bundles = useMemo(() => {
    const raw = Array.isArray(bundlesData)
      ? bundlesData
      : ((bundlesData?.bundles || bundlesData?.data || []) as any[]);
    return raw;
  }, [bundlesData]);

  const campaigns = useMemo(() => {
    const raw = Array.isArray(campaignsData)
      ? campaignsData
      : ((campaignsData?.campaigns || campaignsData?.data || []) as any[]);
    return raw;
  }, [campaignsData]);
  ...
  const activePromos = promos.filter(
    (p: any) => p.isActive && (!p.endsAt || new Date(p.endsAt) > now)
  );
  ...
  const typeDistribution = [
    {
      label: 'Pourcentage',
      count: percentagePromos.length,
      pct: promos.length > 0 ? Math.round((percentagePromos.length / promos.length) * 100) : 0,
      color: 'bg-brand',
    },
    {
      label: 'Montant fixe',
      count: fixedPromos.length,
      pct: promos.length > 0 ? Math.round((fixedPromos.length / promos.length) * 100) : 0,
      color: 'bg-blue-500',
    },
    {
      label: 'Livraison offerte',
      count: shippingPromos.length,
      pct: promos.length > 0 ? Math.round((shippingPromos.length / promos.length) * 100) : 0,
      color: 'bg-emerald-500',
    },
    {
      label: 'Autres',
      count: promos.length - percentagePromos.length - fixedPromos.length - shippingPromos.length,
      pct:
        promos.length > 0
          ? Math.round(
              ((promos.length -
                percentagePromos.length -
                fixedPromos.length -
            ...
            <div className="space-y-3">
              <div className="grid grid-cols-2 gap-2 text-center">
                <div className="p-2 rounded-lg bg-emerald-50 dark:bg-emerald-900/20">
                  ...
                  <p className="text-[9px] text-emerald-600">Actives</p>
                </div>
                <div className="p-2 rounded-lg bg-blue-50 dark:bg-blue-900/20">
                  <p className="text-xs font-bold text-blue-700">
                    {campaigns.filter((c: any) => c.status === 'COMPLETED').length}
                  </p>
                  <p className="text-[9px] text-blue-600">Terminées</p>
                </div>
              </div>
        ...
        <Card className="p-4">
          ...
          <div className="space-y-3">
            ...
            <div className="flex items-center justify-between p-2 rounded-lg bg-gray-50 dark:bg-gray-800/50">
              ...
              <span className="text-sm font-bold text-gray-900 dark:text-white">
                {coupons.filter((c: any) => c.isActive !== false).length}
              </span>
            </div>
            <div className="flex items-center justify-between p-2 rounded-lg bg-gray-50 dark:bg-gray-800/50">
          ...
          <div className="p-3 rounded-xl bg-white/60 dark:bg-gray-800/60">
            ...
            <p className="text-[10px] text-amber-600 dark:text-amber-400 mt-1">
              {promos.length === 0
                ...
                : `${promos.length} promotions actives — pensez à en créer pour vos nouveaux produits`}
            </p>
          </div>
          <div className="p-3 rounded-xl bg-white/60 dark:bg-gray-800/60">
            <p className="text-xs font-medium text-amber-800 dark:text-amber-300">
            ...
            <p className="text-[10px] text-amber-600 dark:text-amber-400 mt-1">
              {coupons.length > 0
                ? `${coupons.length} coupons disponibles — envoyez des codes personnalisés à vos meilleurs clients`
                : 'Créez des coupons pour fidéliser votre clientèle'}
            </p>
          </div>
          <div className="p-3 rounded-xl bg-white/60 dark:bg-gray-800/60">
            <p className="text-xs font-medium text-amber-800 dark:text-amber-300">
            ...
            <p className="text-[10px] text-amber-600 dark:text-amber-400 mt-1">
              {campaigns.length === 0
                ? 'Lancez une campagne marketing pour augmenter votre visibilité'
                : 'Analysez les performances de vos campagnes pour optimiser votre budget'}
            </p>
          </div>
        </div>
      </Card>
