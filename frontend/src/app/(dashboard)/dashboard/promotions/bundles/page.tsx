...
export default function BundlesPage() {
  ...
  const { data: bundlesData, isLoading, error, refetch } = usePromoBundles({ limit: 100 });
  const { data: productsData } = useMyProducts({ limit: 200 });
  const allBundles: any[] = Array.isArray(bundlesData)
    ? bundlesData
    : bundlesData?.bundles || bundlesData?.data || [];
    ...
    <div className="animate-fade-in space-y-6">
      ...
      {filtered.length === 0 ? (
        <Card className="text-center py-12">
          ...
          <p className="text-sm text-gray-500">Aucun bundle trouvé</p>
        </Card>
      ) : (
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3">
          {filtered.map((bundle: any) => {
            ...
            const totalOriginal = items.reduce(
              (s: number, i: any) =>
                s + Number(i.product?.price || i.unitPrice || 0) * (i.quantity || 1),
              0
            );
              ...
              <Card key={bundle.id} className="p-4">
                ...
                {items.length > 0 && (
                  <div className="space-y-1 mb-3">
                    ...
                    {items.length > 4 && (
                      <p className="text-[10px] text-gray-400">+{items.length - 4} autres</p>
                    )}
                  </div>
                )}
                <div className="flex items-center justify-between pt-3 border-t border-gray-100 dark:border-gray-700">
                  <div>
                    <p className="text-sm font-bold text-gray-900 dark:text-white">
                      {formatPrice(Number(bundle.price || 0))}
                    </p>
                    {savings > 0 && (
                  ...
                  <Badge variant={bundle.isActive !== false ? 'success' : 'warning'} size="xs">
            ...
            <form onSubmit={handleCreate} className="space-y-4">
              ...
              <div>
                ...
                >
                  ...
                  {products
                    .filter((p: any) => !form.items.find((i) => i.productId === p.id))
                    .map((p: any) => (
              ...
              {form.items.length > 0 && (
                <div className="p-3 rounded-xl bg-amber-50 dark:bg-amber-900/20 text-xs">
                  <p className="text-amber-700 dark:text-amber-400">
                    Prix total des produits: <strong>{formatPrice(bundleTotal)}</strong>
                  </p>
                  {Number(form.price) < bundleTotal && (
                    <p className="text-emerald-600 mt-1">
                      Économie client: {formatPrice(bundleTotal - Number(form.price))}
                    </p>
                  )}
                </div>
              )}
