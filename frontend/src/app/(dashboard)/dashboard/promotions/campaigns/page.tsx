...
export default function CampaignsPage() {
  ...
  const allCampaigns: PromoCampaign[] = Array.isArray(campaignsData)
    ? campaignsData
    : campaignsData?.campaigns || campaignsData?.data || [];

  const filtered = allCampaigns.filter((c: PromoCampaign) => {
    ...
    <div className="animate-fade-in space-y-6">
        ...
        <Card className="p-3">
          <div className="flex items-center gap-2">
            ...
            <div>
              ...
              <p className="text-sm font-bold">{allCampaigns.length}</p>
            </div>
          </div>
        </Card>
        <Card className="p-3">
          <div className="flex items-center gap-2">
            ...
            <div>
              ...
              <p className="text-sm font-bold">
                {allCampaigns.filter((c: PromoCampaign) => c.status === 'ACTIVE').length}
              </p>
            </div>
          </div>
        </Card>
        <Card className="p-3">
          <div className="flex items-center gap-2">
            ...
            <div>
              ...
              <p className="text-sm font-bold">
                {allCampaigns.filter((c: PromoCampaign) => c.status === 'COMPLETED').length}
              </p>
            </div>
          </div>
        </Card>
        <Card className="p-3">
          <div className="flex items-center gap-2">
            ...
            <div>
              ...
              <p className="text-sm font-bold">
                {formatPrice(
                  allCampaigns.reduce((s: number, c: any) => s + Number(c.budget || 0), 0)
                )}
              </p>
            </div>
          </div>
      ...
      {filtered.length === 0 ? (
        <Card className="text-center py-12">
          <Megaphone className="h-12 w-12 text-gray-200 dark:text-gray-700 mx-auto mb-3" />
          <p className="text-sm text-gray-500">Aucune campagne trouvée</p>
        </Card>
      ) : (
        <div className="space-y-2">
          {filtered.map((campaign: PromoCampaign) => {
                  ...
                  <div className="flex items-start gap-3 flex-1">
                    ...
                    <div>
                      ...
                      <div className="flex items-center gap-3 mt-2 text-[10px] text-gray-400 flex-wrap">
                        ...
                        {campaign.budget && (
                          <span>Budget: {formatPrice(Number(campaign.budget))}</span>
                        )}
                      </div>
                    </div>
                  </div>
                  <div className="text-right shrink-0">
                    ...
                    {campaign.spent && (
                      <p className="text-[10px] text-gray-400">
                        Dépensé: {formatPrice(Number(campaign.spent))}
                      </p>
                    )}
                  </div>
                </div>
            ...
            <form onSubmit={handleCreate} className="space-y-4">
              ...
              <div>
                ...
                <input
                  ...
                  placeholder="Ex: Nouveaux clients, VIP, Tous"
                  className="w-full p-2.5 text-sm border border-gray-200 dark:border-gray-700 rounded-xl bg-transparent dark:text-gray-100"
                />
              </div>
