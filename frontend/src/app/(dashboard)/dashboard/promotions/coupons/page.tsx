...
export default function CouponsPage() {
  ...
  const allCoupons: CouponItem[] = Array.isArray(couponsData)
    ? couponsData
    : couponsData?.coupons || couponsData?.data || [];

  const filtered = allCoupons.filter((c: CouponItem) => {
    if (filter !== 'all' && c.isActive?.toString() !== filter) return false;
    if (search) {
    ...
    <div className="animate-fade-in space-y-6">
      ...
      {filtered.length === 0 ? (
        <Card className="text-center py-12">
          <Tag className="h-12 w-12 text-gray-200 dark:text-gray-700 mx-auto mb-3" />
          <p className="text-sm text-gray-500">Aucun coupon trouvé</p>
        </Card>
      ) : (
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3">
          {filtered.map((coupon: CouponItem) => {
                ...
                <div className="space-y-1 text-xs text-gray-500">
                  ...
                  {coupon.expiresAt && (
                    <p className="flex items-center gap-1">
                      ...
                      Expire: {new Date(coupon.expiresAt).toLocaleDateString('fr-FR')}
                    </p>
                  )}
                </div>
                <div className="mt-3">
                  <Badge
                    variant={
                      isExpired ? 'danger' : coupon.isActive !== false ? 'success' : 'warning'
                    }
                    size="xs"
                  >
          ...
          >
            ...
            <form onSubmit={handleCreate} className="space-y-4">
              ...
              <div className="flex gap-2 pt-2">
                ...
                <button
                  type="submit"
                  className="flex-1 py-2 bg-brand text-white rounded-xl text-sm font-medium hover:bg-brand/90"
                >
                  Créer le coupon
                </button>
              </div>
