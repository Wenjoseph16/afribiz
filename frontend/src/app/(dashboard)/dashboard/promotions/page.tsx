...
export default function PromotionsPage() {
  const {
    ...
    queryFn: async () => {
      try {
        ...
        // Fallback: peut-être que l'utilisateur n'est pas BUSINESS
        return [];
      }
    },
  });
  ...
  const allPromos: PromotionItem[] = Array.isArray(promosData)
    ? promosData
    : promosData?.promotions || promosData?.data || [];

  const stats = {
    total: allPromos.length,
    active: allPromos.filter((p: any) => p.isActive).length,
    campaigns: allPromos.filter((p: any) => p.discountType === 'BUY_X_GET_Y').length,
  };
  ...
  const filtered = useMemo(() => {
    ...
    switch (activeTab) {
      case 'active':
        f = f.filter((p) => p.isActive && (!p.endsAt || new Date(p.endsAt) > now));
        break;
      case 'inactive':
        f = f.filter((p) => !p.isActive);
        break;
      case 'expired':
        f = f.filter((p) => p.endsAt && new Date(p.endsAt) <= now);
        break;
    }
    if (searchQuery) {
    ...
    if (isForbidden) {
      return (
        <div className="space-y-6 animate-fade-in">
          <PageHeader
            title="Promotions"
            description="Gérez vos offres promotionnelles et codes de réduction"
            breadcrumbs={[{ label: 'Promotions' }]}
          />
          <Card className="text-center py-12">
            <Percent className="h-12 w-12 text-amber-400 mx-auto mb-3" />
            <h3 className="text-lg font-semibold text-gray-900 dark:text-gray-100 mb-2">
              Module Promotions non activé
            </h3>
            <p className="text-sm text-gray-500 dark:text-gray-400 max-w-md mx-auto">
              Activez le module Promotions dans les paramètres de votre abonnement pour créer et
              gérer vos offres promotionnelles.
            </p>
          </Card>
        </div>
      );
  ...
  const handleDelete = (_id: string) => {
    // Les clients ne peuvent pas supprimer les promotions
  };
    ...
    <div className="space-y-6 animate-fade-in">
      <div className="flex flex-col sm:flex-row items-start justify-between gap-4">
        <div>
          ...
          <p className="text-sm text-gray-500 dark:text-gray-400 mt-1">
            Gérez vos offres promotionnelles et codes de réduction
          </p>
        </div>
      ...
      <div className="grid grid-cols-2 sm:grid-cols-4 gap-4">
        ...
        <StatsCard
          ...
          value={allPromos.length > 0 ? `${Math.round((stats.active / stats.total) * 100)}%` : '0%'}
        />
      </div>
      ...
      {allPromos.length > 0 &&
        (() => {
          const active = allPromos.filter(
            (p) => p.isActive && (!p.endsAt || new Date(p.endsAt) > now)
          );
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
              {suggestions.map((s, i) => {
                ...
                return (
                  <Link
                    key={i}
                    href={suggestion.link || '/dashboard/promotions'}
                    className={`flex items-start gap-3 p-4 rounded-xl border-l-4 ${colorMap[suggestion.color]} border border-gray-200 dark:border-gray-700 hover:shadow-sm transition-all duration-200`}
                  >
                    <div className="p-2 rounded-lg bg-white dark:bg-gray-800 shadow-sm shrink-0">
                      <suggestion.icon className="h-4 w-4 text-gray-600 dark:text-gray-300" />
                    </div>
        ...
        <Card className="text-center py-12">
          ...
          <p className="text-sm text-gray-500 dark:text-gray-400 mb-4">
            {searchQuery
              ...
              : 'Découvrez les offres des entreprises partenaires sur le marketplace'}
          </p>
...
function PromoCard({ promo, onDelete }: { promo: PromotionItem; onDelete: (id: string) => void }) {
  const now = new Date();
  ...
  return (
    <div className="group bg-white dark:bg-gray-800 rounded-2xl border border-gray-200 dark:border-gray-700 overflow-hidden hover:border-brand/30 hover:shadow-sm transition-all duration-200">
      <div className="aspect-video bg-gradient-to-br from-brand-50 to-amber-50 dark:from-brand-900/20 dark:to-amber-900/20 flex items-center justify-center relative">
        {badge && (
          <span className="absolute top-2 right-2 text-[10px] font-medium px-1.5 py-0.5 rounded-full bg-white/90 dark:bg-gray-900/90 shadow-sm">
            {badge.label}
          </span>
        )}
      ...
      <div className="p-4 space-y-2">
        ...
        <div className="flex items-center gap-2 text-xs text-gray-500">
          ...
          {typeLabels[promo.discountType] || promo.discountType} · {promo.discountValue}
          {promo.discountType === 'PERCENTAGE' ? '%' : ' FCFA'}
        </div>
        {promo.code && (
          <div className="flex items-center gap-2 text-xs">
            <span className="font-mono px-2 py-0.5 rounded bg-gray-100 dark:bg-gray-700 text-brand font-semibold">
              {promo.code}
            </span>
          </div>
        )}
...
function PromoRow({ promo, onDelete }: { promo: PromotionItem; onDelete: (id: string) => void }) {
  const now = new Date();
  const endsAt = promo.endsAt ? new Date(promo.endsAt) : null;
  const isExpired = endsAt && endsAt <= now;
  const active = promo.isActive && !isExpired;
    ...
    <tr className="hover:bg-gray-50 dark:hover:bg-gray-700/30 transition-colors group">
      <td className="p-4">
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            {badge && (
              <span className="text-[10px] font-medium px-1.5 py-0.5 rounded-full mt-0.5 inline-block bg-gray-100 dark:bg-gray-700">
                {badge.label}
              </span>
            )}
          </div>
      ...
      <td className="p-4">
        <span className="text-sm text-gray-600 dark:text-gray-300">
          {typeLabels[promo.discountType] || promo.discountType}
        </span>
      </td>
      <td className="p-4">
        {promo.code ? (
      ...
      <td className="p-4">
        <span className="text-sm text-gray-600 dark:text-gray-300">
          {startDate
            ...
            : 'N/A'}
          {endsAt
            ? ` - ${endsAt.toLocaleDateString('fr-FR', { day: 'numeric', month: 'short', year: 'numeric' })}`
            : ''}
        </span>
      </td>
