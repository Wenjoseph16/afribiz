...
export default function OrderStatsPage() {
  ...
  const allOrders = useMemo(() => {
    const raw = Array.isArray(ordersData)
      ? ordersData
      : ordersData?.orders || ordersData?.data || [];
    return raw.map((o: any) => ({
      ...o,
      amountNum: Number(o.totalAmount || 0),
      createdAtObj: new Date(o.createdAt || o.date),
    }));
  }, [ordersData]);
  ...
  const stats = useMemo(() => {
    ...
    const cancelled = allOrders.filter((o: any) =>
      ['CANCELLED', 'REFUSED'].includes(o.status)
    ).length;
    ...
    allOrders.forEach((o: any) => {
      ...
      if (last30Days[key] !== undefined) {
        last30Days[key] += o.amountNum;
        dayCounts[key]++;
      }
    });

    const spendingChart = Object.entries(last30Days).map(([date, amount]) => ({
      date,
    ...
    const statusDist = Object.entries(STATUS_LABELS).map(([key, label]) => ({
      key,
      label,
      count: allOrders.filter((o: any) => o.status === key).length,
    }));
      ...
      <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-6 gap-3">
        ...
        <Card className="p-3">
          <div className="flex items-center gap-3">
            ...
            <div>
              <p className="text-[10px] text-gray-500 uppercase">Dépensé</p>
        ...
        <Card className="p-3">
          <div className="flex items-center gap-3">
            <div
              className={cn(
                'p-2 rounded-lg',
                stats.avgValue > 5000 ? 'bg-emerald-100' : 'bg-amber-100'
              )}
            >
              <TrendingUp
                className={cn(
                  'w-4 h-4',
                  stats.avgValue > 5000 ? 'text-emerald-600' : 'text-amber-600'
                )}
              />
            </div>
            <div>
              ...
              <p className="text-lg font-bold text-gray-900 dark:text-white">
                {formatPrice(Math.round(stats.avgValue))}
              </p>
            </div>
          </div>
        </Card>
        <Card className="p-3">
          <div className="flex items-center gap-3">
            ...
            <div>
              <p className="text-[10px] text-gray-500 uppercase">Reçues</p>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <Card className="p-4 lg:col-span-2">
          <div className="flex items-center justify-between mb-4">
            ...
            <span className="text-[10px] text-gray-400">{formatPrice(stats.totalSpent)} total</span>
          </div>
          <div className="flex items-end gap-0.5 h-32 sm:h-40">
            {stats.spendingChart.map((day) => (
              ...
              >
                ...
                <div
                  className={cn(
                    'w-full rounded-t-sm transition-all duration-200 hover:opacity-80 cursor-pointer',
                    day.amount > 0 ? 'bg-brand' : 'bg-gray-100 dark:bg-gray-800'
                  )}
                  style={{
                    height: `${Math.max((day.amount / stats.maxAmount) * 100, day.amount > 0 ? 2 : 0)}%`,
                  }}
                />
                <span className="text-[7px] text-gray-400 mt-1 hidden sm:block">
              ...
              .map((s) => (
                <div key={s.key}>
                  ...
                  <div className="w-full h-2 bg-gray-100 dark:bg-gray-800 rounded-full overflow-hidden">
                    <div
                      className={cn('h-full rounded-full', STATUS_COLORS[s.key] || 'bg-gray-400')}
              ...
              <div key={t.key}>
                ...
                <div className="w-full h-2 bg-gray-100 dark:bg-gray-800 rounded-full overflow-hidden">
                  <div
                    ...
                    style={{ width: `${(t.count / stats.typeDist[0]?.count || 1) * 100}%` }}
                  />
                </div>
              </div>
            ))}
