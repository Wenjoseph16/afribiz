...
export default function PublicBookingPage() {
  ...
  useEffect(() => {
    ...
    const fetchData = async () => {
      setLoading(true);
      try {
        const res = await apiClient.get(`/public/businesses/${slug}/booking-info`);
        if (res.data.success) {
    ...
    try {
      const res = await apiClient.post('/public/bookings', {
        ...
        title: selectedService
          ? `${selectedService}`
          : `Réservation ${businessData?.business?.name}`,
    ...
    return (
      <div className="min-h-screen bg-gradient-to-br from-emerald-50 via-white to-teal-50 dark:from-gray-950 dark:via-gray-900 dark:to-emerald-950 flex items-center justify-center">
        <div className="text-center max-w-md mx-auto p-8">
          ...
          <Link
            href="/"
            className="text-sm font-medium text-emerald-600 hover:text-emerald-700 flex items-center gap-1 justify-center"
          >
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-emerald-50 via-white to-teal-50 dark:from-gray-950 dark:via-gray-900 dark:to-emerald-950">
      ...
      <main className="max-w-4xl mx-auto px-4 py-8">
            ...
            <div key={s.key} className="flex items-center gap-2">
              <div
                className={cn(
                  ...
                  step === s.key
                    ...
                    : ['confirm', 'form'].includes(step) && ['form', 'confirm'].includes(s.key)
                      ? 'bg-emerald-100 text-emerald-700'
                      : 'bg-gray-100 dark:bg-gray-800 text-gray-400'
                )}
              >
                {['confirm', 'form'].includes(step) &&
                ['form', 'confirm'].includes(s.key) &&
                s.key !== step ? (
              ...
              <span
                className={cn(
                  ...
                  step === s.key ? 'text-gray-900 dark:text-white' : 'text-gray-400'
                )}
              >
                {s.label}
              </span>
        ...
        {step === 'info' && (
          <div className="space-y-6">
            <div className="bg-white dark:bg-gray-900 rounded-2xl border border-gray-200 dark:border-gray-700 p-4 sm:p-6 shadow-sm">
              ...
              <div className="flex items-center justify-between mb-4">
                ...
                <span className="text-sm font-medium text-gray-700 dark:text-gray-300">
                  {weekDays[0].toLocaleDateString('fr-FR', { month: 'long' })}{' '}
              ...
              <div className="grid grid-cols-7 gap-2">
                {weekDays.map((date, i) => {
                  ...
                  return (
                    <button
                      key={i}
                      onClick={() => available && !isPast && handleDateSelect(date)}
                      disabled={!available || isPast}
                      className={cn(
                        'p-2 rounded-xl text-center transition-all',
                        isSelected
                          ? 'bg-emerald-600 text-white shadow-lg shadow-emerald-200 dark:shadow-emerald-900/30 scale-105'
                          : available && !isPast
            ...
            {selectedDate && (
              <div className="bg-white dark:bg-gray-900 rounded-2xl border border-gray-200 dark:border-gray-700 p-4 sm:p-6 shadow-sm animate-fade-in">
                <h2 className="text-lg font-semibold text-gray-900 dark:text-white mb-1">
                        ...
                        >
                          ...
                          {s.price > 0 && (
                            <p className="text-[10px] text-emerald-600 font-medium mt-0.5">
                              {Number(s.price).toLocaleString()} FCFA
                            </p>
                          )}
                          {s.duration && (
                ...
                <div className="mb-4">
                  ...
                  <div className="flex items-center gap-2">
                    <button
                      ...
                      className="w-8 h-8 rounded-lg border border-gray-200 dark:border-gray-700 flex items-center justify-center text-gray-600 dark:text-gray-400 hover:bg-gray-100 dark:hover:bg-gray-800"
                    >
                      -
                    </button>
                ...
                <div className="grid grid-cols-3 sm:grid-cols-4 md:grid-cols-5 gap-2">
                  {selectedDateSlots.map((slot: any, i: number) => (
                    <button
                      ...
                      className={cn(
                        'p-2.5 rounded-xl text-center transition-all border text-xs font-medium',
                        selectedSlot === slot
        ...
        {step === 'form' && (
          <div className="bg-white dark:bg-gray-900 rounded-2xl border border-gray-200 dark:border-gray-700 p-4 sm:p-6 shadow-sm">
            ...
            <p className="text-sm text-gray-500 mb-6">
              Laissez-nous vos coordonnées pour finaliser la réservation
            </p>
            ...
            <div className="flex items-center gap-3">
              ...
              <div className="flex-1">
                <p className="text-sm font-medium text-gray-900 dark:text-white">
                  Vous préférez réserver autrement ?
                </p>
                <p className="text-xs text-gray-500">Contactez-nous directement sur WhatsApp</p>
              </div>
              <a
                href={`https://wa.me/${business.phone.replace(/[^0-9]/g, '')}?text=Bonjour%20${encodeURIComponent(business.name)}%2C%20je%20souhaite%20r%C3%A9server.`}
                target="_blank"
                rel="noopener noreferrer"
      ...
      <footer className="text-center py-6 text-xs text-gray-400">
        <p>
          Propulsé par <span className="font-semibold text-emerald-600">AfriBiz</span> —
