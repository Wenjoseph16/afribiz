...
export default function CheckoutPage() {
  ...
  const deliveryFree =
    deliveryInfo?.business?.minDeliveryAmount !== null &&
    subtotal >= (deliveryInfo?.business?.minDeliveryAmount ?? Infinity);
  const deliveryFee =
    ...
    return (
      <div className="min-h-[60vh] flex items-center justify-center px-4">
        ...
        >
          ...
          <p className="text-gray-500 dark:text-white/40 mb-8 max-w-md">
            Ajoutez des produits au panier pour finaliser votre commande.
          </p>
  ...
  const handleCheckout = async () => {
    ...
    try {
      ...
      const refCode =
        (typeof window !== 'undefined' && localStorage.getItem('afribiz_ref_code')) || undefined;
      ...
      if (isGuest) {
        ...
        if (res.data.success) {
          ...
          notify({
            ...
            description: 'Votre commande a été transmise au marchand.',
            variant: 'success',
          });
        } else {
      ...
      <div className="max-w-2xl mx-auto py-16 px-4 text-center">
        ...
        >
          ...
          <p className="text-gray-500 dark:text-white/40 mb-8 max-w-md mx-auto">
            Votre commande a été transmise avec succès. Vous recevrez une notification dès
  ...
  const deliveryOptions = [
    {
      value: 'DELIVERY',
      label: 'Livraison',
      description: zones.length > 0 ? `${zones.length} zone(s)` : 'À domicile',
      icon: <Truck className="w-4 h-4" />,
      disabled: !(deliveryInfo?.business?.deliveryEnabled ?? true),
    },
    {
      value: 'PICKUP',
      label: 'Retrait',
      description: 'En boutique',
      icon: <Store className="w-4 h-4" />,
      disabled: !(deliveryInfo?.business?.pickupEnabled ?? true),
    },
  ];
      ...
      >
        ...
        <div>
          ...
          <p className="text-gray-400 dark:text-white/30 text-sm mt-0.5">
            {items.length} article{items.length > 1 ? 's' : ''} · {formatPrice(total, currency)}
          </p>
        </div>
                  ...
                  <div className="p-6">
                    ...
                    <div className="grid grid-cols-2 gap-3 mb-6">
                      {deliveryOptions.map((opt) => (
                        <button
                          ...
                          onClick={() =>
                            patch({
                              deliveryType: opt.value as 'DELIVERY' | 'PICKUP',
                              deliveryZoneId: '',
                            })
                          }
                          className={cn(
                          ...
                          <p
                            className={cn(
                              ...
                              f.deliveryType === opt.value
                                ...
                                : 'text-gray-900 dark:text-white/60'
                            )}
                          >
                            {opt.label}
                          </p>
                    ...
                    {f.deliveryType === 'DELIVERY' && (
                      <div className="space-y-4">
                        {zones.length > 0 && (
                          <div>
                            <label className="block text-xs font-semibold text-gray-500 dark:text-white/50 uppercase tracking-wider mb-2">
                              Quartier / Zone
                            </label>
                        ...
                        <div className="flex items-center gap-2">
                          <button
                            ...
                            className="flex items-center gap-1.5 px-3 py-2 rounded-lg bg-emerald-500/10 border border-emerald-500/20 text-emerald-400 text-xs font-medium hover:bg-emerald-500/20 transition-all duration-200"
                          >
                        ...
                        <div className="flex items-start gap-3">
                          ...
                          <div>
                            <p className="text-sm font-bold text-gray-900 dark:text-white">
                              {deliveryInfo.business.name}
                            </p>
                            <p className="text-xs text-gray-500 dark:text-white/40 mt-0.5">
                              {deliveryInfo.business.address || 'Contactez le marchand'}
                  ...
                  <div className="p-6">
                      ...
                      <div className="mb-5 rounded-xl bg-emerald-500/5 border border-emerald-500/10 p-4 flex items-start gap-3">
                        ...
                        <div>
                          ...
                          <p className="text-[11px] text-gray-400 dark:text-white/30 mt-0.5">
                            Paiement simulé — aucune somme débitée.
                          </p>
                        </div>
                      </div>
                    )}
                    ...
                    <div className="flex gap-3 mt-6 pt-4 border-t border-gray-100 dark:border-white/5">
                      <button
                        ...
                        className="h-12 px-6 rounded-xl bg-gray-100 dark:bg-white/5 border border-gray-200 dark:border-white/10 text-gray-500 dark:text-white/50 text-sm font-medium hover:bg-gray-100 dark:bg-white/10 transition-all duration-200"
                      >
                        Retour
                      </button>
                      <button
                    ...
                    <div key={item.productId} className="flex gap-3">
                      <div className="w-12 h-12 rounded-xl bg-gray-100 dark:bg-white/5 border border-gray-200 dark:border-white/10 shrink-0 overflow-hidden relative">
                        {item.image ? (
                          ...
                          <ShoppingBag className="w-5 h-5 text-gray-300 dark:text-white/15 absolute inset-0 m-auto" />
                        )}
                      </div>
                      <div className="flex-1 min-w-0">
                        ...
                        <p className="text-[11px] text-gray-400 dark:text-white/30">
                          Qte: {item.quantity} × {formatPrice(item.price, currency)}
                        </p>
                      </div>
                      <p className="text-xs font-bold text-emerald-400 tabular-nums whitespace-nowrap">
                  ...
                  <div className="flex justify-between">
                    ...
                    <span
                      className={cn(
                        ...
                        deliveryFee === 0 ? 'text-emerald-400' : 'text-gray-700 dark:text-white/70'
                      )}
                    >
