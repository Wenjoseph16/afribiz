          ...
          {i < 1 && (
            <div
              className={cn(
                ...
                i < idx ? 'bg-emerald-500' : 'bg-white/10'
              )}
            />
          )}
        </div>
  ...
  return (
    <div>
      ...
      <div className="relative">
        ...
        <input
          className="w-full pl-10 pr-4 py-3 rounded-xl bg-white/[0.04] border border-white/[0.08] text-white placeholder:text-white/20 focus:border-emerald-500/40 focus:bg-white/[0.06] focus:ring-0 outline-none transition-all duration-200 text-sm"
          {...props}
        />
      </div>
    </div>
    ...
    return (
      <div className="min-h-[60vh] flex items-center justify-center px-4">
        <motion.div initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} className="text-center">
          ...
          <h1 className="text-2xl font-display font-bold text-white mb-2">Panier vide</h1>
          <p className="text-white/40 mb-8 max-w-md">Ajoutez des produits au panier pour finaliser votre commande.</p>
          <Link href="/marketplace">
            <button className="inline-flex items-center gap-2 px-8 py-4 rounded-full bg-gradient-to-r from-emerald-500 to-emerald-400 text-white font-bold shadow-[0_0_30px_rgba(16,185,129,0.25)] hover:shadow-[0_0_50px_rgba(16,185,129,0.4)] transition-all duration-500 active:scale-[0.98]">
  ...
  if (!isAuthenticated() && !isGuest) {
    return (
      <div className="min-h-[60vh] max-w-md mx-auto flex flex-col items-center justify-center p-6 text-center">
        ...
        <h1 className="text-2xl font-display font-bold text-white mb-2">Finaliser votre commande</h1>
        ...
        <div className="flex flex-col gap-3 w-full">
          <Link href="/login?redirect=/checkout" className="w-full">
            <button className="w-full h-12 rounded-xl bg-gradient-to-r from-emerald-500 to-emerald-400 text-white font-bold text-sm hover:shadow-[0_0_30px_rgba(16,185,129,0.3)] transition-all duration-300">
              Se connecter
            </button>
          </Link>
          <button
            onClick={() => setIsGuest(true)}
            className="w-full h-12 rounded-xl bg-white/5 border border-white/10 text-white/60 text-sm font-medium hover:bg-white/10 hover:border-white/20 transition-all duration-200"
          >
    ...
    return (
      <div className="max-w-2xl mx-auto py-16 px-4 text-center">
        <motion.div initial={{ opacity: 0, scale: 0.8 }} animate={{ opacity: 1, scale: 1 }} transition={{ type: 'spring', stiffness: 200, damping: 15 }}>
          ...
          <h1 className="text-3xl font-display font-bold text-white mb-2">Commande confirmée !</h1>
          ...
          <div className="grid gap-3 sm:grid-cols-2 max-w-md mx-auto">
            ...
            <Link href="/marketplace">
              <button className="w-full h-12 rounded-xl bg-white/5 border border-white/10 text-white/60 text-sm font-medium hover:bg-white/10 transition-all duration-200">
  ...
  return (
    <div className="max-w-6xl mx-auto py-8 px-4">
      ...
      <motion.div initial={{ opacity: 0, y: -10 }} animate={{ opacity: 1, y: 0 }} className="flex items-center gap-4 mb-8">
        <Link href="/cart" className="w-10 h-10 rounded-xl bg-white/5 border border-white/10 flex items-center justify-center text-white/40 hover:text-white hover:bg-white/10 transition-all duration-200">
          <ArrowLeft className="w-5 h-5" />
        </Link>
        <div>
          <h1 className="text-2xl font-display font-bold text-white tracking-tight">Finaliser la commande</h1>
          <p className="text-white/30 text-sm mt-0.5">{items.length} article{items.length > 1 ? 's' : ''} · {formatPrice(total, currency)}</p>
        </div>
                ...
                <div className="glass rounded-2xl">
                  <div className="p-6">
                    <div className="flex items-center gap-3 mb-6">
                      ...
                      <div>
                        <h2 className="text-base font-bold text-white">Mode de réception</h2>
                        <p className="text-xs text-white/30">Comment souhaitez-vous recevoir votre commande ?</p>
                      </div>
                    </div>
                    ...
                    <div className="grid grid-cols-2 gap-3 mb-6">
                      {deliveryOptions.map((opt) => (
                        ...
                        >
                          <div className={cn('w-8 h-8 rounded-lg flex items-center justify-center mb-2', f.deliveryType === opt.value ? 'bg-emerald-500/20 text-emerald-400' : 'bg-white/5 text-white/30')}>
                            {opt.icon}
                          </div>
                          <p className={cn('text-sm font-semibold', f.deliveryType === opt.value ? 'text-white' : 'text-white/60')}>{opt.label}</p>
                    ...
                    {f.deliveryType === 'DELIVERY' && (
                      <div className="space-y-4">
                        {zones.length > 0 && (
                          <div>
                            <label className="block text-xs font-semibold text-white/50 uppercase tracking-wider mb-2">Quartier / Zone</label>
                            <div className="space-y-2">
                              {zones.map((z) => (
                                ...
                                >
                                  ...
                                  <span className={cn('text-xs font-semibold', z.fee === 0 || deliveryFree ? 'text-emerald-400' : 'text-white/40')}>
                    ...
                    {f.deliveryType === 'PICKUP' && deliveryInfo?.business && (
                      <div className="rounded-xl bg-emerald-500/5 border border-emerald-500/10 p-4">
                        <div className="flex items-start gap-3">
                          ...
                          <div>
                            <p className="text-sm font-bold text-white">{deliveryInfo.business.name}</p>
                            <p className="text-xs text-white/40 mt-0.5">{deliveryInfo.business.address || 'Contactez le marchand'}{deliveryInfo.business.city ? ` · ${deliveryInfo.business.city}` : ''}</p>
                    ...
                    <button
                      ...
                      className="w-full mt-6 h-12 rounded-xl bg-gradient-to-r from-emerald-500 to-emerald-400 text-white font-bold text-sm disabled:opacity-40 disabled:cursor-not-allowed hover:shadow-[0_0_30px_rgba(16,185,129,0.3)] transition-all duration-300 active:scale-[0.98] flex items-center justify-center gap-2"
                    >
                ...
                <div className="glass rounded-2xl">
                  <div className="p-6">
                    <div className="flex items-center gap-3 mb-6">
                      ...
                      <div>
                        <h2 className="text-base font-bold text-white">Mode de paiement</h2>
                        <p className="text-xs text-white/30">Choisissez comment payer</p>
                      </div>
                    </div>
                    ...
                    {PAYMENT_DEMO_MODE && (
                      <div className="mb-5 rounded-xl bg-emerald-500/5 border border-emerald-500/10 p-4 flex items-start gap-3">
                        ...
                        <div>
                          <p className="text-xs font-semibold text-white">Mode démo</p>
                          <p className="text-[11px] text-white/30 mt-0.5">Paiement simulé — aucune somme débitée.</p>
                        </div>
                      </div>
                    )}
                    ...
                    <div className="space-y-3">
                      {PAYMENT_METHODS.map((method) => {
                        ...
                        return (
                          ...
                          >
                            <div className={cn('w-10 h-10 rounded-xl flex items-center justify-center', active ? 'bg-emerald-500/20 text-emerald-400' : 'bg-white/5 text-white/30')}>
                              <Icon className="w-5 h-5" />
                            </div>
                            <div className="flex-1">
                              <p className={cn('text-sm font-semibold', active ? 'text-white' : 'text-white/60')}>{method.label}</p>
                    ...
                    <div className="flex gap-3 mt-6 pt-4 border-t border-white/5">
                      <button onClick={() => setStep('info')} className="h-12 px-6 rounded-xl bg-white/5 border border-white/10 text-white/50 text-sm font-medium hover:bg-white/10 transition-all duration-200">
                        Retour
                      </button>
                      <button
              ...
              <div className="relative rounded-[calc(1rem-0.1875rem)] bg-gradient-to-br from-white/[0.02] to-transparent p-5">
                ...
                <div className="space-y-3 mb-5 max-h-[280px] overflow-y-auto pr-1 scrollbar-thin">
                  {items.map((item) => (
                    <div key={item.productId} className="flex gap-3">
                      <div className="w-12 h-12 rounded-xl bg-white/5 border border-white/10 shrink-0 overflow-hidden relative">
                        {item.image ? <Image src={item.image} alt={item.name} fill className="object-cover" sizes="48px" /> : <ShoppingBag className="w-5 h-5 text-white/15 absolute inset-0 m-auto" />}
                      </div>
                      <div className="flex-1 min-w-0">
                        <p className="text-xs font-semibold text-white truncate">{item.name}</p>
                        <p className="text-[11px] text-white/30">Qte: {item.quantity} × {formatPrice(item.price, currency)}</p>
                      </div>
                ...
                <div className="space-y-2 mt-4 text-sm">
                  <div className="flex justify-between">
                    ...
                    <span className="text-white/70 font-medium tabular-nums">{formatPrice(subtotal, currency)}</span>
                  </div>
                  <div className="flex justify-between">
                    <span className="text-white/40">Livraison</span>
                    <span className={cn('font-medium tabular-nums', deliveryFee === 0 ? 'text-emerald-400' : 'text-white/70')}>
                  ...
                  <div className="flex justify-between items-baseline pt-1">
                    ...
                    <span className="text-2xl font-bold text-white tabular-nums">{formatPrice(total, currency)}</span>
                  </div>
                </div>
                ...
                <div className="flex items-center justify-center gap-1.5 mt-4 text-[11px] text-white/15">
