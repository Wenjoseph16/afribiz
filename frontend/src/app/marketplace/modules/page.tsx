...
function TrendingSidebar({ modules }: { modules: ModuleListItem[] }) {
  const topRated = [...modules].sort((a, b) => (b.rating || 0) - (a.rating || 0)).slice(0, 5);
  const mostInstalled = [...modules]
    ...
    <aside className="w-72 shrink-0 hidden xl:block">
      <div className="sticky top-24 space-y-6">
        ...
        <div className="bg-gradient-to-br from-purple-600 to-indigo-700 rounded-xl p-5 text-white">
          ...
          <p className="text-sm text-purple-200/80 mb-4">{modules.length} modules disponibles</p>
          <Link
              ...
              >
                ...
                <div className="min-w-0 flex-1">
                  ...
                  <p className="text-[11px] text-gray-500">
                    {mod.developer?.companyName || 'Développeur'}
                  </p>
                </div>
            ...
            {mostInstalled.map((mod) => (
              ...
              >
                ...
                <div className="min-w-0 flex-1">
                  ...
                  <p className="text-[11px] text-gray-500">
                    {(mod.totalInstalls || 0).toLocaleString()} installs
                  </p>
                </div>
...
export default function ModulesMarketplacePage() {
  ...
  const { data: result, isLoading } = useQuery({
    ...
    queryFn: async () => {
      ...
      return res.data.data as ModulesApiResponse;
    },
  });

  const modules: ModuleListItem[] = result?.data || [];
    ...
    <div className="min-h-screen bg-gray-50">
      ...
      <section className="relative bg-gradient-to-br from-purple-800 via-indigo-700 to-purple-900 overflow-hidden">
        ...
        <div className="relative max-w-7xl mx-auto px-4 py-12 sm:py-16 lg:py-20 text-center">
          <nav className="flex items-center justify-center gap-2 text-sm text-purple-200/70 mb-6">
          ...
          <p className="text-purple-100/80 max-w-2xl mx-auto mb-8 text-base sm:text-lg">
            Découvrez des modules développés par la communauté AfriBiz pour étendre les
            fonctionnalités de votre business.
          </p>
      ...
      <div className="max-w-7xl mx-auto px-4">
        <div className="bg-gradient-to-r from-purple-600 to-indigo-600 rounded-2xl p-5 flex items-center justify-between">
          <div className="text-white">
            <h3 className="font-semibold text-lg">Vous ne trouvez pas ce qu'il vous faut ?</h3>
            <p className="text-purple-100 text-sm">
              Demandez un module sur mesure à notre communauté de développeurs
            </p>
          </div>
          <Link href="/dashboard/business/modules/demands">
            <button className="bg-white text-purple-700 px-5 py-2.5 rounded-xl font-medium text-sm hover:bg-purple-50 transition-colors shadow-lg shadow-purple-900/20 whitespace-nowrap">
              Demander un module
            </button>
          </Link>
        </div>
        ...
        <div className="flex flex-wrap items-center justify-between gap-3">
          <div className="flex items-center gap-3">
            <p className="text-sm text-gray-500">
              <span className="font-semibold text-gray-900">{total}</span> module
              {total !== 1 ? 's' : ''}
        ...
        <div className="flex gap-6">
          ...
          <div className="flex-1 min-w-0">
            {isLoading ? (
              ...
              >
                {modules.map((mod: ModuleListItem) => (
                  <ResultCard
                    key={mod.id}
                    item={{
                      id: mod.id,
                  ...
                  if (start > 1) {
                    return (
                      <>
                        <button
                          onClick={() => setPage(1)}
                          className="w-9 h-9 rounded-lg text-sm font-medium text-gray-600 hover:bg-gray-100 border border-gray-200 transition-colors"
                        >
                          1
                        </button>
