...
export default function GrowthRecommendations({
  ...
  return (
    <Card
      padding="lg"
      title="Recommandations pour grandir"
      titleIcon={<Lightbulb className="h-5 w-5" />}
    >
      <div className="space-y-3">
        {sorted.slice(0, 5).map((rec) => {
          ...
          return (
            <div
              key={rec.id}
              className="flex items-start gap-3 p-3 rounded-xl border border-gray-100 dark:border-gray-700 hover:border-brand/20 transition-colors"
            >
              ...
              <div className="flex-1 min-w-0">
                <div className="flex items-center gap-2">
                  <span className={cn('text-[10px] font-semibold uppercase', cat.color)}>
                    {cat.label}
                  </span>
                  <span
                    className={cn(
                ...
                <Link
                  href={rec.link}
                  className="inline-flex items-center gap-1 text-xs font-semibold text-brand mt-2 hover:underline"
                >
