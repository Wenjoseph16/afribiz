    ...
    <div className={cn('mb-8', className)}>
      {breadcrumbs && breadcrumbs.length > 0 && (
        <nav className="flex items-center gap-1.5 text-sm text-gray-500 dark:text-gray-400 mb-3">
          <Link
            href="/dashboard"
            className="hover:text-brand dark:hover:text-brand-400 transition-colors"
          >
          ...
          {breadcrumbs.map((crumb, i) => (
            <span key={i} className="flex items-center gap-1.5">
              ...
              {crumb.href ? (
                <Link
                  href={crumb.href}
                  className="hover:text-brand dark:hover:text-brand-400 transition-colors font-medium"
                >
                  {crumb.label}
                </Link>
              ) : (
