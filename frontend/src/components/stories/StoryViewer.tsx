...
export function StoryViewer({ groups, initialGroup, onClose }: StoryViewerProps) {
  ...
  const handleStickerClick = (sticker: StorySticker) => {
    setIsPaused(true);
    if (sticker.type === 'LINK' && sticker.value) {
      window.open(sticker.value, '_blank');
    } else if (sticker.type === 'PRODUCT' && sticker.value) {
      window.location.href = '/products/' + sticker.value;
    } else if (sticker.type === 'HASHTAG' && sticker.value) {
      window.location.href = '/search?q=' + encodeURIComponent(sticker.value);
    } else if (sticker.type === 'LOCATION' && sticker.value) {
      window.open(
        'https://maps.google.com/?q=' + encodeURIComponent(sticker.value + ' ' + sticker.label),
        '_blank'
      );
    }
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/98 backdrop-blur-sm animate-fade-in">
      ...
      {showReport && (
        <div
          className="absolute top-16 right-4 z-30 w-64 p-3 rounded-xl bg-white/95 dark:bg-gray-800/95 backdrop-blur-xl border border-white/20 dark:border-gray-700/50 shadow-2xl animate-fade-in"
      ...
      >
        ...
        {/* Stickers overlay */}
        {Array.isArray(currentStory.stickers) && currentStory.stickers.length > 0 && (
        ...
        <div className="absolute bottom-0 left-0 right-0 p-4 sm:p-6 bg-gradient-to-t from-black/85 via-black/40 to-transparent">
          {currentStory.caption && (
            <p className="text-white text-base sm:text-lg font-medium drop-shadow-lg mb-3">
              {currentStory.caption}
            </p>
          )}
          ...
          {hasCommerce && actionConfig && (
            <div className="space-y-2 animate-fade-in-up">
              ...
              <button
                onClick={(e) => {
                  e.stopPropagation();
                  handleCommerceAction(commerce.action, commerce);
                }}
...
function StickerOverlay({
  ...
  onStickerClick: (s: StorySticker) => void;
}) {
  const style: React.CSSProperties = {
    position: 'absolute',
    left: `${sticker.positionX}%`,
    top: `${sticker.positionY}%`,
    transform: sticker.style?.rotation
      ? `rotate(${sticker.style.rotation}deg) translate(-50%, -50%)`
      : 'translate(-50%, -50%)',
    fontSize: sticker.style?.fontSize || '14px',
    zIndex: 10,
    pointerEvents: 'auto',
  };
  ...
  const baseClass =
    'pointer-events-auto cursor-pointer transition-all duration-200 hover:scale-110 active:scale-95 shadow-lg';
  ...
  switch (sticker.type) {
    case 'PRODUCT':
      return (
        <button
          style={style}
          onClick={handleClick}
          className={`${baseClass} flex items-center gap-1.5 px-3 py-1.5 rounded-xl bg-gradient-to-br from-brand-500 to-brand-600 text-white text-xs font-bold border border-white/30 backdrop-blur-sm`}
          title="Voir le produit"
        >
          <ShoppingBag className="w-3.5 h-3.5" />
          <span className="truncate max-w-[100px]">{sticker.label || 'Produit'}</span>
          {sticker.value && (
            <span className="opacity-80">{Number(sticker.value).toLocaleString('fr-FR')} FCFA</span>
          )}
        </button>
      );
    ...
    case 'PROMO':
      return (
        <button
          style={style}
          onClick={handleClick}
          className={`${baseClass} flex items-center gap-1.5 px-3 py-1.5 rounded-xl bg-gradient-to-br from-amber-500 to-orange-600 text-white text-xs font-bold border border-amber-300/30 backdrop-blur-sm`}
          title="Voir la promotion"
        >
          <Percent className="w-3.5 h-3.5" />
          <span className="truncate max-w-[100px]">{sticker.label || 'Promo'}</span>
          {sticker.value && <span className="opacity-80">-{sticker.value}%</span>}
        </button>
      );
    ...
    case 'LINK':
      return (
        <button
          style={style}
          onClick={handleClick}
          className={`${baseClass} flex items-center gap-1.5 px-3 py-1.5 rounded-xl bg-white/20 backdrop-blur-md border border-white/40 text-white text-xs font-medium hover:bg-white/30`}
          title={sticker.value || sticker.label}
        >
          <ExternalLink className="w-3.5 h-3.5" />
          <span className="truncate max-w-[120px]">{sticker.label || 'En savoir plus'}</span>
        </button>
      );
    ...
    case 'POLL':
      return (
        <div
          style={style}
          className="pointer-events-auto min-w-[140px] p-2.5 rounded-xl bg-white/90 dark:bg-gray-800/90 backdrop-blur-md border border-white/30 shadow-xl"
        >
          <p className="text-xs font-semibold mb-2 text-gray-800 dark:text-gray-100 flex items-center gap-1">
            <BarChart3 className="w-3 h-3 text-brand-500" />
            {sticker.label || 'Sondage'}
          </p>
          {(sticker.value ? sticker.value.split(',') : ['Oui', 'Non']).map(
            (opt: string, i: number) => (
              <button
                key={i}
                onClick={(e) => {
                ...
                className="w-full text-left text-xs px-2 py-1.5 mb-1 rounded-lg bg-gray-100 dark:bg-gray-700 text-gray-700 dark:text-gray-300 hover:bg-brand-50 dark:hover:bg-brand-900/30 hover:text-brand-600 transition-colors last:mb-0"
              >
                {opt.trim()}
              </button>
            )
    ...
    case 'QUESTION':
      return (
        <div
          style={style}
          className="pointer-events-auto min-w-[160px] p-2.5 rounded-xl bg-white/90 dark:bg-gray-800/90 backdrop-blur-md border border-white/30 shadow-xl"
        >
          <p className="text-xs font-semibold mb-1.5 text-gray-800 dark:text-gray-100 flex items-center gap-1">
            <HelpCircle className="w-3 h-3 text-brand-500" />
            {sticker.label || 'Question'}
          </p>
          <div className="flex gap-1">
            <input
              type="text"
              placeholder="Votre réponse..."
              className="flex-1 text-xs px-2 py-1.5 rounded-lg bg-gray-100 dark:bg-gray-700 border border-gray-200 dark:border-gray-600 text-gray-800 dark:text-gray-200 placeholder-gray-400 focus:outline-none focus:ring-1 focus:ring-brand-400"
              onClick={(e) => e.stopPropagation()}
            />
            <button
              onClick={(e) => {
              ...
              className="px-2 py-1.5 rounded-lg bg-brand-500 text-white text-xs hover:bg-brand-600 transition-colors"
            >
    ...
    case 'LOCATION':
      return (
        <button
          style={style}
          onClick={handleClick}
          className={`${baseClass} flex items-center gap-1.5 px-2.5 py-1.5 rounded-full bg-white/20 backdrop-blur-md border border-white/40 text-white text-xs font-medium hover:bg-white/30`}
          title={sticker.label}
        >
          <MapPin className="w-3.5 h-3.5 text-red-400" />
          <span className="truncate max-w-[100px]">{sticker.label || 'Localisation'}</span>
        </button>
      );
    ...
    case 'HASHTAG':
      return (
        <button
          style={style}
          onClick={handleClick}
          className={`${baseClass} flex items-center gap-1 px-2.5 py-1.5 rounded-full bg-white/15 backdrop-blur-sm border border-white/30 text-white text-xs font-medium hover:bg-white/25`}
        >
          <Hash className="w-3 h-3" />
          <span>{sticker.label || sticker.value || 'hashtag'}</span>
        </button>
      );
    ...
    default:
      return (
        <div
          style={style}
          className="pointer-events-auto px-3 py-1.5 rounded-lg bg-white/20 backdrop-blur-md border border-white/30 text-white text-xs"
        >
