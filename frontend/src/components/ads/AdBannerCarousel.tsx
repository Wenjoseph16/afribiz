...
export function AdBannerCarousel({
    ...
    if (currentAd) {
      (async () => {
        try {
          ...
          console.error('Erreur tracking carousel:', e);
        }
      })();
    }
  }, [currentIndex]);
  ...
  const trackClick = async () => {
    try {
      ...
      console.error('Erreur click carousel:', e);
    }
  };
            ...
            <div className="absolute inset-0 flex items-center p-6 sm:p-8">
              <div className="max-w-lg">
                <div className="flex items-center gap-2 mb-2">
                  ...
                  {business?.name && (
                    <span className="text-[10px] text-white/60 bg-white/10 backdrop-blur-sm px-2 py-0.5 rounded-full">
                      {business.name}
                    </span>
                  )}
                </div>
