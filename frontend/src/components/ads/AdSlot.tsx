...
const POSITION_STYLES: Record<AdPlacementPosition, string> = {
  HERO_BANNER:
    'relative bg-gradient-to-r from-brand/5 via-emerald-50 to-brand/5 dark:from-brand/10 dark:via-emerald-900/10 dark:to-brand/10 rounded-2xl border border-brand/10 dark:border-brand/20 overflow-hidden',
  TOP_BANNER:
    'relative bg-white dark:bg-gray-900 rounded-xl border border-gray-200 dark:border-gray-800 overflow-hidden',
  BOTTOM_BANNER:
    'relative bg-white dark:bg-gray-900 rounded-xl border border-gray-200 dark:border-gray-800 overflow-hidden',
  SIDEBAR: 'bg-white dark:bg-gray-900 rounded-xl border border-gray-200 dark:border-gray-800 p-4',
  SPONSORED_CARD:
    'bg-gradient-to-br from-gray-50 to-white dark:from-gray-800/50 dark:to-gray-900 rounded-xl border border-gray-100 dark:border-gray-700 hover:border-brand/30 transition-all p-4',
  SPONSORED_RESULT:
    'flex items-center gap-3 p-3 rounded-lg bg-gradient-to-r from-brand/5 to-transparent border border-brand/10',
  CAROUSEL: 'relative rounded-xl overflow-hidden',
  FEATURED_BLOCK:
    'relative bg-gradient-to-br from-brand/10 via-emerald-50/50 to-brand/5 dark:from-brand/20 dark:via-emerald-900/20 dark:to-brand/10 rounded-2xl border border-brand/10 p-6',
  PROMO_WIDGET:
    'bg-gradient-to-r from-brand/5 to-emerald-50 dark:from-brand/10 dark:to-emerald-900/10 rounded-xl border border-brand/10 dark:border-brand/20 p-3',
  RECOMMENDED:
    'bg-white dark:bg-gray-900 rounded-xl border border-gray-200 dark:border-gray-800 p-3',
  POPUP:
    'fixed bottom-4 right-4 w-80 bg-white dark:bg-gray-900 rounded-2xl shadow-2xl border border-gray-200 dark:border-gray-800 overflow-hidden z-50',
};
...
function ReportButton({
  ...
  const doReport = async () => {
    if (!creative || !reportReason) return;
    try {
      ...
      {showReport && (
        <div className="absolute bottom-full left-0 mb-1 w-56 bg-white dark:bg-gray-800 rounded-lg shadow-xl border border-gray-200 dark:border-gray-700 p-3 z-50">
          {reportSent ? (
            ...
            <>
              <p className="text-[10px] font-semibold text-gray-900 dark:text-white mb-2">
                Signaler cette pub
              </p>
              <select
                value={reportReason}
              ...
              <div className="flex gap-1.5">
                <button
                  ...
                  className="flex-1 text-[10px] px-2 py-1 rounded-md border border-gray-200 dark:border-gray-600 text-gray-600 dark:text-gray-400 hover:bg-gray-50 dark:hover:bg-gray-700"
                >
                  Annuler
                </button>
                <button
                  ...
                  className="flex-1 text-[10px] px-2 py-1 rounded-md bg-red-500 text-white hover:bg-red-600 disabled:opacity-50 disabled:cursor-not-allowed"
                >
                  Signaler
                </button>
              </div>
...
export default function AdSlot({ page, position, className, country, dismissible }: AdSlotProps) {
  ...
  const { data: ads, isLoading } = useQuery({
    ...
    queryFn: async () => {
      try {
        ...
        // Ignorer silencieusement les erreurs d'auth (401) ou requêtes invalides (400)
        // sur les pages publiques où l'utilisateur n'est pas connecté.
  ...
  const trackImpression = useCallback(
    async (creative: AdCreative) => {
      try {
        ...
        // Ignorer silencieusement (non-auth, timeout, etc.)
      }
    },
    [page, position]
  );
  ...
  const handleClick = useCallback(
    async (e: React.MouseEvent, creative: AdCreative) => {
      try {
        ...
        <div className="flex items-center gap-4 p-6">
          ...
          <div className="flex-1">
            ...
            <p className="text-lg font-bold text-gray-900 dark:text-white mt-1">
              {creative.adText || (creative as any).campaign?.name || business?.name || 'Publicité'}
            </p>
          </div>
      ...
      <div className="flex items-center gap-3 p-4">
        ...
        <div className="flex-1 min-w-0">
          <div className="flex items-center gap-2">
            ...
            <ReportButton creative={creative} onReportSent={() => {}} />
          </div>
          <p className="text-sm font-semibold text-gray-900 dark:text-white truncate">
            {creative.adText || (creative as any).campaign?.name || business?.name}
          </p>
        </div>
        {creative.destinationUrl && (
          <a
  ...
  const renderCard = () => (
    <div className={cn(POSITION_STYLES.SPONSORED_CARD, className)}>
      ...
      <p className="text-sm font-semibold text-gray-900 dark:text-white mt-2 line-clamp-2">
        {creative.adText || (creative as any).campaign?.name}
      </p>
      {creative.destinationUrl && (
        <a
          href={creative.destinationUrl}
