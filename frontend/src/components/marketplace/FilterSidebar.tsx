...
import { X, ChevronDown, Search, MapPin } from 'lucide-react';
...
const FILTER_STORAGE_KEY = 'afribiz-marketplace-filters';
...
// Auto-load filters from storage on mount
export function usePersistentFilters(initialFilters: FilterState, initialCountry?: string) {
  const [filters, setFilters] = useState<FilterState>(() => {
    const stored = loadFiltersFromStorage();
    const merged = { ...initialFilters, ...stored };
    if (initialCountry && !merged.country) {
      merged.country = initialCountry;
    }
    return merged;
  });
  ...
  const clearFilters = () => {
    ...
    if (initialCountry) reset.country = initialCountry;
    setFilters(reset);
    localStorage.removeItem(FILTER_STORAGE_KEY);
  };

  return { filters, updateFilters, clearFilters, setFilters };
}
      ...
      content: (
        <div className="flex flex-wrap gap-1.5">
          {PROXIMITY_OPTIONS.map((opt) => (
            <button
              key={opt.value}
              onClick={() => update({ proximity: opt.value })}
              className={cn(
                'px-2.5 py-1 rounded-lg text-xs font-medium border transition-colors',
                filters.proximity === opt.value
                  ? 'bg-brand text-white border-brand'
                  : 'border-gray-200 dark:border-gray-700 text-gray-600 dark:text-gray-400 hover:border-brand/30 hover:text-brand'
              )}
            >
              {opt.label}
            </button>
      ...
      content: (
        <div className="space-y-1">
          ...
          {citySearch ? (
            ...
            >
              <MapPin className="h-3.5 w-3.5 text-gray-400 inline mr-1" />
              {citySearch}
            </button>
          ) : (
            (filters.country === 'TG'
                    ...
                    : filters.country === 'BF'
                      ...
                      : filters.country === 'SN'
                        ...
                        : filters.country === 'NG'
                          ...
                          : ['Lomé', 'Cotonou', 'Accra', 'Abidjan', 'Ouagadougou', 'Dakar', 'Lagos']
            ).map((city) => (
              <button
                key={city}
                onClick={() => update({ city })}
      ...
      content: (
        <div className="space-y-1">
          {CATEGORIES.map((opt) => (
            <button
              key={opt.value}
              onClick={() => update({ category: opt.value, subCategory: '' })}
              className={cn(
                'block w-full text-left px-3 py-1.5 rounded-lg text-sm transition-colors',
          ...
          <div className="pt-1">
            <div className="flex items-center justify-between mb-1">
              ...
              <span className="text-xs font-semibold text-brand">
                {filters.minRating > 0 ? `${filters.minRating}+` : 'Tous'}
              </span>
            </div>
            <input
              type="range"
              id="marketplace-rating-filter"
              name="minRating"
              min={0}
              max={5}
              step={0.5}
      ...
      content: (
        <div className="space-y-3">
          <PriceHistogram
            ...
            currentMin={filters.priceMin || filters.price === 'free' ? 0 : undefined}
            currentMax={filters.priceMax || undefined}
            onSelect={(min, max) => update({ priceMin: min, priceMax: max, price: 'custom' })}
          />
  ...
  const content = (
    <div className="space-y-1">
      <div className="flex items-center justify-between px-1 pb-3 mb-2 border-b border-gray-100 dark:border-gray-800">
        ...
        <button
          onClick={clearAll}
          className="text-xs text-brand hover:text-brand-700 dark:hover:text-brand-400 font-medium"
        >
          Tout effacer
        </button>
        {mobile && onClose && (
