      ...
      >
        <div className="w-24 h-24 rounded-xl bg-gray-100 dark:bg-gray-800 flex items-center justify-center overflow-hidden shrink-0">
          {item.image ? (
            ...
            <Home className="h-8 w-8 text-gray-300 dark:text-gray-600" />
          )}
        </div>
          ...
          <div>
            ...
            <div className="flex items-center gap-4 mt-2">
              <span className="text-sm font-bold text-gray-900 dark:text-gray-100">
                {item.dailyRate.toLocaleString()} FCFA/jour
              </span>
              <span className="text-xs text-gray-500 dark:text-gray-400">
                {item.weeklyRate.toLocaleString()} FCFA/sem
              </span>
            </div>
            <p className="text-xs text-gray-400 dark:text-gray-500 mt-1">
              Caution : {item.deposit.toLocaleString()} FCFA
            </p>
          </div>
      ...
      <div className="p-4">
        ...
        <div className="flex items-center justify-between mb-1">
          <span className="text-base font-bold text-gray-900 dark:text-gray-100">
            {item.dailyRate.toLocaleString()} FCFA/j
          </span>
          <span className="text-xs text-gray-500 dark:text-gray-400">
            {item.weeklyRate.toLocaleString()} FCFA/sem
          </span>
        ...
        >
          Réserver - {item.dailyRate.toLocaleString()} FCFA/jour
        </button>
      </div>
    </div>
  );
