...
import type { RentalResult } from './types';
...
export default function RentalCard({ item, view = 'grid' }: RentalCardProps) {
  if (view === 'list') {
    return (
      <div className="flex gap-4 p-4 bg-white dark:bg-gray-900 rounded-xl border border-gray-200 dark:border-gray-800 hover:border-brand/20 hover:shadow-card transition-all duration-200">
        <div className="w-24 h-24 rounded-xl bg-gray-100 dark:bg-gray-800 flex items-center justify-center overflow-hidden shrink-0">
          {item.image ? <Image src={item.image} alt="" width={96} height={96} className="w-full h-full object-cover" unoptimized /> : <Home className="h-8 w-8 text-gray-300 dark:text-gray-600" />}
        </div>
        <div className="flex-1 min-w-0 flex items-center justify-between">
          <div>
            ...
            <div className="flex items-center gap-4 mt-2">
              <span className="text-sm font-bold text-gray-900 dark:text-gray-100">{item.dailyRate.toLocaleString()} FCFA/jour</span>
              <span className="text-xs text-gray-500 dark:text-gray-400">{item.weeklyRate.toLocaleString()} FCFA/sem</span>
            </div>
            <p className="text-xs text-gray-400 dark:text-gray-500 mt-1">Caution : {item.deposit.toLocaleString()} FCFA</p>
          </div>
    ...
    <Link href={`/business/${item.businessSlug || item.id}`}>
      <div className="bg-white dark:bg-gray-900 rounded-xl border border-gray-200 dark:border-gray-800 hover:border-brand/20 hover:shadow-card transition-all duration-200 overflow-hidden group">
        <div className="h-36 bg-gray-100 dark:bg-gray-800 flex items-center justify-center overflow-hidden">
          {item.image ? <Image src={item.image} alt="" width={400} height={144} className="w-full h-full object-cover group-hover:scale-105 transition-transform" unoptimized /> : <Home className="h-10 w-10 text-gray-300 dark:text-gray-600" />}
        </div>
        <div className="p-4">
          <h3 className="text-sm font-semibold text-gray-900 dark:text-gray-100 mb-0.5">{item.name}</h3>
          <p className="text-xs text-gray-500 dark:text-gray-400 mb-2">{item.city}</p>
          <div className="flex items-center justify-between mb-1">
            <span className="text-base font-bold text-gray-900 dark:text-gray-100">{item.dailyRate.toLocaleString()} FCFA/j</span>
            <span className="text-xs text-gray-500 dark:text-gray-400">{item.weeklyRate.toLocaleString()} FCFA/sem</span>
          </div>
          <p className="text-xs text-gray-400 dark:text-gray-500 mb-3">Caution : {item.deposit.toLocaleString()} FCFA</p>
