...
export default function ProductCard({ item, view = 'grid' }: ProductCardProps) {
  ...
  const handleAddToCart = async (e: React.MouseEvent) => {
    e.stopPropagation();
    // Always add to local cart first (works for everyone)
    addToLocalCart({
      id: item.id,
      productId: item.id,
      name: item.name,
  ...
  const handleContactBusiness = (e: React.MouseEvent) => {
    ...
    router.push(
      '/dashboard/messages?business=' +
        ...
        '&businessName=' +
        encodeURIComponent(item.businessName)
    );
  };
  ...
  const handleQuickOrder = async (e: React.MouseEvent) => {
    e.stopPropagation();
    // Add to local cart (handleAddToCart already does this)
    addToLocalCart({
      id: item.id,
      productId: item.id,
      name: item.name,
    ...
    // Redirect to public checkout (gère l'authentification, synchro panier et confirmation)
    ...
    if (badge > 0) {
      return (
        <div className="flex items-center gap-2">
          <span
            className={
              view === 'list' ? 'text-lg font-bold text-brand' : 'text-base font-bold text-brand'
            }
          >
      ...
      <>
        ...
        >
              ...
              <div className="text-gray-300 dark:text-gray-600">
                <svg className="h-8 w-8" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                  <path
                    ...
                    strokeWidth={1.5}
                    d="M3 7h18M3 7l2 14h14l2-14M3 7l3-4h12l3 4"
                  />
                </svg>
              </div>
            )}
          ...
          <div className="flex-1 min-w-0">
            ...
            <div className="flex items-center gap-3 mt-2">{renderPrice()}</div>
            {item.reviews?.[0]?.comment && (
              <p className="text-xs text-gray-400 dark:text-gray-500 italic mt-1 line-clamp-1">
    ...
    <>
      ...
      >
        ...
        <div className="p-4">
          ...
          <div className="mb-1">{renderPrice()}</div>
          {item.reviews?.[0]?.comment && (
            <p className="text-xs text-gray-400 dark:text-gray-500 italic mb-2 line-clamp-1">
              &ldquo;{item.reviews[0].comment}&rdquo;
            </p>
          )}
          {renderActions(false)}
