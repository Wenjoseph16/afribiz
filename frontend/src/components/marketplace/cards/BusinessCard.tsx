    ...
    return (
      <div className="flex gap-4 p-4 bg-white dark:bg-gray-900 rounded-xl border border-gray-200 dark:border-gray-800 hover:border-brand/20 dark:hover:border-brand/30 hover:shadow-card transition-all duration-200 group">
        <div className="w-20 h-20 rounded-xl bg-gradient-to-br from-brand-50 to-brand-100 dark:from-brand-900/30 dark:to-brand-800/30 flex items-center justify-center text-2xl font-bold text-brand shrink-0 overflow-hidden">
          {item.logo ? (
            <Image
              src={toMediaUrl(item.logo) || item.logo}
              alt=""
              width={80}
              height={80}
          ...
          <div className="flex items-center justify-between mt-3">
            <div className="flex gap-1.5 flex-wrap">
              {item.modules.map((m) => (
                <span
                  key={m}
                  className="text-[10px] font-medium px-2 py-0.5 rounded-full bg-gray-100 dark:bg-gray-800 text-gray-600 dark:text-gray-400"
                >
                  {m}
                </span>
              ))}
            ...
            <div className="flex gap-2">
              <div className="flex items-center gap-2">
                <Link
                  ...
                  className="text-xs font-medium text-brand hover:text-brand-700 dark:hover:text-brand-400 px-3 py-1.5 rounded-lg border border-brand/20 hover:bg-brand-50 dark:hover:bg-brand-900/20 transition-colors"
                >
                  Voir le business
                </Link>
                <button
                  onClick={handleContactBusiness}
                  className="text-xs font-medium px-3 py-1.5 rounded-lg border border-purple-200 dark:border-purple-800 text-purple-600 dark:text-purple-400 hover:bg-purple-50 dark:hover:bg-purple-900/20 transition-colors inline-flex items-center gap-1.5"
                >
              ...
              {item.modules.includes('Produits') && (
                <Link
                  href={`/business/${item.slug || item.id}`}
                  className="text-xs font-medium text-white bg-brand hover:bg-brand-700 px-3 py-1.5 rounded-lg transition-colors inline-block"
                >
                  Commander
                </Link>
              )}
    ...
    <div className="bg-white dark:bg-gray-900 rounded-xl border border-gray-200 dark:border-gray-800 hover:border-brand/20 dark:hover:border-brand/30 hover:shadow-card transition-all duration-200 group overflow-hidden">
      <div className="p-5">
        ...
        <div className="flex gap-2">
          <button
            onClick={handleContactBusiness}
            className="flex items-center justify-center gap-1 text-xs font-medium px-2 py-2 rounded-lg border border-purple-200 dark:border-purple-800 text-purple-600 dark:text-purple-400 hover:bg-purple-50 dark:hover:bg-purple-900/20 transition-colors"
          >
            <MessageCircle className="w-3 h-3" />
            Message
          </button>
          <Link
            href={`/business/${item.slug || item.id}`}
            className="flex-1 text-center text-xs font-medium text-brand border border-brand/20 rounded-lg py-2 hover:bg-brand-50 dark:hover:bg-brand-900/20 transition-colors"
          >
            Voir le business
          </Link>
          {item.modules.includes('Produits') && (
            <Link
              href={`/business/${item.slug || item.id}`}
              className="flex-1 text-center text-xs font-medium text-white bg-brand rounded-lg py-2 hover:bg-brand-700 transition-colors inline-block"
            >
              Commander
            </Link>
          )}
