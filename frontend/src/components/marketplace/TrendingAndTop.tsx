        ...
        <div className="flex items-center gap-2">
          ...
          <div>
            <h2 className="text-lg font-bold text-gray-900 dark:text-white">
              <Flame className="h-5 w-5 text-orange-500 inline" /> Tendances
              {items.some((i) => i.city) && ` à ${items.find((i) => i.city)?.city}`}
            </h2>
      ...
      {items.length > 4 && (
        <div className="mt-4 text-center">
          <Link
            ...
            className="inline-flex items-center gap-1 text-sm font-medium text-brand hover:text-brand-700 dark:hover:text-brand-400 transition-colors"
          >
...
export function TopBusinessSection({
  items,
  isLoading,
}: {
  items: PremiumBusiness[];
  ...
  const sorted = useMemo(() => {
    const sorted = [...items].sort((a, b) => {
      if (period === 'today') return (b.rating || 0) - (a.rating || 0);
      if (period === 'week') return (b.reviewCount || 0) - (a.reviewCount || 0);
      if (period === 'month')
        ...
        {sorted.map((item, idx) => (
          ...
          >
            ...
            <div className="flex-1 min-w-0">
              <p className="text-sm font-medium text-gray-900 dark:text-gray-100 group-hover:text-brand transition-colors truncate">
                {item.name}
              </p>
