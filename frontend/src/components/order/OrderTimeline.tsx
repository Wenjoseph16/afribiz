...
const STATUS_COLORS: Record<string, { active: string; completed: string; text: string }> = {
  PENDING: {
    ...
    completed: 'bg-amber-100 border-amber-300 dark:bg-amber-900/30',
    text: 'text-amber-600',
  },
  ACCEPTED: {
    active: 'bg-blue-500 border-blue-500',
    completed: 'bg-blue-100 border-blue-300 dark:bg-blue-900/30',
    text: 'text-blue-600',
  },
  PREPARING: {
    active: 'bg-purple-500 border-purple-500',
    completed: 'bg-purple-100 border-purple-300 dark:bg-purple-900/30',
    text: 'text-purple-600',
  },
  READY: {
    active: 'bg-teal-500 border-teal-500',
    completed: 'bg-teal-100 border-teal-300 dark:bg-teal-900/30',
    text: 'text-teal-600',
  },
  DELIVERED: {
    active: 'bg-emerald-500 border-emerald-500',
    completed: 'bg-emerald-100 border-emerald-300 dark:bg-emerald-900/30',
    text: 'text-emerald-600',
  },
  COMPLETED: {
    active: 'bg-gray-600 border-gray-600',
    completed: 'bg-gray-100 border-gray-300 dark:bg-gray-800',
    text: 'text-gray-600 dark:text-gray-400',
  },
  CANCELLED: {
    active: 'bg-red-500 border-red-500',
    completed: 'bg-red-100 border-red-300 dark:bg-red-900/30',
    text: 'text-red-600',
  },
  REFUSED: {
    active: 'bg-red-500 border-red-500',
  ...
  return (
    <div className="bg-white dark:bg-gray-800 rounded-2xl border border-gray-200 dark:border-gray-700 p-5 sm:p-6">
      <div className="flex items-center justify-between mb-5">
        ...
        {orderNumber && <span className="text-xs text-gray-500 font-mono">{orderNumber}</span>}
      </div>
        ...
        <div className="flex items-center gap-2.5 p-3 mb-5 rounded-xl bg-amber-50 dark:bg-amber-950/30 border border-amber-200 dark:border-amber-800/50">
          ...
          <div>
            ...
            <p className="text-xs text-amber-600 dark:text-amber-400">
              Le vendeur va bientôt prendre en charge votre commande
            </p>
          </div>
        </div>
      )}
                    ...
                    <span className="text-[10px] text-gray-400 dark:text-gray-500 shrink-0">
                      ...
                      {pendingDate && step.status !== 'PENDING' && (
                        <span className="ml-1 text-gray-400">
                          ({getDuration(step.date, pendingDate)})
                        </span>
                      )}
                    </span>
                  )}
