        ...
        <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-6">
          {promotions.map((promo) => {
            ...
            return (
              <div
                key={promo.id}
                className={`relative bg-white dark:bg-gray-800 rounded-xl shadow-sm border overflow-hidden ${isExpired ? 'border-gray-200 dark:border-gray-700 opacity-60' : 'border-brand-200 dark:border-brand-800'}`}
              >
                {promo.image && (
                ...
                <div className="p-5">
                  <div className="flex items-start justify-between mb-2">
                    ...
                    <span
                      className={`flex items-center gap-1 px-2 py-1 rounded-full text-xs font-medium ${promo.discountType === 'PERCENTAGE' ? 'bg-red-100 dark:bg-red-900/30 text-red-600 dark:text-red-400' : 'bg-green-100 dark:bg-green-900/30 text-green-600 dark:text-green-400'}`}
                    >
                  ...
                  <div className="flex items-center justify-between text-xs text-gray-500 dark:text-gray-400">
                    {promo.startsAt && (
                      <span>Du {new Date(promo.startsAt).toLocaleDateString('fr-FR')}</span>
                    )}
                    {promo.endsAt && (
                      <span>au {new Date(promo.endsAt).toLocaleDateString('fr-FR')}</span>
                    )}
                  </div>
                  {isExpired && (
                    <span className="mt-2 inline-block text-xs text-red-500">Expirée</span>
                  )}
                </div>
              </div>
            );
