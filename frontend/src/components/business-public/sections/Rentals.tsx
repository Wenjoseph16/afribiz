            ...
            >
              ...
              <div className="p-5">
                ...
                <div className="space-y-2 text-sm text-gray-500 dark:text-gray-400 mb-4">
                  <div className="flex items-center justify-between">
                    ...
                    <span className="font-semibold text-gray-900 dark:text-white">
                      {formatPrice(Number(rental.price), rental.currency)}/{rental.unit}
                    </span>
                  </div>
                  {rental.weeklyPrice != null && (
                    <div className="flex items-center justify-between">
                      ...
                      <span className="font-semibold text-gray-900 dark:text-white">
                        {formatPrice(Number(rental.weeklyPrice), rental.currency)}/sem
                      </span>
                    </div>
                  )}
                  {rental.deposit != null && (
                    <div className="flex items-center justify-between">
                      <span className="flex items-center gap-1">
                        <Shield className="w-3.5 h-3.5" /> Caution
                      </span>
                      <span className="font-semibold text-gray-900 dark:text-white">
                        {formatPrice(Number(rental.deposit), rental.currency)}
                      </span>
                    </div>
                  )}
                </div>
                ...
                <div className="flex items-center justify-between pt-3 border-t border-gray-100 dark:border-gray-700">
                  <span
                    className={`text-xs ${rental.isAvailable ? 'text-green-600 dark:text-green-400' : 'text-red-500'}`}
                  >
