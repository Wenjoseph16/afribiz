...
test.describe('Ads Management', () => {
  test('ads page loads for business users', async ({ page }) => {
    ...
    await page.locator('input[name="identifier"]').fill('boutique@afribiz.com');
    await page.locator('input[name="password"]').fill('Afribiz@2026!');
...
test.describe('Public Business Page', () => {
  test('public business page loads with sections', async ({ page }) => {
    await page.goto('/business/saveur-dabidjan', { waitUntil: 'networkidle', timeout: 30000 });
    await expect(page.locator('h1').first()).toBeVisible({ timeout: 10000 });
    await expect(page.locator('text=Saveur d\'Abidjan').first()).toBeVisible({ timeout: 5000 });
  });
});
